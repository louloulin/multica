...
This document covers advanced configuration for self-hosted Lumen deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://lumen:lumen@localhost:5432/lumen?sslmode=disable` |
...
Lumen supports two email backends. `SMTP_HOST` takes priority when set; otherwise `RESEND_API_KEY` is used. With neither configured, verification codes are printed to the server log — copy them from there to log in.
...
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@lumen.ai`) |
...
> **Note:** If neither Resend nor SMTP is configured, generated verification codes are printed to backend logs — copy them from there to log in. A fixed local testing code (e.g. `888888`) is **opt-in only**: set `LUMEN_DEV_VERIFICATION_CODE=888888` in `.env` and keep `APP_ENV` non-production. The Docker self-host stack pins `APP_ENV=production`, so the shortcut is ignored there. **Never enable a fixed code on a publicly reachable instance.**
...
If the frontend and backend are served from different hostnames, `COOKIE_DOMAIN` is **required**, not optional: the browser must be able to read the `lumen_csrf` cookie from the page's own origin to send the `X-CSRF-Token` header, and without it every write request fails with `403 {"error":"CSRF validation failed"}`. Scope it to the narrowest parent domain that covers both hosts — it also shares the `lumen_auth` session cookie with every host under that domain. See [Reverse Proxy](#reverse-proxy) for the full split-domain configuration and its trust requirements.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `LUMEN_WECOM_TRACE` | empty (off) | `1` records every WeCom frame the backend reads and writes, including the first 120 runes of each message body. Anything else is off. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `LUMEN_SERVER_URL` | `ws://localhost:8080/ws` | WebSocket URL for daemon → server connection |
| `LUMEN_APP_URL` | `http://localhost:3000` | Frontend URL for CLI login flow |
| `LUMEN_DAEMON_POLL_INTERVAL` | `30s` | Catch-up poll for tasks; WebSocket wake signals normally deliver work sooner |
| `LUMEN_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` | Upper bound for healthy WebSocket claim safety polls, configured independently of `LUMEN_DAEMON_POLL_INTERVAL`; downward jitter keeps normal polls at `2m30s`–`2m45s`, while old servers and uncertain claims retain the ordinary poll interval |
| `LUMEN_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat frequency |
...
| `LUMEN_COPILOT_PATH` | Custom path to the `copilot` (GitHub Copilot CLI) binary |
...
| `LUMEN_GROK_PATH` | Custom path to the `grok` binary |
| `LUMEN_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |

## Database Setup

Lumen requires PostgreSQL 17. It does **not** use pgvector — no migration
...
# Kubernetes
kubectl -n lumen exec deploy/lumen-backend -- \
  ./backfill_task_usage_hourly --sleep-between-slices=2s
```
...
Pin `LUMEN_IMAGE_TAG` in `.env` to an exact release like `v0.2.4` if you want to stay on a specific version. Migrations run automatically on backend startup. They are idempotent — running them multiple times has no effect.
