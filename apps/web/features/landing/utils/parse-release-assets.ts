 ...
 *   lumen-desktop-{version}-mac-{arch}.{dmg|zip}
 *   lumen-desktop-{version}-windows-{arch}.exe
 *   lumen-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^lumen-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
