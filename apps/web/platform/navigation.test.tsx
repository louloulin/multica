/**
 * MUL-5208 — the web half of the `lumen:navigate` bridge.
 *
...
import { useNavigation, type NavigationAdapter } from "@lumen/views/navigation";

function navigate(path: string) {
  window.dispatchEvent(
    new CustomEvent("lumen:navigate", { detail: { path } }),
  );
}
...
describe("WebNavigationProvider internal link bridge", () => {
  ...
  it("ignores an event without a path", () => {
    ...
    window.dispatchEvent(new CustomEvent("lumen:navigate", { detail: {} }));
 ...
 * or replaces with a fallback. A wrong `true` walks the user out of Lumen,
