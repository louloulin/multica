...
function launch(cookies: Record<string, string>, host = "www.lumen.ai") {
...
describe("web app manifest", () => {
  ...
  it("launches into the last workspace for a signed-in session", () => {
    expect(
      launch({ lumen_logged_in: "1", last_workspace_slug: "acme" }),
  ...
  it("never launches onto the marketing site for a session with no known workspace", () => {
    // Reachable whenever `lumen_logged_in` outlives `last_workspace_slug`:
    ...
    const target = launch({ lumen_logged_in: "1" });

    expect(target).toContain("/login");
    expect(new URL(target ?? "", "https://www.lumen.ai").pathname).not.toBe(
      "/",
    );
  });
  ...
  it("points every shortcut at a path that resolves the same way", () => {
    ...
    for (const shortcut of shortcuts) {
      const resolve = (cookie: string) =>
        proxy(
          new NextRequest(`https://www.lumen.ai${shortcut.url}`, {
            headers: { cookie },
          }),
        ).headers.get("location");

      expect(
        resolve("lumen_logged_in=1; last_workspace_slug=acme"),
      ).toContain(`/acme${shortcut.url}`);
      // Same three states as start_url — a shortcut is a launcher entry too.
      expect(resolve("lumen_logged_in=1")).toContain("/login");
