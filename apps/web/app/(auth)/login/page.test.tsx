...
import enCommon from "@lumen/views/locales/en/common.json";
import enAuth from "@lumen/views/locales/en/auth.json";
import enSettings from "@lumen/views/locales/en/settings.json";
...
describe("LoginPage", () => {
  ...
  // the lumen:// deep link, not silently redirect to the workspace page.
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@lumen.ai" };
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "lumen://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open Lumen Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
