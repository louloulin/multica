...
import { createPersistStorage, defaultStorage } from "@lumen/core/platform";
import { createSafeId } from "@lumen/core/utils";
 ...
 * lumen-ai#6405) to invalidate restoration when the content changed
...
// render time via `routeIconForPath` (@lumen/views/layout), which shares the
// route → icon map in `@lumen/core/paths` with the sidebar nav — so the two
