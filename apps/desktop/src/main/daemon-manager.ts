...
import { clientDiagnostics } from "@lumen/core/diagnostics";
...
const PREFS_PATH = join(homedir(), ".lumen", "desktop_prefs.json");
...
// `lumen daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/lumen/cmd_daemon.go) to
 ...
 * a stuck start is an auth problem. Hits the same endpoint `lumen auth status`
 ...
 * the user's default CLI profile at `~/.lumen/` — neither its files nor its
 * health port.
 */
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["lumen.exe"] : ["lumen"];
 ...
 *   to `apps/desktop/resources/bin/lumen`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Lumen.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "lumen.exe" : "lumen";
...
/**
 * Returns a usable `lumen` binary path. Priority:
 ...
 *   5. `lumen` on PATH (dev convenience / user-installed via brew).
  ...
  const res = await withTimeout(
    (signal) =>
      fetch(url, {
        ...
        body: JSON.stringify({ name: "Lumen Desktop" }),
        signal,
      }),
    15_000,
  );
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, LUMEN_LAUNCHED_BY: "desktop" };
}
...
async function startDaemon(
  ...
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "lumen CLI is not installed" };
