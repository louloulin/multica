...
// lumen CLI can find agent binaries like claude/codex/opencode. Must run
...
function installLocaleRefresh(window: BrowserWindow): void {
  // Electron has no dedicated OS-language event. Check whenever any Lumen
...
// single-worktree case still lands at "Lumen Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Lumen Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Lumen Canary";
...
if (is.dev) {
  ...
  // to "Lumen", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.lumen.desktop.dev" : "ai.lumen.desktop",
    );
