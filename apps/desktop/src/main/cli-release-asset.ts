const RELEASE_ARCHIVE_PREFIX = "lumen-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `lumen-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `lumen_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `lumen_${os}_${mappedArch}.${ext}`;
