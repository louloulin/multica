...
// (keyed by the app name "Lumen Canary"). The env hooks to override both
...
// "/tmp/lumen-3494" is 1189739000, and 1189739000 % 1000 === 0). Range 5174–6173.
...
// The dev app then shows e.g. "Lumen Canary mul-3724-194" in Cmd+Tab and gets
...
// a basename at different paths (e.g. /a/lumen vs /b/lumen) or whose names
...
// the primary checkout keeps the unchanged 5173 / "Lumen Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "Lumen Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
