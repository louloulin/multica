...
describe("worktree-dev-env", () => {
  ...
  it("never reuses 5173 even when the offset is 0", () => {
    // POSIX cksum("/tmp/lumen-3494") === 1189739000, % 1000 === 0
    expect(offsetForPath("/tmp/lumen-3494")).toBe(0);
    expect(rendererPortForPath("/tmp/lumen-3494")).toBe(5174);
    expect(rendererPortForPath("/tmp/lumen-3494")).not.toBe(5173);
  });
  ...
  it("disambiguates worktrees that share a folder name at different paths", () => {
    // Same basename "lumen", different parent dirs → different offsets/suffixes,
    // so each gets its own single-instance lock.
    expect(offsetForPath("/tmp/a/lumen")).not.toBe(offsetForPath("/tmp/b/lumen"));
    expect(appSuffixForPath("/tmp/a/lumen")).not.toBe(
      appSuffixForPath("/tmp/b/lumen"),
    );
  });
