...
<Callout type="info">
Before an agent can execute, the computer that runs it needs at least one [supported AI coding tool](/providers) installed and signed in — for example Claude Code, Codex, or Cursor. Lumen does not ship these tools.
</Callout>

## 1. Sign in and open your workspace

Sign in on the web ([lumen.ai](https://lumen.ai)) or with [Lumen Desktop](https://lumen.ai/download). Email verification codes and Google are both supported.
...
- **With Web, or to add another computer**: click **Add a computer** in the top right, then run the two commands from the dialog in a terminal on the target computer:
  ...
  **Windows PowerShell**
  ...
  `lumen setup` opens a browser to complete sign-in, then leaves the daemon running in the background. Once the daemon is online, the dialog usually detects the computer within a minute.
...
3. Submit. Lumen creates the issue, sets the agent as the assignee, and starts executing immediately.

![New issue dialog in agent mode: Created by set to Lumen Assistant](/images/docs/quickstart-new-issue.webp)
...
| Symptom | Check first |
|---|---|
| No runtime found | Confirm the AI coding tool is installed and runs in a terminal. On Desktop, click **Refresh**; on the CLI path, rerun `lumen setup`. |
| Runtime shows offline | Keep Desktop open; on the CLI path, run `lumen daemon status`, and `lumen daemon start` if it is not running. |
...
- [How Lumen works](/how-lumen-works) — how issues, agents, runtimes, and each run relate.
