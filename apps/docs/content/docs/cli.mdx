---
title: Using the CLI
description: Install and log in to the Lumen CLI, and manage workspaces, issues, agents, and runtimes from the terminal.
---

import { Callout } from "fumadocs-ui/components/callout";

The Lumen CLI both connects local runtimes and manages workspaces, issues, agents, and automations from the terminal. This page covers the common paths; the flags your installed version supports are always defined by each command's own `--help`.
...
If you only need to sign in again without overwriting other configuration, run `lumen login`. On a machine without a browser, create a personal access token in the web settings first, then enter it with:
...
Subsequent commands use this workspace. A single command can override it with `--workspace-id`, or you can set `LUMEN_WORKSPACE_ID`.

Invite members:

```bash
lumen workspace member invite teammate@example.com
lumen workspace member invite admin@example.com --role admin
```
...
# View and search
lumen issue list
lumen issue get LUM-123
lumen issue search "login failure"

# Create and update
lumen issue create --title "Fix login failure"
lumen issue status LUM-123 in_progress
lumen issue assign LUM-123 --to "Backend Agent"

# Comments and runs
lumen issue comment list LUM-123
lumen issue comment add LUM-123 --content "Check the regression tests first"
lumen issue comment update <comment-id> --expected-revision 2 --content "Check the regression tests and release notes"
lumen issue runs LUM-123
```

Read long descriptions or comments from stdin to avoid dealing with newlines and quoting:

```bash
lumen issue create --title "Upgrade notes" --description-stdin < notes.md
lumen issue comment add LUM-123 --content-stdin < review.md
lumen issue comment update <comment-id> --expected-revision 2 --content-file revised-review.md
```

View the messages of a single run, or stop one:

```bash
lumen issue run-messages <task-id> --issue LUM-123
lumen issue cancel-task <task-id> --issue LUM-123
```
...
```bash
lumen agent list
lumen agent get <agent-id>
lumen agent create --help
lumen agent update <agent-id> --help

lumen skill list
lumen skill get <skill-id>
lumen skill import --url <skill-url>
lumen agent skills add <agent-id> --skill-ids <skill-id>
```

When an import hits a skill with the same name, the default is to stop without modifying existing content. Pick by intent:

```bash
lumen skill import --url <skill-url> --on-conflict overwrite
lumen skill import --url <skill-url> --on-conflict rename
lumen skill import --url <skill-url> --on-conflict skip
```
...
lumen daemon logs --follow

lumen runtime list
lumen runtime rename <runtime-id> "Office Mac"
lumen runtime usage <runtime-id>
lumen runtime activity <runtime-id>
```

Deleting a runtime that still has active agents bound to it is refused by default. `lumen runtime delete <id> --cascade` unbinds those agents, preserves their configuration and history, and cancels their active runs.
...
`lumen chat` reads **the external chat session an agent is currently handling**; it is mainly for agents in chat integrations, not a general command for browsing arbitrary workspace conversations.

## IDs and output formats

Issues use keys like `LUM-123` or full UUIDs; short UUID prefixes are not accepted.
...
| | `mcp list/add/update/remove` | Manage the workspace's MCP server library | Writes are owner/admin. A server added here is assigned to no agent — give it to one with `lumen agent mcp add`. `list` shows names and transports only; the stored entry is write-only and never returned. `--server-config-file` / `--server-config-stdin` keep tokens out of shell history |
...
| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `daemon` | `start` | Start the local daemon | `--foreground`, `--device-name`, `--runtime-name`, `--workspaces-root`, `--poll-interval`, `--heartbeat-interval`, `--agent-timeout` (`0` means no limit), `--max-concurrent-tasks`, `--no-auto-update`; each has a matching `LUMEN_*` environment variable |
...
## Driving Lumen from another coding agent

If most of your work already happens inside Codex, Claude Code, or Cursor, you can
operate Lumen from there instead of switching to a terminal. The
[Lumen CLI skill](https://github.com/lumen-ai/lumen-cli) teaches those agents
...
- [Lumen CLI skill](https://github.com/lumen-ai/lumen-cli) — operate Lumen from Codex, Claude Code, or Cursor.
