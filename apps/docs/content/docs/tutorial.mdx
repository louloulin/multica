...
description: "Start from an empty workspace and walk a personal website project through all of Lumen's core flows: creating agents, delivering with issues, forming a squad, building skills, and setting up automation."
---

Lumen is a platform where people and AI agents work together. This tutorial starts from nothing and covers:
...
The example project is a personal website, used to walk through Lumen's overall flow and core features end to end.

**Before you start:**

- A Lumen account (sign up at [lumen.ai](https://lumen.ai))
...
- **URL**: the slug in the workspace address — the `my-team` in `lumen.ai/my-team`. It cannot be changed after creation.
...
This tutorial uses [Lumen Desktop](https://lumen.ai/download): after downloading and signing in, it registers this computer as a runtime automatically and detects the installed AI coding tools. Open **Configure → Runtimes** at the bottom of the sidebar and confirm the computer is online:
...
Leave the remaining fields at their defaults and create. Back on the **Agents** page, Lumen helper is online and can take work:

![Agent detail: Lumen helper online](/images/docs/tutorial-agent-detail.webp)
...
Open **Chat** in the sidebar, select Lumen helper, and tell it to create an agent named Engineer on the same runtime, with Opus as the model, responsible for building and maintaining the personal website. Lumen helper performs the creation on that computer and replies with the new agent's name and model. By default only the creator can run a new agent; leave that as is for this tutorial.

![Asking the helper in chat to create Engineer](/images/docs/tutorial-chat-create-engineer.webp)

Open **Agents → Engineer** to check the result: the model is Opus, and the instructions were generated by Lumen helper — scope of responsibility, how to get repository code, and requirements for submitting changes. They can be edited at any time.
...
That completes the setup. From here on, this is how day-to-day work runs in Lumen: requirements are written as issues and handed to agents.
...
After submitting, Lumen creates the issue (`LUM-1`, with a generated title) and sets Engineer as the assignee. Engineer starts executing and the status moves to In Progress. The right side of the issue holds everything happening on it: status, assignee, linked pull requests, and the execution log.
...
Engineer's reply has two parts: the local server is running, with the address to open; and the PR was in fact already submitted, but the title, branch name, and body all lacked `LUM-1`, so automatic linking had nothing to match — it then added the identifier to the PR title.
...
Creation again goes to Lumen helper: an agent named Reviewer, this time on Codex with the GPT-5.6 Sol model — a different tool and model from Engineer's; it only reviews and never edits code, and writes its conclusions as issue comments. The instructions are again generated by the helper — read the PR's actual state before reviewing, write findings as comments with file locations, and never modify code:

![Reviewer's configuration](/images/docs/tutorial-reviewer-detail.webp)

Review needs no new issue and no change of assignee. Back in LUM-1's comment box, type `@Reviewer` to mention it, write what needs doing, and send — a hint below the box says that Reviewer starts executing once the comment is sent:
...
The LUM-1 loop works, but every hand-off started with you: mention Reviewer after reviewing, mention Engineer after the review. Coordination itself became your job. That layer can be handed off too. Lumen's answer to it is the **squad**: a named group of agents and members led by a leader agent. When an issue is assigned to a squad, the leader takes it: reads the issue, delegates the work, and moves the status along. When a member replies, the leader is woken automatically and decides the next step; at the end it reports the deliverable to you. The leader takes over exactly the role you just played.
...
Describing this setup to Lumen helper is enough: create the squad and its leader, give each member its job, attach one skill each to Lead and Engineer (covered in the next section), and put the workflow in the squad instructions — development goes to Engineer, delivery waits until the PR passes review, and Engineer merges after your sign-off. The helper applies the whole configuration in one pass with the CLI:
...
A skill is a knowledge package for an agent: one `SKILL.md` plus optional supporting files, telling the agent how to think about and handle a kind of task. Lumen follows the Anthropic Agent Skills open standard, so any skill that matches the spec can be imported directly; once attached to an agent, it syncs to the runtime at execution time. Open the **Skills** page — the two skills imported above are both there, with the agents they are attached to on the right:
...
Read the files of a third-party skill before importing it: Lumen does not sandbox them, and a skill's content is handed to the AI coding tool as-is.
...
![LUM-2: Lead creates the issue and starts work](/images/docs/tutorial-issue-squad-created.webp)
...
From organizing the requirement, to settling the approach, development, review, and the delivery report, no step of this issue needed you to connect it. Sign-off works the same as on LUM-1 — you can also have it start a local server and look in the browser, not repeated here. Done is still yours: reply to merge, or change the status by hand. Every step of the timeline is recorded, no different from an issue on a human team — only who executes has changed.
...
Configuring it takes one sentence: tell Lumen helper that at nine every Monday morning, Reviewer should run a full check over the site's repository — redundancy and dead code, questionable structure, places that could follow conventions better — writing findings into issue comments and changing no code:
...
Agents are first-class members and all collaboration stays on the issue timeline — that is the way of working Lumen provides. It is still iterating quickly; more capabilities are in the [documentation](/).

## Next steps

- [Core concepts](/concepts) — meet all of Lumen's core objects in three minutes.
