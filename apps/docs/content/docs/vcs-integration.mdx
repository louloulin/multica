...
description: Connect a self-hosted Forgejo, Gitea, or GitLab instance per workspace so pull/merge requests that carry an issue identifier in their branch name or title, or after a closing keyword in the body, auto-attach to that issue, move it to Done on merge, and show CI status.
---

import { Callout } from "fumadocs-ui/components/callout";

<Callout type="warn">
**Self-hosted Lumen only.** This integration is available only when you run
Lumen yourself; it is not offered on Lumen Cloud. "Self-hosted" here means
**Lumen** is self-hosted — typically so it can reach a Git instance on your own
network. A server operator must enable it (`LUMEN_VCS_INTEGRATION_ENABLED=true`)
and set `LUMEN_VCS_SECRET_KEY`; until then the section does not appear in
Settings → Integrations.
</Callout>

Lumen connects to a self-hosted Git provider per workspace: **Forgejo**, **Gitea**, or **GitLab**. Once connected, any pull request (GitLab: merge request) whose branch name or title contains an issue identifier (for example `LUM-123`), or whose body names one after a closing keyword (`Closes LUM-123`), is **auto-linked** to that [issue](/issues), appears under **Pull requests** in the issue sidebar, and — when merged with a closing keyword — moves the issue to **Done**. CI for the head commit shows as a checks bar on the card.
...
```sh
LUMEN_VCS_INTEGRATION_ENABLED=true
```
...
```sh
LUMEN_VCS_SECRET_KEY=<base64 32-byte key>
```
...
Set `LUMEN_PUBLIC_URL` to the server's public base URL so Lumen can show a ready-to-paste webhook URL. Without it, the UI shows only the webhook path and you prepend your own origin.
...
2. In Lumen, open **Settings → Integrations → Git providers**.
3. Choose the **provider**, enter the instance URL (for example `https://forgejo.example.com`) and the access token, then **Connect**. Lumen validates the token against the instance before saving.
...
Lumen authenticates every delivery against the stored secret, so a webhook without the matching secret is rejected.

## What gets mirrored

- **Pull / merge requests** — open, closed, merged, and draft state, author, branch, and (where the provider supplies them) diff stats.
- **Issue links** — an identifier in the title or branch links the PR to the issue, as does an identifier after a closing keyword in the body; a bare mention in the body links nothing. A closing keyword (`Closes`/`Fixes`/`Resolves LUM-123`) on a merged PR moves the issue to **Done** once no linked PR is still open.
