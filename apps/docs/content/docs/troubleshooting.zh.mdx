...
先确认问题发生在哪一层：Lumen 服务、守护进程、运行时，还是 AI 编程工具。下面几条命令通常足以找到第一条有效错误：
...
自托管部署还需要从**执行电脑**请求 API 的 `/health`——服务器本机测试无法暴露执行电脑侧的 DNS、TLS 或防火墙问题。需要修改地址时，重新运行 `lumen setup self-host`，或检查当前 profile 的 `server_url`：
...
如果运行时列表缺少预期工具，先在同一个系统账号和 `PATH` 中确认工具可以运行并且已经登录，再执行 `lumen daemon restart`。

### `waiting_local_directory` 状态

这表示另一条在途运行正在使用同一个本地目录。Lumen 会等待目录锁释放，避免两个智能体同时修改同一份文件。
...
否则通常只需等前一次运行结束。若前一次已经卡住，可以从它的执行日志中停止；也可以为当前智能体选择其他本地目录。这个目录互斥由守护进程在内存中维护，不写磁盘锁文件；怀疑锁状态异常时运行 `lumen daemon restart` 即可释放，没有需要手动删除的文件。
...
`SMTP_HOST` 和 Resend 同时配置时，Lumen 会优先使用 SMTP。配置方式见[登录与注册](/auth-setup#邮箱验证码)。
...
如果它是另一个 Lumen checkout，先在那个目录运行 `make stop`。否则正常停止占用端口的程序，或修改当前服务端口。公网 80/443 由 Caddy、Nginx 等反向代理监听。

## 日志位置

| 组件 | 查看方式 |
| --- | --- |
| 后台守护进程 | `lumen daemon logs --lines 100` |
| 实时跟随守护进程日志 | `lumen daemon logs --follow` |
| 默认 profile 的日志文件 | `~/.lumen/daemon.log` |
| 默认 profile 的启动或崩溃日志 | `~/.lumen/daemon.err.log` |
| 命名 profile | `~/.lumen/profiles/<name>/` 下的对应日志 |
| Docker backend | `docker compose -f docker-compose.selfhost.yml logs -f backend` |
| 浏览器 | 开发者工具中的 Console 和 Network |

哪个文件是当前活跃的日志，取决于守护进程启动时使用的 profile；早前守护进程残留的旧日志同样能正常读出来，最容易让人调错文件。不要靠猜去开文件：`lumen daemon logs` 会在输出日志内容前先打印它解析到的绝对路径。读命名 profile 的日志时加上 `--profile <name>`。
...
仍然无法定位时，到 [GitHub 任务](https://github.com/lumen-ai/lumen/issues) 搜索已有问题或提交新的任务。
