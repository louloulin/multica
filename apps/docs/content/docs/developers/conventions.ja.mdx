...
Lumen のコードを書く、翻訳を変更する、あるいは中国語の製品コピーを書く場合は、このページを参照してください。
...
| `packages/ui` | なし | `@lumen/core`, ビジネスロジック |
...
| `apps/mobile/` | `@lumen/core` の型と純粋関数 | Web/Desktop の React ページ、store、プラットフォーム実装 |
...
すべてのタスクには `LUM-123` のような人が読めるキーがあります。ワークスペースの `issue_prefix`（大文字と数字、通常 3 文字、最大 10 文字）+ 連番です。ワークスペースの admin は Settings → General で接頭辞を変更できますが、変更すると既存のすべてのタスクが番号を振り直されるため、古い接頭辞が埋め込まれた外部参照（PR タイトル、ブランチ名、ドキュメントやチャット内のリンク）は解決されなくなります。
...
### 中核となる区別: 日常語 vs Lumen 固有語

Lumen の製品名詞は 2 つのカテゴリに分かれます。

- **日常語** — ユーザーが口に出して言う言葉。データベースのエンティティかどうかに関わらず**完全に翻訳**します: `issue` → タスク、`workspace` → ワークスペース、`project` → プロジェクト。
- **Lumen 固有語** — その言語に対応する語がない概念（`skill`）、またはユーザーが入力したり突き合わせたりするスキーマレベルの識別子（`todo`、`in_progress`、`task_id`）。**小文字の英語**で表記し、型名のように読ませます。
...
- **コード参照とコマンドそのもの**は英語のまま: `lumen issue ...`、Slack と Lark の `/issue` スラッシュコマンド。
- **`skill`** は中国語テキストでも小文字の英語のまま — 定着した訳語がない Lumen 固有の概念です。タイトルでは `Skills` のように大文字にできます。
- **「問題」の意味の `issue`**（ランタイムのヘルス）はエンティティではなく普通名詞です。マシンカードの `{{count}} issues` は `{{count}} 个异常` / `問題 {{count}} 件` / `문제 {{count}}개` であり、エンティティの語は使いません。

**なぜ `issue` は翻訳し、`skill` は翻訳しないのか**: ユーザーは毎日 issue を登録し、読んでいます。そして "issue" という語は、中国語・日本語・韓国語では開発者の隠語以外に意味を持ちません。現地の人がもともとこの対象を指して使っているのは「タスク」に当たる日常語です。`skill` は、その意味を担える現地語が存在しない Lumen 固有の概念です。
...
| カテゴリ | 用語 |
| --- | --- |
| ブランド | **Lumen**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
