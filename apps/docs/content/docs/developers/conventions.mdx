...
If you write Lumen code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@lumen/core`, business logic |
...
| `apps/mobile/` | types and pure functions from `@lumen/core` | Web/Desktop React pages, stores, and platform implementations |
...
### The core distinction: everyday noun vs Lumen-specific term

Lumen's product nouns split into two categories:
...
- **Lumen-specific term** — a concept no local word carries (`skill`), or a schema-level identifier a user may have to type or match (`todo`, `in_progress`, `task_id`). Render as **lowercase English** so it reads like a type name.
...
| --- | --- | --- | --- | --- | --- |
| the tracked unit of work (`issue`) | Issue | **任务** | **タスク** | **태스크** | **Tâche** |
...
- **Code references and literal commands** stay English: `lumen issue ...`, the `/issue` Slack and Lark slash command.
- **`skill`** keeps lowercase English in Chinese text — a Lumen-specific concept with no established Chinese term; titles may capitalize as `Skills`.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. The everyday task word is what people already say for this. `skill` is a Lumen-specific concept where no local word carries the meaning.
...
| Category | Terms |
| --- | --- |
| Brands | **Lumen**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
