...
| `packages/ui` | 无业务依赖 | `@lumen/core`、业务逻辑 |
...
| `apps/mobile/` | `@lumen/core` 的类型和纯函数 | Web/Desktop 的 React 页面、store 和平台实现 |
...
每个 issue 有人类可读的编号，比如 `LUM-123`：工作区 `issue_prefix`（大写字母和数字，通常 3 个字符，最长 10 个）+ 流水号。工作区管理员可以在**设置 → 通用**中修改前缀；修改会让所有现有 issue 重新编号，外部引用——PR 标题、分支名、文档与聊天里的链接——里的旧前缀会失效。
...
### 核心区分：日常词 vs Lumen 专有词

Lumen 的产品名词分两类：

- **日常词** —— 用户会直接说出口的词。**完整翻译成中文**，是不是数据库实体都一样：`issue` → 任务、`workspace` → 工作区、`project` → 项目。
- **Lumen 专有词** —— 中文里没有对应词的概念（`skill`），或用户可能要照着敲、照着比对的 schema 标识符（`todo`、`in_progress`、`task_id`）。**用小写英文**呈现，读起来像类型名。
...
| --- | --- | --- | --- | --- | --- |
| 提交出来的一件事（`issue`） | Issue | **任务** | **タスク** | **태스크** | **Tâche** |
...
- **代码引用和字面命令**保持英文：`lumen issue ...`、Slack 与飞书的 `/issue` 斜杠命令。
- **`skill`** 在中文里仍用小写英文 —— Lumen 特有概念，没有公认中文译法；标题可大写为 `Skills`。
- **表示「问题」的 issue** 不是这个实体：机器卡片上的 `{{count}} issues` 是 `{{count}} 个异常` / `問題 {{count}} 件` / `문제 {{count}}개`，不要用实体那个词。

**为什么 `issue` 要译，而 `skill` 不译**：用户每天都在提任务、看任务，而 "issue" 这个词在中文、日文、韩文里除了开发圈黑话之外没有含义，当地人本来就用「任务」在说这件事。`skill` 则是 Lumen 特有概念，没有哪个中文词能承载它的意思。
...
| 类别 | 词 |
| --- | --- |
| 品牌 | **Lumen**、GitHub、Slack、Google、Anthropic、OpenAI、Claude、Codex、Cursor、Linear、Jira |
