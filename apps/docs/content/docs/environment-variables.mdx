...
description: Common server, storage, integration, and runtime configuration for self-hosting Lumen.
---

import { Callout } from "fumadocs-ui/components/callout";

Lumen reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/lumen?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://lumen.example.com
LUMEN_APP_URL=https://lumen.example.com
LUMEN_PUBLIC_URL=https://api.lumen.example.com
```
...
`JWT_SECRET` is required in production: with `APP_ENV=production` the backend refuses to boot if it is empty or a known placeholder (generate one with `openssl rand -hex 32`). Do not set `LUMEN_DEV_VERIFICATION_CODE`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `lumen` database | PostgreSQL connection string |
...
| `LUMEN_DATABASE_STARTUP_TIMEOUT` | `3m` | Container-level retry budget shared by migration and API startup; set to `0` for one fail-fast attempt per phase |
| `LUMEN_DATABASE_CONNECT_TIMEOUT` | `5s` | Fallback timeout for each startup connection attempt; native pgx `connect_timeout`, `PGCONNECT_TIMEOUT`, or service settings take precedence |
...
| `LUMEN_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
| `LUMEN_RUNTIME_RECONNECT_GRACE` | `3h` | How long an offline runtime may reconnect before its in-flight runs fail; values below `150s` are clamped |
...
| `LUMEN_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `LUMEN_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
| `LUMEN_DAEMON_SERVER_URL` | falls back to `LUMEN_PUBLIC_URL`, then `LUMEN_APP_URL` / `FRONTEND_ORIGIN` | Server URL the API process inserts into `lumen setup self-host`; set it when daemons reach the API at a different URL than the public webhook URL |
...
<Callout type="warning">
`LUMEN_DAEMON_SERVER_URL` is returned by the unauthenticated `/api/config` endpoint and is visible to clients. Treat it as public configuration; never put credentials, tokens, or other secrets in it.
</Callout>
...
| `RESEND_FROM_EMAIL` | `noreply@lumen.ai` | Sender address; must belong to a verified domain |
...
| `LUMEN_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

## Attachment storage

When `S3_BUCKET` is unset, Lumen uses local disk.
...
| `LUMEN_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `LUMEN_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `LUMEN_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Telegram | `LUMEN_TELEGRAM_SECRET_KEY` | Base64-encoded 32-byte Bot token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `LUMEN_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `LUMEN_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `LUMEN_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
| Plugins | `LUMEN_PLUGIN_SECRET_KEY` | Base64-encoded 32-byte key for stored secrets and encrypted surface launch URLs |
| Plugins | `LUMEN_PLUGIN_SURFACE_ORIGIN` | Dedicated cookie-free browser origin routed to this backend; must differ from app/API origins and preserve `Host` |
| Plugins | `LUMEN_PLUGIN_API_URL` | Complete versioned Plugin Public API base URL, such as `https://plugin-api.example.com/v1`; falls back to `LUMEN_PUBLIC_URL` + `/v1` |
| Plugins | `LUMEN_PLUGIN_DIR` | Optional absolute directory used to publish local plugin bundles during development |
...
| Variable | Default | Description |
|---|---|---|
| `LUMEN_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `LUMEN_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `LUMEN_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
| `LUMEN_LLM_MAX_RETRIES` | `2` | Retry ceiling per call; `0` disables retries, `1`–`5` cap it at N |

`LUMEN_LLM_MAX_RETRIES` is the single source for the retry policy. Leave it unset for the default of 2, set `0` to send exactly one request per call, or set 1–5 to cap retries at that many. It is a ceiling, not a quota: only retryable failures consume it, and a success or the caller's own deadline can end the call sooner. Any other value — negative, non-numeric, or above 5 — fails startup instead of being corrected silently. The ceiling is a latency budget: backoff starts at 0.5s and doubles to an 8s cap, so a larger budget would outlast the callers' own deadlines and turn a retryable failure into a timeout. Retries cover connection failures and HTTP 408, 409, 429 and 5xx; all other 4xx responses are returned as-is. The server logs the effective policy at startup as `llm retry policy`, with no credentials in the line.
...
<Callout type="info">
This covers the assist layer only. Running an agent is a separate data path: when an agent answers a chat, your daemon executes that agent's AI coding tool using the tool's own credentials, and does not forward the `LUMEN_LLM_*` settings above to it. (The run-scoped Lumen connection variables the agent needs are injected by the daemon separately.) Emptying the variables above does not affect that path — govern it through the agent's runtime configuration.
</Callout>
...
| Variable | Default | Description |
|---|---|---|
| `LUMEN_SERVER_URL` | `ws://localhost:8080/ws` | Lumen API / WebSocket URL; also accepts `http(s)` |
| `LUMEN_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `LUMEN_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `LUMEN_DAEMON_POLL_INTERVAL` | `30s` | Run polling interval when no wake event arrives |
| `LUMEN_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` | Upper bound for healthy WebSocket claim safety polls, configured independently of `LUMEN_DAEMON_POLL_INTERVAL`; downward jitter makes the normal default `2m30s`–`2m45s`, while old servers and uncertain claims retain the ordinary poll interval |
| `LUMEN_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `LUMEN_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent run ceiling per daemon |
| `LUMEN_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `LUMEN_AGENT_IDLE_WATCHDOG` | `2h` | Silence ceiling with no output and no tool execution; `0` disables the watchdog entirely |
| `LUMEN_AGENT_TOOL_WATCHDOG` | same as `LUMEN_AGENT_IDLE_WATCHDOG` | Silence ceiling for a single tool call; set it only to give tools more room than the model gets, `0` never force-stops a tool call. Cursor background shells stay in flight until their owned processes exit; at this boundary the daemon stops those processes and gives Cursor a fresh watchdog budget to produce its terminal result. If process ownership or cleanup cannot be verified, the ordinary run-cancellation policy applies |
| `LUMEN_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | same as `LUMEN_AGENT_IDLE_WATCHDOG` | Codex semantic-silence threshold. Codex's own timer cannot see that a tool is running, so it tracks the larger of the idle / tool budgets instead of holding a separate, shorter ceiling |
| `LUMEN_CODEX_FIRST_TURN_TIMEOUT` | `0` | Explicit override for the Codex first-turn no-progress ceiling; `0` keeps the default. The effective first-turn wait stays bounded by `LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` and the overall execution timeout — set `LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` strictly above this value (with some margin), or the wait is truncated to it and the model-catalog startup retry is skipped. Equal values are not enough: the semantic timer is armed first, so at equal durations the retry can still be lost |
| `LUMEN_CODEX_HANDSHAKE_TIMEOUT` | `30s`; `thread/start`, `thread/resume`: `60s` | Codex app-server startup handshake ceilings. An explicit value overrides both budgets globally |
| `LUMEN_CODEX_TURN_INTERRUPT_TIMEOUT` | `2s` | Grace period after cancellation for Codex app-server to acknowledge `turn/interrupt` and emit `turn/completed`, preserving final token usage before forced process cleanup. On unusually slow hosts, tune this from the interrupt latency recorded in daemon logs |
| `LUMEN_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `LUMEN_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `LUMEN_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `lumen` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `LUMEN_DAEMON_AUTO_UPDATE` |
| `LUMEN_WORKSPACES_ROOT` | `~/lumen_workspaces` | Root directory for run working directories |
| `LUMEN_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS only. Parent directory for private per-run temp dirs; must be an existing, writable absolute directory, and an invalid value fails run startup instead of falling back to `/tmp`. Pick a short path — child tools may bind AF_UNIX sockets under it, and `sun_path` is limited to 108 bytes on Linux and 104 on macOS |
| `LUMEN_KEEP_ENV_AFTER_TASK` | `false` | Keep run directories for debugging |
...
Each AI coding tool accepts `LUMEN_<PROVIDER>_PATH` and `LUMEN_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw and MiniMax Code have no model variable because Lumen never sends them a model; MiniMax Code's path variable is `LUMEN_MCODE_PATH` — see [AI coding tools comparison](/providers#model-sources). DeepSeek Harness supports `LUMEN_DSH_PATH` and `LUMEN_DSH_MODEL` (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`). DeepSeek Harness additionally accepts `LUMEN_DSH_PROFILE_BUNDLE`: a comma-separated list of bundles to install into its `lumen` profile when that profile is missing, tried in order, where each entry is an npm spec, a directory, or a packed tarball. It is unset by default, and the `--stdio` protocol Lumen drives is what that profile supplies, so an unset variable leaves the DSH installation untouched and the daemon reports the missing profile instead of registering an unusable runtime. Lumen's own bridge is not on a public npm registry yet, so choosing a value is covered in [Install an agent runtime](/install-agent-runtime); whatever you set is installed into every daemon host's DSH home without further confirmation, so treat it as a supply-chain decision. The install `LUMEN_DSH_PROFILE_BUNDLE` enables is bounded on purpose: it runs at most once per daemon lifetime, only when the probe reports the profile genuinely absent, and a failure is logged with the package manager's own output and retried only after a daemon restart. A probe that times out, or answers with a protocol version this daemon does not drive, is reported and never installed over — the daemon does not overwrite a profile that is deliberately there. To diagnose, search the daemon log for `DSH runtime profile`; to undo an install, delete `$DSH_HOME/profiles/lumen` (default `~/.dsh/profiles/lumen`). `LUMEN_DSH_PLUGIN_PATH` overrides the directory the install resolves pnpm from. DSH Desktop keeps the pnpm that `dsh plugin` forwards to inside its own runtime-commands directory on both macOS and Windows, and the daemon finds and prepends it automatically — including the versioned layout a newer Desktop uses — so this variable is only needed for an installation the daemon cannot find, or to make the install use a pnpm of your own. Setting it replaces the search entirely: if the directory does not exist, the install falls back to whatever pnpm is on the daemon's `PATH` rather than to the bundled one. Linux has no DSH Desktop, so there the install always uses `PATH`. Dim supports `LUMEN_DIM_PATH` and `LUMEN_DIM_MODEL` (a model id from the dim catalog). ZeroClaw supports `LUMEN_ZEROCLAW_PATH` but has no model variable; its agent profile owns the model selection. Lumos supports `LUMEN_LUMOS_PATH` but has no model variable for the same reason — Lumos's `do_set_config_option` accepts a `model` value but never persists it (crates/lumos-acp/src/acp/agent.rs), so a daemon-readable env var would advertise a knob the picker silently ignores. Machine-wide default arguments via `LUMEN_<PROVIDER>_ARGS` are currently supported for five tools: Claude Code, Codex, CodeBuddy, Qwen Code, and QwenPaw. The variables are `LUMEN_CLAUDE_ARGS`, `LUMEN_CODEX_ARGS`, `LUMEN_CODEBUDDY_ARGS`, `LUMEN_QWEN_ARGS`, and `LUMEN_QWENPAW_ARGS`. For example:

```dotenv
LUMEN_CLAUDE_PATH=/opt/bin/claude
LUMEN_CLAUDE_ARGS=--max-turns 40
```

Cursor's verified background cleanup requires Linux 6.9 or newer (process-group pidfds), macOS kernel support for process identity queries and audit-token signalling, or a successfully assigned nested Job Object on Windows. On macOS, each process is signalled using a kernel-checked PID version; no same-session group join is required. These macOS APIs are private and checked at capture time. A launch that cannot be claimed returns its original tool result and uses the ordinary idle watchdog, including when the tool watchdog is `0`. Already-claimed work remains in flight if cleanup cannot be confirmed. With the tool watchdog set to `0` there is no such boundary at all: claimed background work keeps the run in flight for as long as it lives, so the run is bounded only by `LUMEN_AGENT_TIMEOUT`, which is `0` by default.
...
Precedence is command-line flag → environment variable → `~/.lumen/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.lumen/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.lumen/profiles/<name>/config.json`:

```bash
lumen config set poll_interval 10s
lumen config show
```
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Lumen API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `lumen config set poll_interval ""`.
