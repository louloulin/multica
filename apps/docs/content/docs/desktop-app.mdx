...
Desktop connects to Multica Cloud by default. On first launch with no `~/.multica/desktop.json`, a **Welcome** screen asks you to choose Cloud or a self-hosted API URL before login. After that, change the choice any time from **Settings → Backend**.

Open **Settings → Backend** in the app and paste your API URL — Desktop saves it to `~/.multica/desktop.json` and asks you to restart. Web and WebSocket URLs are derived automatically; override only when your deployment doesn't match the `api.<host>` convention.
