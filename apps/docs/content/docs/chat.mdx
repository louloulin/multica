...
A chat is **not bound to any issue**: the agent doesn't automatically receive an issue's description and comments, but it can still use the Lumen CLI to query workspaces, projects, issues, and skills within its permissions.
...
4. After you send, Lumen creates a run for the message, and the reply comes back to the current conversation.
...
Direct chats created in Lumen are visible only to their creator; other workspace members and admins cannot read them. Conclusions the team needs to see together should be written into an issue, a project description, or a skill.

## Multi-turn context

A conversation tries to continue the original session in the AI coding tool, so later messages don't need to repeat earlier content. Lumen stores the session ID and routes subsequent runs to a runtime that can access that session.
