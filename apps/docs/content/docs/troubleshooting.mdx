...
First work out which layer the problem lives in: the Lumen service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:

```bash
lumen version
lumen auth status
lumen daemon status --output json
lumen daemon logs --lines 100
```
...
- The daemon is connected to the wrong Lumen service.
...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `lumen setup self-host`, or check the current profile's `server_url`:
...
If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `lumen daemon restart`.

### `waiting_local_directory` status

This means another in-flight run is using the same local directory. Lumen waits for the directory lock to be released so two agents never modify the same files at once.
...
## `lumen setup` reports the server is not reachable
...
When `SMTP_HOST` and Resend are both configured, Lumen prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-verification-codes).
...
If it is another Lumen checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.

## Log locations

| Component | How to view |
| --- | --- |
| Background daemon | `lumen daemon logs --lines 100` |
| Follow daemon logs live | `lumen daemon logs --follow` |
| Default profile's log file | `~/.lumen/daemon.log` |
| Default profile's startup or crash log | `~/.lumen/daemon.err.log` |
| Named profiles | the matching logs under `~/.lumen/profiles/<name>/` |
...
If you still cannot pin it down, search existing issues or file a new one on [GitHub Issues](https://github.com/lumen-ai/lumen/issues).
