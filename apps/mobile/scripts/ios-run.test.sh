...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/lumen-ios-run.XXXXXX")
...
mkdir -p "$BIN_DIR"
export LUMEN_TEST_PNPM_CALLS="$CALLS_FILE"
...
printf '%s\n' "$*" >>"$LUMEN_TEST_PNPM_CALLS"

if [ -n "${LUMEN_TEST_FAIL_PREBUILD:-}" ]; then
  case "$*" in
    *prebuild*)
...
: >"$CALLS_FILE"
set +e
LUMEN_TEST_FAIL_PREBUILD=1 "$SCRIPT_DIR/ios-run.sh" >/dev/null 2>&1
status=$?
set -e
