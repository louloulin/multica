...
- Native examples: `Alert.prompt` for text prompts, `Alert.alert` for confirmation, the shared action-sheet wrapper (`lib/action-sheet.ts`) for action menus, existing native date/media/document pickers, `Share.share`, and `expo-haptics`. Never import `ActionSheetIOS` at a feature call site: iOS keeps the native sheet, Android gets the repo fallback.
...
Mobile ships iOS + Android from one codebase; iOS remains the visual reference client. Match product semantics and adapt only where Android requires it.

- `formSheet` routes give native iOS sheet behavior; Android falls back to a regular modal presentation. Keep route bodies self-contained, verify keyboard/search layouts, and do not promise iOS grabber/detent behavior on Android.
- `sf:` (SF Symbols) icons are iOS-only. Every SF Symbol usage needs an Android fallback (currently Ionicons where applicable).
- Route stacks handle hardware Back automatically, but every non-route overlay needs explicit verification that Back closes the topmost layer instead of exiting the app.
- Android runs edge-to-edge: verify safe-area/header/tab-bar spacing under both gesture and 3-button navigation, and keep status/navigation bar colors aligned with the active theme.

## Build and Release

- `.github/workflows/mobile-verify.yml` runs typecheck/lint/test plus an Android `assembleDebug`. Keep mobile work out of `.github/workflows/ci.yml`; mobile failures do not block web/desktop.
- Local iOS builds route through `scripts/ios-run.sh` (prebuild then `expo run:ios`); local Android builds use `expo run:android` after `expo prebuild --platform android`.
- Android release APKs are produced by the tag-triggered `.github/workflows/mobile-release.yml` or locally via `expo run:android --variant release`. Distribution is local/internal APK install — there is no Play Store, AAB, EAS Build/Submit, or OTA Update wiring here; treat cloud release automation as new work.
- Mobile release cadence is decoupled from the main `v*.*.*` tags (server/CLI/desktop).
