...
This guide documents the local development workflow for contributors working on the Lumen codebase.
...
By submitting a contribution to Lumen — a pull request, a patch, or any
other work — you agree to condition 2 of the [Lumen License](LICENSE):

- your contribution is submitted under the Lumen License as a whole (the
...
- the producer can adjust the Lumen License to be more strict or relaxed
  as deemed necessary.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=lumen`
...
```bash
POSTGRES_DB=lumen_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://lumen:lumen@localhost:5432/lumen_my_feature_702?sslmode=disable
```
...
- **API, Web and Desktop renderer ports, database names and profiles are allocated, not recomputed.** The
  ...
  listener says the slot is taken. The registry lives in `~/.lumen/dev/`;
...
```bash
git worktree add ../lumen-feature -b feat/my-change main
cd ../lumen-feature
make dev
```
...
`lumen repo checkout` uses the user's system/global Git identity, including
...
name/email settings with private defaults in `lumen-identity.config`, included
...
```bash
make remove-worktree WORKTREE=../lumen-feature
```
...
The daemon authenticates using the CLI's stored token (`lumen login`).
...
token, creates a workspace, writes the CLI profile, builds `server/bin/lumen`
...
- **The daemon runs from a built binary, never `go run`.** The daemon records
  ...
  every task with `fork/exec …/go-build…/exe/lumen: no such file or directory`.
- **`daemon start` is refused under a daemon-managed task.** A checkout below a
  `.lumen/daemon_task_context.json` marker cannot start a second daemon
...
Nothing in this flow touches the system-installed `lumen` or the default
`~/.lumen/config.json`:

| Resource | System / Production | Local Dev (per environment) |
|---|---|---|
| Config | `~/.lumen/config.json` | `~/.lumen/profiles/dev-<slug>-<offset>/config.json` |
| Daemon PID | `~/.lumen/daemon.pid` | `~/.lumen/profiles/dev-<slug>-<offset>/daemon.pid` |
| Workspaces dir | `~/lumen_workspaces/` | `~/lumen_workspaces_dev-<slug>-<offset>/` |
| Database | remote / production | local: `lumen_<slug>_<offset>` |
| Registry | — | `~/.lumen/dev/envs/<name>/` |
| Desktop profile | `desktop-api.lumen.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U lumen -d postgres -At -c "select datname from pg_database order by datname;"
```
...
database `lumen` unless `ALLOW_MAIN_DB_DROP=1` is explicitly supplied.
