...
Deploy Lumen on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`lumen` CLI** and runs the **agent daemon** on their own machine.

## Anonymous Deployment Telemetry

The self-hosted API server sends one first-party, deployment-level anonymous snapshot per UTC day to the fixed Lumen endpoint `https://telemetry.lumen.ai/v1/telemetry/events`. This helps maintainers understand which release versions are in use, deployment-size ranges, and aggregate run volume. The endpoint is compiled into the server and cannot be redirected through configuration.
...
```powershell
# 1. Install CLI + provision the self-host server
$env:LUMEN_MODE="with-server"; irm https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.ps1 | iex
...
This installs the `lumen` CLI, checks out the latest self-host assets, pulls the official Lumen images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `lumen-backend:dev` / `lumen-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `LUMEN_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `LUMEN_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
- [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh` on PATH with the Lumen runtime profile installed; set `DEEPSEEK_API_KEY`)
...
Lumen Desktop works against a self-hosted backend out of the box — open **Settings → Backend** in the app and paste your API URL. The app writes `~/.lumen/desktop.json`, derives the web and WebSocket URLs automatically, and prompts you to restart. Full reference: [`SELF_HOSTING_DESKTOP.md`](SELF_HOSTING_DESKTOP.md).

---

## Kubernetes Deployment (Alternative)

If you already run a Kubernetes cluster, you can deploy Lumen there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/lumen-ai/charts/lumen` or the source chart at [`deploy/helm/lumen/`](deploy/helm/lumen/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `lumen-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `lumen-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `lumen-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `lumen-config` ConfigMap (rendered from `values.yaml`)

The `lumen-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **Runtime frontend upstreams:** current `lumen-web` images read `REMOTE_API_URL` and `DOCS_URL` when the Next.js server runs, so API/docs upstream changes do not require a web rebuild. The chart defaults `REMOTE_API_URL` to this release's backend Service. `frontend.compatibility.backendAlias` exists only for legacy images that still baked `REMOTE_API_URL=http://backend:8080` at build time.
...
The chart defaults to `lumen.dev.lan` (web) and `api.lumen.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  lumen.dev.lan api.lumen.dev.lan
  ```
...
### Step 3 — Create the `lumen-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n lumen create secret generic lumen-secrets \
  --from-literal=JWT_SECRET="$(openssl rand -hex 32)" \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -hex 16)" \
  --from-literal=RESEND_API_KEY="" \
  --from-literal=GOOGLE_CLIENT_SECRET="" \
  --from-literal=CLOUDFRONT_PRIVATE_KEY="" \
  --from-literal=LUMEN_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install lumen oci://ghcr.io/lumen-ai/charts/lumen \
...
```bash
helm show values oci://ghcr.io/lumen-ai/charts/lumen \
...
```bash
helm install lumen deploy/helm/lumen -n lumen
```
...
```bash
curl -H "Host: api.lumen.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://lumen.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n lumen patch secret lumen-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n lumen rollout restart deploy/lumen-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n lumen logs -f deploy/lumen-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `LUMEN_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade lumen oci://ghcr.io/lumen-ai/charts/lumen \
    --version <chart-version> \
    -n lumen \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n lumen patch secret lumen-secrets --type=merge \
    -p '{"stringData":{"LUMEN_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n lumen rollout restart deploy/lumen-backend
  ```
...
```bash
lumen setup self-host \
  --server-url http://api.lumen.dev.lan \
  --app-url http://lumen.dev.lan
```
...
```bash
kubectl -n lumen rollout restart deploy/lumen-backend deploy/lumen-frontend
```

To upgrade to a specific Lumen release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
helm -n lumen rollback lumen
```

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** As of MUL-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically before applying migration `103`, so a clean upgrade is a single `helm upgrade` + backend rollout. If you are still on a pre-MUL-2957 binary or the auto-hook fails, run the standalone backfill against the same database the chart is using (`kubectl -n lumen exec deploy/lumen-backend -- ./backfill_task_usage_hourly --sleep-between-slices=2s`), then restart the backend deployment to re-apply migrations. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the full recovery flow.
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Lumen's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
```bash
curl -fsSL https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Lumen Cloud

If you've been self-hosting and want to switch your CLI to [Lumen Cloud](https://lumen.ai):

```bash
lumen setup
```

This reconfigures the CLI for lumen.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `LUMEN_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `lumen setup`:
