...
$runnerScript = Join-Path ([System.IO.Path]::GetTempPath()) "lumen-install-ps1-case.ps1"
...
if (-not $env:USERPROFILE) { $env:USERPROFILE = [System.IO.Path]::GetTempPath() }
$env:LUMEN_INSTALL_DIR = $WorkDir
$env:LUMEN_SELFHOST_REF = "main"
...
foreach ($case in $cases) {
    $workDir = Join-Path ([System.IO.Path]::GetTempPath()) ("lumen-ps1-" + [guid]::NewGuid().ToString("N"))
