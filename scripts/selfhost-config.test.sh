...
printf '\nBACKEND_PORT=9100\nSMTP_FROM_EMAIL=lumen@example.com\n' >>"$tmp_env"
printf 'LUMEN_LLM_API_KEY=llm-key-from-env\nLUMEN_LLM_BASE_URL=http://gateway.example/v1\nLUMEN_LLM_DEFAULT_MODEL=model-from-env\nLUMEN_LLM_MAX_RETRIES=3\n' >>"$tmp_env"
printf 'DATABASE_REPLICA_URL=postgres://reader:secret@replica.example.com:5432/lumen?sslmode=require\nDATABASE_REPLICA_MAX_CONNS=12\nDATABASE_REPLICA_MIN_CONNS=1\n' >>"$tmp_env"
...
require_config "$config" 'LUMEN_APP_URL: http://localhost:3100'
require_config "$config" 'SMTP_FROM_EMAIL: lumen@example.com'
require_config "$config" 'LUMEN_DATABASE_STARTUP_TIMEOUT: 3m'
require_config "$config" 'LUMEN_DATABASE_CONNECT_TIMEOUT: 5s'
...
require_config "$config" 'DATABASE_REPLICA_URL: postgres://reader:secret@replica.example.com:5432/lumen?sslmode=require'
...
require_config "$config" 'LUMEN_LLM_API_KEY: llm-key-from-env'
require_config "$config" 'LUMEN_LLM_BASE_URL: http://gateway.example/v1'
require_config "$config" 'LUMEN_LLM_DEFAULT_MODEL: model-from-env'
require_config "$config" 'LUMEN_LLM_MAX_RETRIES: "3"'
...
done < <(grep -oE '^LUMEN_LLM_[A-Z_]+' .env.example)
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "LUMEN_APP_URL=${LUMEN_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "LUMEN_SERVER_URL=${LUMEN_SERVER_URL}" \
...
require_env "$local_env" 'LUMEN_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'LUMEN_SERVER_URL=ws://localhost:9100/ws'
...
require_env "$(cat "$worktree_env")" "LUMEN_PUBLIC_URL=http://localhost:${worktree_backend_port}"
...
grep -v '^LUMEN_PUBLIC_URL=' "$worktree_env" >"$old_worktree_env"
require_env \
...
printf '\nLUMEN_PUBLIC_URL=https://api.explicit.example\n' >>"$explicit_worktree_env"
require_env \
