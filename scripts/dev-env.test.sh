...
#
# Everything here runs against a throwaway LUMEN_DEV_HOME holding hand-written
...
export LUMEN_DEV_HOME="$tmp_dir/dev"
export LUMEN_DEV_WORKSPACES_PARENT="$tmp_dir/workspaces-parent"
export LUMEN_DEV_DESKTOP_APP_DATA="$tmp_dir/app-data"
export LUMEN_DEV_PROFILES_HOME="$tmp_dir/profiles"
...
write_manifest() {
  ...
  mkdir -p "$LUMEN_DEV_HOME/envs/$name/logs"
  cat > "$LUMEN_DEV_HOME/envs/$name/manifest.env" <<EOF
NAME=$name
DIR=$(printf '%q' "$dir")
...
DB_NAME=lumen_dev_env_test_$offset
DATABASE_URL=postgres://lumen:lumen@localhost:5432/lumen_dev_env_test_$offset?sslmode=disable
PROFILE=$profile
WORKSPACES_ROOT=$(printf '%q' "$LUMEN_DEV_WORKSPACES_PARENT/lumen_workspaces_$profile")
...
LUMEN_TASK_CONFIG_ROOT=/task/config \
LUMEN_TASK_WORKSPACES_ROOT=/task/workspaces \
LUMEN_WORKSPACES_ROOT=/owner/workspaces \
  dev_env exec clean-env-903 -- sh -c '
    test -z "${LUMEN_TASK_CONFIG_ROOT:-}" &&
    test -z "${LUMEN_TASK_WORKSPACES_ROOT:-}" &&
    test "$LUMEN_WORKSPACES_ROOT" = "$1"
  ' _ "$LUMEN_DEV_WORKSPACES_PARENT/lumen_workspaces_dev-dev-env-test-903" \
...
[ -f "$LUMEN_DEV_HOME/envs/orphan-902/manifest.env" ] || fail "gc --dry-run deleted a manifest"
...
[ -f "$LUMEN_DEV_HOME/envs/drop-fails-904/manifest.env" ] \
...
[ ! -d "$LUMEN_DEV_HOME/envs/probe-901" ] || fail "destroy left the environment directory behind"
...
[ -d "$LUMEN_DEV_HOME/envs/orphan-902" ] || fail "declined destroy removed the environment anyway"
