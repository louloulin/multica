# Lumen installer for Windows - one command to get started.
#
# Install CLI (default): connects to lumen.ai
#   irm https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Lumen server + installs CLI + configures
#   $env:LUMEN_MODE="local"; irm https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/lumen-ai/lumen.git"
$RepoWebUrl    = "https://github.com/lumen-ai/lumen"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".lumen\server"
$InstallDir    = if ($env:LUMEN_INSTALL_DIR) { $env:LUMEN_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/lumen-ai/lumen/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:LUMEN_SELFHOST_REF) {
        return $env:LUMEN_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = lumen version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Lumen CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Lumen requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/lumen-ai/lumen/releases/download/$latest/lumen-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "lumen-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "lumen.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/lumen-ai/lumen/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "lumen.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "lumen-cli-$version-windows-$arch.zip"
        $legacyAsset = "lumen_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "lumen.zip") -DestinationPath $tmpDir -Force
    ...
    $exeSrc = Join-Path $tmpDir "lumen.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "lumen.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "lumen.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "lumen.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Lumen CLI installed to $binDir\lumen.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "lumen") {
        ...
        if ($isUpToDate) {
            Write-Ok "Lumen CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Lumen CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Lumen CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "lumen")) {
        Write-Fail "CLI installed but 'lumen' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Lumen self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:LUMEN_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Lumen server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Lumen repository..."
    ...
    Write-Info "Pulling official Lumen images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Lumen services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Lumen server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Lumen - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Lumen CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     lumen setup               " -NoNewline; Write-Host "# Connect to Lumen Cloud (lumen.ai)" -ForegroundColor DarkGray
    Write-Host "     lumen setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:LUMEN_MODE="with-server"; irm https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Lumen - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Lumen server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     lumen setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:LUMEN_MODE="stop"; irm https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Lumen services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Lumen installation found at $InstallDir"
    }

    if (Test-CommandExists "lumen") {
        try {
            lumen daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:LUMEN_MODE) { $env:LUMEN_MODE.ToLower() } else { "default" }
