...
DEV_HOME="${LUMEN_DEV_HOME:-$HOME/.lumen/dev}"
ENVS_DIR="$DEV_HOME/envs"
LOCK_DIR="$DEV_HOME/lock.d"
DEV_WORKSPACES_PARENT="${LUMEN_DEV_WORKSPACES_PARENT:-$HOME}"
DEV_DESKTOP_APP_DATA="${LUMEN_DEV_DESKTOP_APP_DATA:-}"
DEV_PROFILES_HOME="${LUMEN_DEV_PROFILES_HOME:-$HOME/.lumen/profiles}"

DEV_EMAIL="${LUMEN_DEV_EMAIL:-dev@localhost}"
DEV_CODE_DEFAULT=888888
WORKSPACE_NAME="${LUMEN_DEV_WORKSPACE_NAME:-Dev}"
WORKSPACE_SLUG="${LUMEN_DEV_WORKSPACE_SLUG:-dev}"

ALL_COMPONENTS="api web daemon desktop"
DEFAULT_COMPONENTS="api web"

# An agent runs with TMPDIR=/tmp/lumen-task-<id>, deleted when the run ends.
...
DEV_TMPDIR="${LUMEN_DEV_TMPDIR:-$HOME/.lumen/dev-tmp}"

# The agent runtime exports these pointing at PRODUCTION, and LUMEN_SERVER_URL
...
CLEAN_ENV=(env
  -u LUMEN_SERVER_URL -u LUMEN_TOKEN -u LUMEN_WORKSPACE_ID
  -u LUMEN_DAEMON_PORT -u LUMEN_AGENT_ID -u LUMEN_AGENT_NAME
  -u LUMEN_TASK_ID -u LUMEN_TASK_SLOT
  -u LUMEN_TASK_CONFIG_ROOT -u LUMEN_TASK_WORKSPACES_ROOT
  -u LUMEN_WORKSPACES_ROOT)
...
desktop_user_data_dir() {
  printf '%s/Lumen Canary %s' "$(desktop_app_data_root)" "$1"
}
...
ensure_dev_code() {
  ...
  if grep -qE '^LUMEN_DEV_VERIFICATION_CODE=[0-9]{6}$' "$file"; then
    return 0
  fi
  if grep -q '^LUMEN_DEV_VERIFICATION_CODE=' "$file"; then
    tmp="$(mktemp)"
    sed "s/^LUMEN_DEV_VERIFICATION_CODE=.*/LUMEN_DEV_VERIFICATION_CODE=$DEV_CODE_DEFAULT/" "$file" > "$tmp"
    mv "$tmp" "$file"
  else
    printf '\nLUMEN_DEV_VERIFICATION_CODE=%s\n' "$DEV_CODE_DEFAULT" >> "$file"
  fi
  info "Set LUMEN_DEV_VERIFICATION_CODE=$DEV_CODE_DEFAULT in $1 (ignored when APP_ENV=production)."
}
...
rewrite_env_ports() {
  ...
  sed \
    ...
    -e "s|^LUMEN_SERVER_URL=.*|LUMEN_SERVER_URL=ws://localhost:${backend}/ws|" \
    -e "s|^LUMEN_PUBLIC_URL=.*|LUMEN_PUBLIC_URL=http://localhost:${backend}|" \
    -e "s|^LUMEN_APP_URL=.*|LUMEN_APP_URL=http://localhost:${frontend}|" \
...
ensure_credentials() {
  ...
  local code="${LUMEN_DEV_VERIFICATION_CODE:-$DEV_CODE_DEFAULT}"
  ...
  curl -sf -X POST "$server/auth/send-code" -H 'Content-Type: application/json' \
    ...
    || die "send-code failed. Is LUMEN_DEV_VERIFICATION_CODE set and APP_ENV non-production?"
...
daemon_task_marker() {
  ...
  while :; do
    marker="$dir/.lumen/daemon_task_context.json"
    if [ -f "$marker" ] && grep -q 'lumen-daemon-task' "$marker" 2>/dev/null; then
      printf '%s' "$marker"
      return 0
    fi
    [ "$dir" != "/" ] || break
...
start_daemon() {
  ...
  # "fork/exec .../go-build.../exe/lumen: no such file or directory".
  info "Building $LUMEN_BIN (a go run daemon would fail every task later)."
  (cd "$REPO_ROOT/server" && go build -o bin/lumen ./cmd/lumen) || die "Failed to build the lumen CLI."

  "${CLEAN_ENV[@]}" LUMEN_WORKSPACES_ROOT="$WORKSPACES_ROOT" \
    "$LUMEN_BIN" daemon start --profile "$PROFILE" 2>&1 | sed 's/^/    /' || true

  status="$("${CLEAN_ENV[@]}" LUMEN_WORKSPACES_ROOT="$WORKSPACES_ROOT" \
    "$LUMEN_BIN" daemon status --profile "$PROFILE" --output json 2>/dev/null || true)"
...
stop_component() {
  ...
  case "$name" in
    daemon)
      if [ -x "$LUMEN_BIN" ]; then
        if "${CLEAN_ENV[@]}" LUMEN_WORKSPACES_ROOT="$WORKSPACES_ROOT" \
          "$LUMEN_BIN" daemon stop --profile "$PROFILE" >/dev/null 2>&1; then
          ok "daemon stopped"
        else
          status="$("${CLEAN_ENV[@]}" LUMEN_WORKSPACES_ROOT="$WORKSPACES_ROOT" \
            "$LUMEN_BIN" daemon status --profile "$PROFILE" --output json 2>/dev/null || true)"
        ...
        if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
          warn "cannot stop daemon pid $pid because no usable lumen binary was found"
          return 1
        fi
...
${C_GREEN}✓ Environment ready.${C_OFF}
  ...
  ${entrypoint}
  Sign in     ${DEV_EMAIL}  ·  code ${LUMEN_DEV_VERIFICATION_CODE:-$DEV_CODE_DEFAULT}
...
bind_paths() {
  ...
  WORKSPACES_ROOT="${WORKSPACES_ROOT:-$DEV_WORKSPACES_PARENT/lumen_workspaces_$PROFILE}"
  ...
  LUMEN_BIN="$DIR/server/bin/lumen"
  if [ ! -x "$LUMEN_BIN" ] && [ -x "$REPO_ROOT/server/bin/lumen" ]; then
    LUMEN_BIN="$REPO_ROOT/server/bin/lumen"
  fi
  mkdir -p "$LOG_DIR"
}
...
Start the rest with 'make up C=api,web', or run 'make up C=daemon' from your own shell."
  ...
  if [ -n "$existing" ]; then
    ...
    if [ "$PORT" -lt 18080 ] || [ "$FRONTEND_PORT" -ne $((13000 + offset)) ] \
      ...
      local new_db="lumen_$(slugify "$(basename "$REPO_ROOT")")_${offset}"
    ...
    WORKSPACES_ROOT="$DEV_WORKSPACES_PARENT/lumen_workspaces_$PROFILE"
...
cmd_destroy() {
  ...
  expected_workspaces="$DEV_WORKSPACES_PARENT/lumen_workspaces_$PROFILE"
...
# production LUMEN_* values and with a durable TMPDIR. Replaces the prefix
...
cmd_exec() {
  ...
  export PORT="$BACKEND_PORT" FRONTEND_PORT DATABASE_URL POSTGRES_DB="$DB_NAME"
  export TMPDIR="$DEV_TMPDIR" TMP="$DEV_TMPDIR" TEMP="$DEV_TMPDIR"
  export LUMEN_DEV_PROFILE="$PROFILE"
  exec "${CLEAN_ENV[@]}" LUMEN_WORKSPACES_ROOT="$WORKSPACES_ROOT" "$@"
}
