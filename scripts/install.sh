#!/usr/bin/env bash
# Lumen installer — installs the CLI and optionally provisions a self-host server.
#
# Install / upgrade CLI only:
#   curl -fsSL https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.sh | bash
#
# Install CLI + provision self-host server:
#   curl -fsSL https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.sh | bash -s -- --with-server
#
# After installation, run `lumen setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/lumen-ai/lumen.git"
REPO_WEB_URL="https://github.com/lumen-ai/lumen"  # without .git, for GitHub web APIs
INSTALL_DIR="${LUMEN_INSTALL_DIR:-$HOME/.lumen/server}"
BREW_PACKAGE="lumen-ai/tap/lumen"
...
print_remote_server_token_hint() {
  ...
  printf "     1. On your local computer, open ${CYAN}https://lumen.ai/settings?tab=tokens${RESET}\n"
  printf "        and create a token under ${BOLD}Settings > API Tokens${RESET}.\n"
  printf "     2. On this server, run:\n"
  printf "        ${CYAN}lumen login --token <YOUR_TOKEN>${RESET}\n"
  printf "        ${CYAN}lumen daemon start${RESET}\n"
  printf "\n"
}
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Lumen supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing Lumen CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap lumen-ai/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Lumen CLI already installed via Homebrew"
    else
      warn "Failed to install lumen via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Lumen CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Lumen CLI from GitHub Releases..."
  ...
  local url="https://github.com/lumen-ai/lumen/releases/download/${latest}/lumen-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/lumen.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/lumen.tar.gz" -C "$tmp_dir" lumen

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with LUMEN_BIN_DIR.
  local bin_dir="${LUMEN_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/lumen" "$bin_dir/lumen"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/lumen" "$bin_dir/lumen"
  else
  ...
  rm -rf "$tmp_dir"
  ok "Lumen CLI installed to $bin_dir/lumen"
}
...
get_selfhost_ref() {
  if [ -n "${LUMEN_SELFHOST_REF:-}" ]; then
    printf '%s' "$LUMEN_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Lumen CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Lumen CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Lumen CLI is already the latest version"
  fi
}
...
install_cli() {
  if command_exists lumen; then
    local current_ver
    # `lumen version` outputs "lumen 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)" — extract just the version
    current_ver=$(lumen version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Lumen CLI is up to date ($current_ver)"
      return 0
    fi

    info "Lumen CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(lumen version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "Lumen CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists lumen; then
    fail "CLI installed but 'lumen' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Lumen self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  info "Pulling official Lumen images..."
  pull_official_selfhost_images
  info "Starting Lumen services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Lumen — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Lumen CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}lumen setup${RESET}                # Connect to Lumen Cloud (lumen.ai)\n"
  printf "     ${CYAN}lumen setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  print_remote_server_token_hint
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Lumen — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Lumen server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}lumen setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/lumen-ai/lumen/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Lumen installation found at $INSTALL_DIR"
  fi

  if command_exists lumen; then
    lumen daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Lumen CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  LUMEN_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.lumen/server)"
        echo "  LUMEN_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  LUMEN_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'lumen setup' to configure your environment."
        exit 0
        ;;
