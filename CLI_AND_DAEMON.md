...
The `lumen` CLI connects your local machine to Lumen. It handles authentication, workspace management, issue tracking, and runs the agent daemon that executes AI tasks locally.
...
```bash
git clone https://github.com/lumen-ai/lumen.git
cd lumen
make build
cp server/bin/lumen /usr/local/bin/lumen
```
...
`lumen update` uses GitHub Releases by default. Self-hosted installations can
...
```bash
export LUMEN_RELEASE_API_BASE_URL=https://updates.example/api
export LUMEN_RELEASE_DOWNLOAD_BASE_URL=https://updates.example/releases/download
export LUMEN_DAEMON_AUTO_UPDATE=true
lumen update
```

The metadata mirror must serve `/repos/lumen-ai/lumen/releases/latest` and
`/repos/lumen-ai/lumen/releases/tags/<tag>`. The artifact mirror must serve
...
`lumen login` automatically discovers all workspaces you belong to and adds them to the daemon watch list.
...
The daemon is the local agent runtime. It detects available AI CLIs on your machine, registers them with the Lumen server, and executes tasks when agents are assigned work.
...
directory of the profile it was started with — **not always `~/.lumen/`**:

| Profile | State directory |
| --- | --- |
| Default (no `--profile`) | `~/.lumen/` |
| Named (`--profile <name>`) | `~/.lumen/profiles/<name>/` |
...
both, `~/.lumen/daemon.log` and `~/.lumen/profiles/<name>/daemon.log` both
exist and both read as plausible logs — only one is being written to. Don't
guess: `lumen daemon logs` prints the absolute path it resolved (see
[Logs](#logs)).

To run in the foreground (useful for debugging):

```bash
lumen daemon start --foreground
```

#### Following a replaced binary

A CLI-launched daemon periodically compares its own compile-time version against
the `--version` output of the `lumen` binary it would re-exec. When they differ
— `brew upgrade lumen`, a re-download, a local `make build` — it waits for any
...
check, and `lumen daemon status` shows why it's still on the old version.
...
```bash
LUMEN_DAEMON_AUTO_RELOAD=0 lumen daemon start
# or
lumen daemon start --no-auto-reload
# or persist it
lumen config set disable_auto_reload true
```
...
**without restarting**, so subsequent tasks pick up the new CLI while Lumen's
...
```bash
lumen daemon logs              # Last 50 lines
lumen daemon logs -f           # Follow (tail -f)
lumen daemon logs -n 100       # Last 100 lines
lumen daemon logs --profile staging
```
...
```
$ lumen daemon logs -n 100
Reading /Users/you/.lumen/profiles/desktop-mbp/daemon.log (profile: desktop-mbp)
...
```
...
```bash
lumen daemon logs -n 500 | grep ERROR   # the path line is not in the pipe
```
...
`lumen daemon status --profile <name>` confirms which one is live.
...
| [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) | `dsh` | DeepSeek Harness (`dsh --profile lumen --stdio`; requires the Lumen runtime profile to be installed; reads AGENTS.md and .dsh/skills/) |
...
|---------|------|--------------|---------|
| Poll interval | `--poll-interval` | `LUMEN_DAEMON_POLL_INTERVAL` | `30s` (catch-up fallback; WebSocket wake signals deliver work sooner) |
| Healthy WebSocket claim poll upper bound | `--ws-claim-poll-interval` | `LUMEN_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` (configured independently of `--poll-interval`; downward jitter makes the normal interval `2m30s`–`2m45s`, while old servers and uncertain claims retain the ordinary poll interval) |
| Heartbeat interval | `--heartbeat-interval` | `LUMEN_DAEMON_HEARTBEAT_INTERVAL` | `15s` |
| Agent timeout | `--agent-timeout` | `LUMEN_AGENT_TIMEOUT` | `0` (no cap; bounded by the watchdogs) |
| Agent idle watchdog | — | `LUMEN_AGENT_IDLE_WATCHDOG` | `2h` (`0` disables the whole watchdog suite) |
| Agent tool watchdog | — | `LUMEN_AGENT_TOOL_WATCHDOG` | same as the idle watchdog (`0` = never force-stop during a tool call) |
| Codex semantic inactivity timeout | `--codex-semantic-inactivity-timeout` | `LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | same as the idle watchdog (Codex's timer is not tool-aware, so it tracks the larger of the idle / tool budgets) |
| Codex first-turn no-progress timeout | — | `LUMEN_CODEX_FIRST_TURN_TIMEOUT` | `0` (keeps the built-in `60s` ceiling) |
| Codex handshake timeout | `--codex-handshake-timeout` | `LUMEN_CODEX_HANDSHAKE_TIMEOUT` | `30s`; `thread/start` and `thread/resume`: `60s` (an explicit value overrides both budgets globally) |
| Codex turn-interrupt timeout | — | `LUMEN_CODEX_TURN_INTERRUPT_TIMEOUT` | `2s` (bounded grace period for `turn/interrupt` acknowledgement and `turn/completed`; tune from the logged interrupt latency on unusually slow hosts) |
| OpenCode idle watchdog | — | `LUMEN_OPENCODE_IDLE_WATCHDOG` | `10m` (`0` falls back to the generic idle watchdog; cannot extend it) |
| Max concurrent tasks | `--max-concurrent-tasks` | `LUMEN_DAEMON_MAX_CONCURRENT_TASKS` | `20` |
| Daemon ID | `--daemon-id` | `LUMEN_DAEMON_ID` | hostname |
| Device name | `--device-name` | `LUMEN_DAEMON_DEVICE_NAME` | hostname |
| Runtime name | `--runtime-name` | `LUMEN_AGENT_RUNTIME_NAME` | `Local Agent` |
| Workspaces root | — | `LUMEN_WORKSPACES_ROOT` | `~/lumen_workspaces` |
| GC enabled | — | `LUMEN_GC_ENABLED` | `true` (set `false`/`0` to disable) |
| GC scan interval | — | `LUMEN_GC_INTERVAL` | `2h` |
| GC TTL (done/cancelled issues) | — | `LUMEN_GC_TTL` | `24h` |
| GC completed-task TTL (issue tasks) | — | `LUMEN_GC_COMPLETED_TASK_TTL` | `14d` on Lumen Cloud, `0` (disabled) elsewhere |
| GC orphan TTL (no `.gc_meta.json`) | — | `LUMEN_GC_ORPHAN_TTL` | `72h` |
| GC artifact TTL (completed tasks) | — | `LUMEN_GC_ARTIFACT_TTL` | `12h` (set `0` to disable) |
| GC artifact patterns | — | `LUMEN_GC_ARTIFACT_PATTERNS` | `node_modules,.next,.turbo` |
| GC repo cache TTL (`.repos`) | — | `LUMEN_GC_REPO_TTL` | `720h` (30d; set `0` to disable) |
| GC repo maintenance | — | `LUMEN_GC_REPO_MAINTENANCE_ENABLED` | `true` (set `false`/`0` to disable heavy Git maintenance only) |
| GC Hermes memory TTL (per-agent `memories/`) | — | `LUMEN_GC_HERMES_MEMORY_TTL` | `2160h` (90d; set `0` to disable) |
| GC Hermes session TTL (per-conversation `state.db`) | — | `LUMEN_GC_HERMES_SESSION_TTL` | `336h` (14d; set `0` to disable) |
| GC task temp legacy TTL (pre-lock `lumen-task-*`) | — | `LUMEN_GC_TASK_TEMP_LEGACY_TTL` | `0` (disabled; set a duration to opt in) |

#### Workspace garbage collection

The daemon periodically scans `LUMEN_WORKSPACES_ROOT` and applies several disk-reclamation policies:

- **Full task cleanup** — when an issue's status is `done` or `cancelled` and has been idle for `LUMEN_GC_TTL`, the entire task directory is removed.
- **Completed-task retention bound** — `LUMEN_GC_COMPLETED_TASK_TTL` fully removes an inactive issue task once its `.gc_meta.json` `completed_at` age exceeds the configured duration, even while the parent issue remains open. Cleanup waits for a successful parent-issue status check, never removes an active environment, and never fully removes a `local_directory` environment. A later rerun provisions a fresh environment instead of resuming the removed checkout.
  - The default depends on where the daemon points: `14d` against Lumen Cloud, and `0` (disabled, retain indefinitely) for self-host and every other origin — including cloud staging and previews. Set the variable to opt in or out on either side; an explicit `0` disables the policy on Cloud too.
  - Removing an environment discards work an agent left uncommitted or unpushed on its branch, along with that task's `output/` and `logs/`. The per-issue Codex session store lives outside `LUMEN_WORKSPACES_ROOT` under its own TTL, so a later rerun still resumes the agent's prior session — it just starts from a fresh checkout. Size the TTL against that trade, and keep it comfortably above `LUMEN_GC_INTERVAL`: the active-root guard protects a task that is currently running, not one whose follow-up run is queued but unclaimed.
- **Orphan cleanup** — task directories with no `.gc_meta.json` (e.g. left over from a daemon crash) are removed once they exceed `LUMEN_GC_ORPHAN_TTL`.
- **Artifact-only cleanup** — when a task has been completed for at least `LUMEN_GC_ARTIFACT_TTL` but the issue is still open, regenerable build outputs whose directory basename matches `LUMEN_GC_ARTIFACT_PATTERNS` are removed. The daemon also reclaims the exact managed path `codex-home/.sandbox-bin`; old task metadata without `completed_at` becomes eligible for this managed-only cleanup after its `.gc_meta.json` file has been idle for `LUMEN_GC_ORPHAN_TTL`. The rest of the task (source, `.git`, `output/`, `logs/`, `.gc_meta.json`, Codex auth/config/session state) is preserved so the agent can resume it.
- **Managed-cache reclamation** — the exact managed path above is reclaimed for *every* task kind once the task has been completed for `LUMEN_GC_ARTIFACT_TTL`, not just for issue tasks whose issue is still open. It applies even while the parent record says the directory itself must stay — an active chat session, a still-running autopilot run — and even when the parent record could not be reached this cycle, because the contents are regenerable and the next run re-provisions them on demand. A task currently running on the directory is never touched. Set `LUMEN_GC_ARTIFACT_TTL=0` to disable this along with the rest of artifact cleanup.

- **Repo cache eviction** — the bare git clones under `.repos/` are shared object stores: each task workdir is a `git worktree` off one of them rather than its own clone, so a task's `.git` is only a pointer file. They are evicted only when all of the following hold: the repo is no longer attached to any workspace this daemon watches, it has no worktrees left, and no task has created a worktree from it for `LUMEN_GC_REPO_TTL`. A cache created before this stamp existed is not treated as ancient — its clock starts at the first GC cycle that sees it, so upgrading does not wipe every cache. Evicting is safe by construction: the next task that needs the repo re-clones it on demand, so a wrong eviction costs a clone, not a failure.

  Short worktree cleanup and eligible cache eviction continue on every GC cycle, including while agents are active. Heavy repo maintenance (`reflog expire` and `git gc`) starts only while the daemon is otherwise idle. A checkout or newly claimed task cancels it and takes priority; interrupted work remains pending for a later idle GC cycle. Operators can disable only these heavy commands with `LUMEN_GC_REPO_MAINTENANCE_ENABLED=false` without disabling worktree cleanup or cache eviction.

- **Hermes session store reclamation** — a conversation's Hermes transcript (`state.db`) lives at `<profile dir>/hermes-sessions/<agent-id>/<hermes-profile>/<conversation>/`, outside any task directory, so a follow-up turn can resume it (see [Hermes agent memory](#hermes-agent-memory)). A store untouched for `LUMEN_GC_HERMES_SESSION_TTL` is removed. The default matches the Codex session store rather than the memory store above: these hold full transcripts, and reclaiming an idle one costs a thread that starts fresh (with a continuity notice), not an agent that forgot what it learned. A store a running task holds is never reclaimed.
- **Hermes memory store reclamation** — a Hermes agent's long-term memory (`memories/`) lives at `<profile dir>/hermes-state/<agent-id>/<hermes-profile>/`, outside any task directory, so it survives across tasks and issues (see [Hermes agent memory](#hermes-agent-memory)). A store untouched for `LUMEN_GC_HERMES_MEMORY_TTL` is removed, giving a deleted agent's memory an eventual-reclamation guarantee. The default is deliberately long: these are a handful of markdown files, and reclaiming one is user-visible amnesia rather than a cache miss. A store a running task holds is never reclaimed.
- **Task temp dir reclamation** — every task gets a private temp directory (`lumen-task-*`) under the system temp base (`/tmp`, or `LUMEN_AGENT_TEMP_BASE`), exported to the agent as `TMPDIR`/`TMP`/`TEMP`. It is removed when the run ends, but that removal never happens when the daemon is killed and does not succeed while a file inside is still open — common on Windows, where an open handle makes the delete fail outright. These directories live outside `LUMEN_WORKSPACES_ROOT`, so nothing else reclaimed them and whatever the end-of-run removal missed accumulated forever. Every GC cycle now sweeps the temp base. Liveness is decided by the directory's `.task_lock` — the same OS advisory lock an env root uses, which the kernel releases when the holding process dies — not by age: a directory still in use is never removed however old it is, including one owned by a different daemon sharing the same temp base, and a directory whose owner is gone is removed on the next cycle however new it is. Directories left by a daemon predating that lock carry no lock file, so nothing can be proven about them and age is the only signal available. Reclaiming those is an operator's explicit decision: `LUMEN_GC_TASK_TEMP_LEGACY_TTL` defaults to `0`, which leaves them in place. Set it to a duration only once you know no pre-lock daemon is still running tasks on this machine — a task may legitimately run for weeks (there is no default agent timeout), a daemon on another profile can still be on the old binary, and every daemon on the machine shares one temp base, so a TTL here can delete a `TMPDIR` that is still in use. Each GC cycle logs how many such directories it left alone. Even with a TTL set, a directory holding no task content is never reclaimed on age — an old empty leftover, or a shell left by a daemon that died between creating the directory and publishing its lock — because holding no content is exactly what a directory currently being published looks like, and deleting one of those would take the `TMPDIR` of a task that is starting. Those shells are a few bytes each. Only entries carrying the `lumen-task-` prefix are ever considered — the temp base itself is usually shared with other programs — and a directory this sweep cannot read is never touched.

Configured patterns are basename-only — entries containing `/` or `\` are silently dropped — and `.git` subtrees are never descended into. The managed Codex cache is matched by its exact relative path, so a repository's own `.sandbox-bin` is not removed unless an operator explicitly adds that basename to `LUMEN_GC_ARTIFACT_PATTERNS`. The default list (`node_modules`, `.next`, `.turbo`) is intentionally narrow; extend it per deployment if your repos consistently produce other regenerable directories (for example, `LUMEN_GC_ARTIFACT_PATTERNS=node_modules,.next,.turbo,target,__pycache__`). To disable artifact cleanup entirely, including the managed Codex cache, set `LUMEN_GC_ARTIFACT_TTL=0`.

`lumen daemon disk-usage` reports the `.repos` footprint on its own line rather than folding it into the per-task totals — every task in a workspace checks out from that shared cache, so attributing it to individual task directories would double-count it. Note that the repo cache is reclaimed on the schedule above and not by any per-issue status change, so it is normal for it to persist after every task directory is gone.

Agent-specific overrides:

| Variable | Description |
|----------|-------------|
| `LUMEN_CLAUDE_PATH` | Custom path to the `claude` binary |
| `LUMEN_CLAUDE_MODEL` | Override the Claude model used |
| `LUMEN_CLAUDE_ARGS` | Default extra arguments for Claude Code runs |
| `LUMEN_ANTIGRAVITY_PATH` | Custom path to the `agy` binary |
| `LUMEN_ANTIGRAVITY_MODEL` | Override the Antigravity model used |
| `LUMEN_CODEBUDDY_PATH` | Custom path to the `codebuddy` binary |
| `LUMEN_CODEBUDDY_MODEL` | Override the CodeBuddy model used |
| `LUMEN_CODEBUDDY_ARGS` | Default extra arguments for CodeBuddy runs |
| `LUMEN_CODEARTS_PATH` | Custom path to the `codearts` launcher or binary |
| `LUMEN_CODEARTS_MODEL` | Override the CodeArts model used |
| `LUMEN_DEVECO_PATH` | Custom path to the `deveco` binary |
| `LUMEN_DEVECO_MODEL` | Override the DevEco Code model used |
| `LUMEN_CODEX_PATH` | Custom path to the `codex` binary |
| `LUMEN_CODEX_MODEL` | Override the Codex model used |
| `LUMEN_CODEX_ARGS` | Default extra arguments for Codex runs |
| `LUMEN_COPILOT_PATH` | Custom path to the `copilot` binary |
| `LUMEN_COPILOT_MODEL` | Override the Copilot model used (note: GitHub Copilot routes models through your account entitlement, so this may not be honoured) |
| `LUMEN_OPENCODE_PATH` | Custom path to the `opencode` binary |
| `LUMEN_OPENCODE_MODEL` | Override the OpenCode model used |
| `LUMEN_OPENCLAW_PATH` | Custom path to the `openclaw` binary |
| `LUMEN_OPENCLAW_MODEL` | Override the OpenClaw model used |
| `LUMEN_OPENCLAW_CLI_TIMEOUT` | Deadline for each `openclaw config ...` call during task preparation (default 30s; accepts `45s` or `45`). Raise it when the local CLI is slow to start; the daemon also reads it from `backends.openclaw.cli_timeout` in the CLI config |
| `LUMEN_HERMES_PATH` | Custom path to the `hermes` binary |
| `LUMEN_HERMES_MODEL` | Override the Hermes model used |
| `LUMEN_PI_PATH` | Custom path to the `pi` binary |
| `LUMEN_PI_MODEL` | Override the Pi model used |
| `LUMEN_CURSOR_PATH` | Custom path to the `cursor-agent` binary |
| `LUMEN_CURSOR_MODEL` | Override the Cursor Agent model used |
| `LUMEN_KIMI_PATH` | Custom path to the `kimi` binary |
| `LUMEN_KIMI_MODEL` | Override the Kimi model used |
| `LUMEN_REASONIX_PATH` | Custom path to the `reasonix` binary |
| `LUMEN_REASONIX_MODEL` | Override the Reasonix model used |
| `LUMEN_DIM_PATH` | Custom path to the `dim` binary |
| `LUMEN_DIM_MODEL` | Override the Dim model used |
| `LUMEN_KIRO_PATH` | Custom path to the `kiro-cli` binary |
| `LUMEN_KIRO_MODEL` | Override the Kiro model used |
| `LUMEN_QODER_PATH` | Custom path to the `qodercli` binary |
| `LUMEN_QODER_MODEL` | Override the Qoder model used |
| `LUMEN_QODERCLICN_PATH` | Custom path to the `qoderclicn` binary |
| `LUMEN_QODERCLICN_MODEL` | Override the Qoder CN model used |
| `LUMEN_TRAECLI_PATH` | Custom path to the `traecli` binary |
| `LUMEN_TRAECLI_MODEL` | Override the Trae model used (a model id from your logged-in traecli catalog, e.g. `Doubao-Seed-2.1-Pro`) |
| `LUMEN_GROK_PATH` | Custom path to the `grok` binary (defaults to `grok` on PATH; often `~/.grok/bin/grok`) |
| `LUMEN_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |
| `LUMEN_QWEN_PATH` | Custom path to the `qwen` binary |
| `LUMEN_QWEN_MODEL` | Override the Qwen Code model used |
| `LUMEN_QWEN_ARGS` | Daemon-wide extra Qwen arguments (POSIX shellword parsing; managed protocol flags are filtered) |
| `LUMEN_QWENPAW_PATH` | Custom path to the `qwenpaw` binary |
| `LUMEN_QWENPAW_ARGS` | Daemon-wide extra QwenPaw arguments (POSIX shellword parsing; managed protocol flags are filtered) |
| `LUMEN_MCODE_PATH` | Custom path to the `mcode` binary |
| `LUMEN_DSH_PATH` | Custom path to the `dsh` binary |
| `LUMEN_DSH_MODEL` | Override the DeepSeek Harness model used (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`) |

If a previously generated `~/.lumen/hooks` wrapper is first on `PATH` and calls the same command name again, the daemon skips that hooks directory during built-in agent discovery and records the real binary path behind it. If your interactive shell still recurses when you run `claude`, `codex`, or `hermes` manually, remove the hooks entry from your shell startup file or replace the wrapper body with an absolute `exec /path/to/real-binary "$@"`.
...
The daemon launches QwenPaw as `qwenpaw acp --workspace <per-task dir>`. It writes the task brief to `AGENTS.md`, and materialises the run's bound skills into `<per-task dir>/skills/` plus a `skill.json` manifest, so QwenPaw discovers them through its own workspace skill discovery. `acp` and `--workspace` are reserved: `custom_args` cannot override them. QwenPaw is the one runtime with no `LUMEN_QWENPAW_MODEL`: its `session/set_model` writes to a shared, persistent agent config rather than the session, so Lumen never sends it a model and leaves that choice to QwenPaw's own configuration.

The daemon launches MiniMax Code as `mcode acp`, writes the task brief to `AGENTS.md`, and injects bound skills under `.minimax/skills/`. MCode owns model selection and currently advertises `loadSession: false`; Lumen therefore starts a fresh MCode session when a later run cannot load the saved session.

#### Hermes agent memory

Hermes discovers skills only from its own home, so binding Lumen skills to a Hermes agent makes the daemon build a per-task `HERMES_HOME` overlay for that agent. The agent's long-term memory (`memories/`) does **not** live inside that task-scoped overlay: it is linked to a persistent store at
...
- **Every Hermes agent gets the overlay in practice**, so every one of them gets a persistent memory store. The daemon builds the overlay only when a task carries skills, but the server appends Lumen's built-in skills to every agent's skill set (`LoadAgentSkillBundles`), so that list is never empty — leaving an agent's own skill list empty does not opt out of the overlay, and is not a way to keep using the host's `~/.hermes/memories`.

`LUMEN_CLAUDE_ARGS`, `LUMEN_CODEX_ARGS`, `LUMEN_CODEBUDDY_ARGS`, `LUMEN_QWEN_ARGS`, and `LUMEN_QWENPAW_ARGS` are parsed with POSIX shellword quoting, so values such as `--model "gpt-5.1 codex" --sandbox read-only` are split like a shell command line. Agent arguments are applied in this order: hardcoded Lumen defaults, daemon-wide env defaults, then per-agent `custom_args` from the task.

### Self-Hosted Server

When connecting to a self-hosted Lumen instance, the easiest approach is:
...
lumen config set server_url http://localhost:8080
lumen config set app_url http://localhost:3000
...
lumen setup self-host --profile staging --server-url https://api-staging.example.com --app-url https://staging.example.com

# Start its daemon
lumen daemon start --profile staging

# Default profile runs separately
lumen daemon start
```

Each profile gets its own config directory (`~/.lumen/profiles/<name>/`), daemon state, health port, and workspace root. Daemon state means that profile's own `daemon.log`, `daemon.err.log`, and `daemon.pid` live in that directory too — see [Start](#start) for the layout, and pass `--profile <name>` to `daemon status` / `daemon logs` to act on it.
...
2. `LUMEN_WORKSPACE_ID` environment variable
3. The default workspace stored in your current profile (set by `lumen workspace switch` or `lumen login`)

`lumen workspace switch <id|slug>` is the day-to-day way to change the default workspace. For scripting and headless setups where you don't want any stored state, prefer the `--workspace-id` flag or the env variable. `lumen config set workspace_id <id>` is the low-level equivalent of `switch` (it writes the same setting but skips the access check).
...
```bash
lumen workspace list
lumen workspace list --full-id
lumen workspace list --output json
```
...
```bash
lumen workspace switch <workspace-id>
lumen workspace switch <slug>
```

Verifies you have access to the workspace, then sets it as the default for the current profile. Subsequent commands without `--workspace-id` and `LUMEN_WORKSPACE_ID` target this workspace. Pair `--profile` if you want to change a non-default profile's workspace.

### Get Details

```bash
lumen workspace get <workspace-id>
lumen workspace get <workspace-id> --output json
```

Passing no `<workspace-id>` resolves to the current default workspace, so `lumen workspace get` doubles as "what workspace am I on?".

### List Members

```bash
lumen workspace member list <workspace-id>
```
...
```bash
lumen issue list
lumen issue list --status in_progress
lumen issue list --priority urgent --assignee "Agent Name"
lumen issue list --assignee-id 5fb87ac7-23b5-4a7a-81fa-ed295a54545d
lumen issue list --full-id
lumen issue list --limit 20 --output json
lumen issue list --status todo --sort position       # board order (the default)
lumen issue list --sort created_at --direction desc  # newest first
lumen issue list --output json --fields=id,title,status,priority  # narrow the JSON payload
lumen issue list --output json --resolve-properties  # property names beside the ids
```
...
```bash
lumen issue list --metadata pipeline_status=waiting_review
lumen issue list --metadata pr_number=482 --metadata is_blocked=true
```
...
```bash
lumen issue list --property "Impact=High" --property "Impact=Medium"
lumen issue list --property "Impact=__none__" --status in_review
lumen issue list --property "Score=42" --property "Ship Date=2026-08-28"
```
...
```bash
lumen issue list --output json --resolve-properties | jq '.issues[] | {identifier, properties: [.properties[]? | {name, display}]}'
```
...
```bash
lumen issue get <id>
lumen issue get <id> --output json
lumen issue get <id> --resolve-properties   # property names beside the ids, as in issue list
```

### Create Issue

```bash
lumen issue create --title "Fix login bug" --description "..." --priority high --assignee "Lambda"
lumen issue create --title "Fix login bug" --assignee-id 5fb87ac7-23b5-4a7a-81fa-ed295a54545d
```

Flags: `--title` (required), `--description`, `--status`, `--priority`, `--assignee` / `--assignee-id`, `--parent`, `--project`, `--due-date`. Pass `--assignee-id <uuid>` (mutually exclusive with `--assignee`) when scripting against the IDs returned by `lumen workspace member list --output json` / `lumen agent list --output json`.

### Update Issue

```bash
lumen issue update <id> --title "New title" --priority urgent
lumen issue update <id> --position 4.5
```
...
```bash
lumen issue reorder <id> --top              # top of its status column
lumen issue reorder <id> --bottom           # bottom of its status column
lumen issue reorder <id> --before <other>   # directly above another issue in the same column
lumen issue reorder <id> --after  <other>   # directly below another issue in the same column
```
...
```bash
lumen issue assign <id> --to "Lambda"
lumen issue assign <id> --to-id 5fb87ac7-23b5-4a7a-81fa-ed295a54545d
lumen issue assign <id> --unassign
```
...
```bash
lumen issue status <id> in_progress
```
...
lumen issue comment list <issue-id> --thread <comment-id>
...
lumen issue comment list <issue-id> --thread <comment-id> --tail 30
...
lumen issue comment list <issue-id> --recent 10
...
# Add a comment
lumen issue comment add <issue-id> --content "Looks good, merging now"

# Reply to a specific comment
lumen issue comment add <issue-id> --parent <comment-id> --content "Thanks!"

# Delete a comment
lumen issue comment delete <comment-id>
```
...
no-op. The server emits the cursor headers (`X-Lumen-Next-Before` /
`X-Lumen-Next-Before-Id`) only when an older page actually exists —
...
lumen issue metadata list <issue-id>

# Read a single key
lumen issue metadata get <issue-id> --key pipeline_status

# Write a single key — value auto-typed (true/false → bool, numbers → number, else string)
lumen issue metadata set <issue-id> --key pipeline_status --value waiting_review
lumen issue metadata set <issue-id> --key pr_number --value 482
lumen issue metadata set <issue-id> --key is_blocked --value true

# Force a specific type when sniffing would pick the wrong one
lumen issue metadata set <issue-id> --key code --value 42 --type string

# Remove a key
lumen issue metadata delete <issue-id> --key pipeline_status
```

All writes are single-key atomic — concurrent agents writing different keys do not lose each other's updates. To query, use `lumen issue list --metadata key=value` (see *List Issues* above).

### Subscribers

```bash
# List subscribers of an issue
lumen issue subscriber list <issue-id>

# Subscribe yourself to an issue
lumen issue subscriber add <issue-id>

# Subscribe another member or agent by name
lumen issue subscriber add <issue-id> --user "Lambda"

# Unsubscribe yourself
lumen issue subscriber remove <issue-id>

# Unsubscribe another member or agent
lumen issue subscriber remove <issue-id> --user "Lambda"
```
...
lumen issue runs <issue-id>
lumen issue runs <issue-id> --full-id
lumen issue runs <issue-id> --output json

# Only work in flight (queued / dispatched / running / waiting_local_directory)
lumen issue runs <issue-id> --active --output json
...
lumen issue run-messages <task-id>
lumen issue run-messages <short-task-id> --issue <issue-id>
lumen issue run-messages <task-id> --output json

# Incremental fetch (only messages after a given sequence number)
lumen issue run-messages <task-id> --since 42 --output json

# Aggregated token usage for an issue (sum across all its task runs)
lumen issue usage <issue-id>
lumen issue usage <issue-id> --output json
```
...
```bash
lumen project list
lumen project list --status in_progress
...
```bash
lumen project get <id>
lumen project get <id> --output json
```

### Create Project

```bash
lumen project create --title "2026 Week 16 Sprint" --icon "🏃" --lead "Lambda"
```
...
```bash
lumen project update <id> --title "New title" --status in_progress
lumen project update <id> --lead "Lambda"
lumen project update <id> --due-date 2026-04-15
```
...
```bash
lumen project status <id> in_progress
```
...
```bash
lumen issue create --title "Login bug" --project <project-id>
lumen issue update <issue-id> --project <project-id>
lumen issue list --project <project-id>
```

## Setup

```bash
# One-command setup for Lumen Cloud: configure, authenticate, and start the daemon
lumen setup
...
# Custom ports
lumen setup self-host --port 9090 --frontend-port 4000

# On-premise with custom domains
lumen setup self-host --server-url https://api.example.com --app-url https://app.example.com
```

`lumen setup` configures the CLI, opens your browser for authentication, and starts the daemon — all in one step. Use `lumen setup self-host` to connect to a self-hosted server instead of Lumen Cloud.
...
lumen config set server_url https://api.example.com
lumen config set app_url https://app.example.com
lumen config set workspace_id <workspace-id>
```

`config set workspace_id <id>` is the low-level interface — it writes the value verbatim without checking that the workspace exists or that you have access. Prefer `lumen workspace switch <id|slug>` for day-to-day workspace changes; it does both checks before saving.
...
```bash
lumen autopilot list
lumen autopilot list --full-id
lumen autopilot list --status active --output json
```
...
```bash
lumen autopilot get <id>
lumen autopilot get <id> --output json   # includes triggers
```
...
lumen autopilot update <id> --status paused
lumen autopilot update <id> --description "New prompt"
lumen autopilot update <id> --subscriber "Alice" --subscriber "Bob"
lumen autopilot update <id> --clear-subscribers
lumen autopilot delete <id>
```
...
```bash
lumen autopilot trigger <id>            # Fires the autopilot once, returns the run
```
...
```bash
lumen autopilot runs <id>
lumen autopilot runs <id> --limit 50 --output json
```

### Schedule Triggers

```bash
lumen autopilot trigger-list <autopilot-id>              # ids, kind, schedule, next run
lumen autopilot trigger-list <autopilot-id> --full-id    # canonical UUIDs
lumen autopilot trigger-add <autopilot-id> --cron "0 9 * * 1-5" --timezone "America/New_York"
lumen autopilot trigger-update <autopilot-id> <trigger-id> --enabled=false
lumen autopilot trigger-delete <autopilot-id> <trigger-id>
```
...
```bash
lumen version              # Show CLI version and commit hash
lumen update               # Update to latest version
lumen agent list           # List agents in the current workspace
```
...
- **Friendly, single-line message.** Transport failures (timeout, DNS,
  ...
  `LUMEN_HTTP_TIMEOUT`, and a 401 tells you to run `lumen login`.
...
```bash
LANG=zh_CN.UTF-8 lumen issue get MUL-9999   # 错误信息显示为中文
```
...
```bash
lumen issue get MUL-9999
if [ $? -eq 4 ]; then echo "no such issue"; fi
```

### Seeing the full detail (`--debug`)

Pass the global `--debug` flag (or set `LUMEN_DEBUG=1`) to print the complete
...
```bash
lumen issue list --debug
LUMEN_DEBUG=1 lumen issue update MUL-1234 --title "x"
```

### Request timeout

API requests use a default timeout of 30 seconds. Override it with
`LUMEN_HTTP_TIMEOUT` when you are on a slow network; it accepts a Go duration
...
```bash
LUMEN_HTTP_TIMEOUT=60s lumen issue list
```
...
Override the no-progress budget with `LUMEN_HTTP_STALL_TIMEOUT` (same format
as `LUMEN_HTTP_TIMEOUT`). If only `LUMEN_HTTP_TIMEOUT` is set it applies on
...
```bash
LUMEN_HTTP_STALL_TIMEOUT=45s lumen skill get <id>
```
...
`lumen skill get` and `lumen skill files list` return **metadata only** by
...
```bash
lumen skill files list <id>                  # paths and sizes
lumen skill files list <id> --with-content   # bodies inlined
```
...
Create custom Oh-My-Pi profiles with `lumen runtime profile create --runtime-type omp --command-name omp --display-name "Custom Oh-My-Pi"`.
