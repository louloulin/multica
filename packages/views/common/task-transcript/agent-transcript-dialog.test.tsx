...
import type { AgentRuntime, AgentTask } from "@lumen/core/types/agent";
import { useTranscriptViewStore } from "@lumen/core/agents/stores";
...
vi.mock("@lumen/ui/lib/clipboard", () => ({
  ...
  it("states the agent once in the header, not on every prose row", () => {
    renderWithI18n(
      <AgentTranscriptDialog
        ...
        agentName="【Chores|Opus5】Lumen Helper"
      />,
    );

    expect(screen.getAllByTestId("rich-content")).toHaveLength(3);
    expect(screen.getAllByText("【Chores|Opus5】Lumen Helper")).toHaveLength(1);
...
describe("readable issue references", () => {
  it("searches both the displayed identifier and original UUID", async () => {
    ...
    renderDialog([{ seq: 1, type: "tool_use", tool: "exec_command", input: { command: `lumen issue get ${issueId} --output json` } }]);
    expect(screen.getByText("lumen issue get DEV-17 --output json")).toBeInTheDocument();
