...
import { runtimeKeys } from "@lumen/core/runtimes/queries";
...
import { CODE_LIGATURE_CLASS } from "@lumen/ui/lib/code-style";
import { copyText } from "@lumen/ui/lib/clipboard";
import {
  UI_EASE_OUT,
  UI_MOTION_DURATION,
} from "@lumen/ui/lib/motion";
...
const CLOUD_SERVER_URL = "https://api.lumen.ai";
const CLOUD_APP_URL = "https://lumen.ai";
...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `lumen setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `lumen config set server_url ${normalizedServerUrl}
lumen config set app_url ${normalizedAppUrl}
lumen login --token <YOUR_TOKEN>
lumen daemon start`,
    };
  }

  return {
    setupCmd: "lumen setup",
    tokenCmd: `lumen config set server_url ${CLOUD_SERVER_URL}
lumen config set app_url ${CLOUD_APP_URL}
lumen login --token <YOUR_TOKEN>
lumen daemon start`,
  };
}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `lumen setup` is one blocking command that handles config + login
