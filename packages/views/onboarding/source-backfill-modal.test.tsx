...
vi.mock("@lumen/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@lumen/core/onboarding")>(
      "@lumen/core/onboarding",
    );
...
vi.mock("@lumen/core/api", async () => {
  const actual =
    await vi.importActual<typeof import("@lumen/core/api")>(
      "@lumen/core/api",
    );
  return {
    ...actual,
...
describe("SourceBackfillModal", () => {
  ...
  // @lumen/core/onboarding/needs-backfill.test.ts. What belongs here is the
  ...
  it("does not render when the user already recorded a source", () => {
    ...
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Lumen/i),
  ...
  it("opens for an onboarded user with empty source once agents completed enough issues", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Lumen/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("lumen.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
