...
4. Confirm your terminal can find it:
   which codex
   codex --version
5. Wait for Lumen to pick it up. A running daemon re-checks for newly
...
5. 等 Lumen 识别到它。运行中的守护进程每隔几分钟会重新检查一次新装的 CLI,通常不需要重启。
   想立刻生效:
   lumen daemon restart
...
const ko = `Lumen에 오신 것을 환영합니다.

agent가 작업을 실행하려면 먼저 runtime이 필요합니다. runtime을 설치하는 동안에도 Lumen를 가벼운 프로젝트 관리 워크스페이스로 먼저 사용할 수 있습니다.
...
전체 가이드: https://lumen.ai/docs/install-agent-runtime
...
4. 터미널에서 찾을 수 있는지 확인합니다:
   which codex
   codex --version
5. Lumen가 인식할 때까지 기다립니다. 실행 중인 daemon은 몇 분마다 새로 설치된 CLI를
...
const ja = `Lumen へようこそ。

agent が作業を実行するには、まず runtime が必要です。runtime をインストールしている間も、Lumen を軽量なプロジェクト管理ワークスペースとして先に使うことができます。
...
詳しいガイド: https://lumen.ai/docs/install-agent-runtime
...
4. ターミナルから見つけられるか確認します:
   which codex
   codex --version
5. Lumen が認識するまで待ちます。動作中の daemon は数分ごとに新しくインストールされた
