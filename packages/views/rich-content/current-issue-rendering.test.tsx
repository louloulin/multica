...
vi.mock("../issues/hooks", () => ({
  useResolveIssueIdentifier: (identifier: string) => {
    if (identifier === "LUM-7") return resolvedIssues.current;
    if (identifier === "LUM-8") return resolvedIssues.other;
    return null;
  },
}));
...
const CONTENT = [
  `See [LUM-7](mention://issue/${CURRENT_ID})`,
  "LUM-8",
...
const SELF_REFERENCE_FORMS: ReadonlyArray<readonly [string, string]> = [
  ["a bare identifier", "See LUM-7 and LUM-8."],
  [
    "a UUID URL",
  ...
  [
    "an identifier URL",
    `See ${APP_ORIGIN}/acme/issues/LUM-7 and ${APP_ORIGIN}/acme/issues/LUM-8.`,
  ],
];
...
beforeEach(() => {
  resolvedIssues.current = makeIssue(CURRENT_ID, "LUM-7");
  resolvedIssues.other = makeIssue(OTHER_ID, "LUM-8");
});
...
describe("RichContent current-issue rendering", () => {
  it("marks only the current UUID mention in a paragraph with two other references as current", () => {
    ...
    expect(chips[0]).toHaveTextContent("This issue · LUM-7");
  });
