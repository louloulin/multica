...
import type { DraftUpload } from "@lumen/core/drafts";
...
function renderInput(props: Partial<React.ComponentProps<typeof ChatInput>> = {}) {
  ...
  const view = render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={onSend} uploadEnabled agentName="Lumen" {...props} />
    </I18nProvider>,
  );
  return { onSend, ...view };
}
...
function element(props: Partial<React.ComponentProps<typeof ChatInput>>) {
  return (
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={vi.fn()} uploadEnabled agentName="Lumen" {...props} />
    </I18nProvider>
  );
}
...
describe("ChatInput focusRequest", () => {
  it("focuses the editor when focusRequest becomes a non-zero value (new chat)", () => {
    const { rerender } = render(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Lumen" focusRequest={0} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Lumen" focusRequest={1} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Lumen" focusRequest={2} />
      </I18nProvider>,
    );
...
describe("ChatInput revoked-access placeholder", () => {
  ...
  it("leaves the normal placeholder alone when access is intact", () => {
    renderInput({ agentName: "Lumen" });

    expect(editorProps.last?.placeholder).toBe("Message Lumen…");
  });
});
