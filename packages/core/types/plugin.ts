/**
 * A plugin relates to Lumen in exactly three ways: Action (plugin calls
 * Lumen), Hook (Lumen calls plugin), and Resource (a static contribution
...
export interface PluginSurfaceLaunch {
  /** Lumen's cookie-free content URL, never the plugin author's server. */
  url: string;
