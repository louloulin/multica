// Brand host shown as the workspace URL prefix on the managed Lumen Cloud,
...
// self-hosted server that has not set LUMEN_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "lumen.ai";
 ...
 * self-hosted instances show their own domain instead of `lumen.ai`. Falls
