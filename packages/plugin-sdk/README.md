...
What a Lumen plugin surface imports.

```js
import { lumen } from "@lumen/plugin-sdk";

const ctx   = await lumen.context.get();
const issue = await lumen.issue.get();
await lumen.issue.comment({ body: "hello" });
const note  = await lumen.storage.user.get("note");
lumen.ui.resize(320);
```
...
is no module graph: you publish an artifact, Lumen stores it and serves your
...
- **No browser storage.** `localStorage`, `sessionStorage` and cookies all throw
  or are empty. Use `lumen.storage` — it is server-side, scoped per workspace
...
- **A CSP you did not write.** Lumen generates the response and derives
  ...
  policy now that Lumen serves your code. `net:` is an exact host, so declare
...
`LumenPluginError` with a `status` mirroring HTTP.
...
`lumen.ui.onThemeChange(fn)` if you need to react in JS.

## Sizing

The frame does not auto-size. Call `lumen.ui.resize(px)` after your content
