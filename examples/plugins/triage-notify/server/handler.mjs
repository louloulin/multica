 ...
 *
 * This is the half that does NOT run in Lumen. It is an ordinary HTTP server
 * the author operates; Lumen only ever sends it a signed POST. Everything
...
/**
 * Calls Lumen back using the one-shot token that arrived with the request.
 *
...
async function callback(body, method, path, payload) {
  ...
  if (!response.ok) {
    throw new Error(`Lumen answered ${response.status}: ${await response.text()}`);
  }
