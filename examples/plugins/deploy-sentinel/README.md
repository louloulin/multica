...
Two servers, both the plugin author's own. Lumen never runs either.
...
LUMEN_SIGNING_SECRET=whsec_... node server/handler.mjs      # :8788
...
Then point Lumen at them. Both endpoints are on loopback, which the outbound
guard refuses by design, so name them explicitly:

```bash
export LUMEN_PLUGIN_DIR=examples/plugins
export LUMEN_PLUGIN_DEV_ORIGINS=https://127.0.0.1:8788,https://127.0.0.1:8789
export LUMEN_PLUGIN_DEV_CA=/path/to/your-dev-ca.pem
```

Publish it from `LUMEN_PLUGIN_DIR` as `deploy-sentinel`, install the version
...
Note what `LUMEN_PLUGIN_DIR` does and does not cover. It is a publishing
...
and `LUMEN_PLUGIN_DEV_CA` is how you get a self-signed certificate trusted.
