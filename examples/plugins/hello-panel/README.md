...
The reference Lumen plugin: one `issue_panel` surface that exercises every
...
- `lumen.context.get()` — who is looking and which issue the panel is on
- `lumen.issue.get()` — reading the issue behind `issues:read`
- `lumen.issue.comment()` — a write that lands as **the user**, marked with
  the plugin (`via_plugin_id`), behind `comments:write`
- `lumen.storage.user` — per-member state behind `storage:user`
- `lumen.ui.resize()` — asking the host for the height it actually needs

## Running it

Zip this folder — the manifest plus every file it names — and upload it in
**Settings → Plugins**. You need no server of your own: Lumen stores the
...
simplification for the example: Lumen serves the entry inside one generated
document with no module graph, so a bare module specifier has nowhere to
resolve. Bundle your dependencies in.

While you are iterating, `LUMEN_PLUGIN_DIR` publishes straight from disk
