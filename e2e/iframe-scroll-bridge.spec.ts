 ...
 * (lumen-ai#6405). No app server: `page.setContent` with a real
...
<script>
  ...
  window.addEventListener("message", (e) => {
    if (e.data && e.data.__lumen === "__lumen") window.__messages.push(e.data);
  });
</script></body></html>`;
}
...
test.describe("iframe scroll bridge (real Chromium, sandboxed srcdoc)", () => {
  ...
  test("restores scroll position on a parent restore message", async ({
    ...
    await page.evaluate(
      ([token]) => {
        ...
        w.postMessage(
          { __lumen: "__lumen", kind: "restore", y: 600, token },
          "*",
        );
      },
      [TOKEN],
  ...
  test("holds position across a 1s-delayed content append, then yields on user wheel (v4 guarantee)", async ({
    ...
    await page.evaluate(
      ([token]) => {
        const w = document.querySelector<HTMLIFrameElement>("#f")!.contentWindow!;
        w.postMessage(
          { __lumen: "__lumen", kind: "restore", y: 500, token },
          "*",
        );
      },
      [TOKEN],
  ...
  test("ignores restore messages with a mismatched token", async ({
    ...
    await page.evaluate(() => {
      const w = document.querySelector<HTMLIFrameElement>("#f")!.contentWindow!;
      w.postMessage(
        { __lumen: "__lumen", kind: "restore", y: 777, token: "old" },
        "*",
      );
    });
    await page.waitForTimeout(200);
      ...
      ([token, target]) =>
        new Promise<{
          ...
          window.addEventListener("message", (e) => {
            ...
            const d = e.data as
              | { __lumen?: string; token?: string; kind?: string }
              | undefined;
