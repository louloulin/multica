...
RUN cd server && CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${COMMIT} -X main.date=${DATE}" -o bin/lumen ./cmd/lumen
...
COPY --from=builder /src/server/bin/lumen .
