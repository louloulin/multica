...
This document is designed for AI agents to execute. Follow these steps exactly to deploy a local Lumen instance and connect to it.
...
Wait for the server output `✓ Lumen server is running and CLI is ready!` before running `lumen setup self-host`.

**Expected result:**
- Frontend at http://localhost:3000
- Backend at http://localhost:8080
- `lumen` CLI installed and configured for localhost
...
The `lumen setup self-host` command will:
...
2. Run `make selfhost`
3. Run `lumen setup self-host --port <PORT> --frontend-port <FRONTEND_PORT>`
...
- **Daemon issues:** `lumen daemon logs`
