...
not import any other Lumen package, so it can be reused by other services or
...
import (
    ...
    "github.com/lumen-ai/lumen/server/pkg/composio"
)
...
link, err := client.CreateLink(ctx, composio.CreateLinkRequest{
    ...
    UserID:       lumenUserID.String(), // your own user id
    CallbackURL:  "https://lumen.ai/api/integrations/composio/callback",
})
...
- **Standalone.** Zero coupling to Lumen internals — depend on this package
