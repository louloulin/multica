...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `lumen issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `lumen issue comment add` by its command
...
// `lumen issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "lumen issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "lumen" && !strings.HasSuffix(executable, "/lumen") {
		return false
	}
...
// like `LUMEN_TOKEN=x lumen issue comment add ...` is still recognized.
