...
const (
	lumenCodexMcpBeginMarker = "# BEGIN lumen-managed mcp_servers (do not edit; regenerated by daemon)"
	lumenCodexMcpEndMarker   = "# END lumen-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(lumenCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(lumenCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = lumenCodexMcpBeginMarker + "\n" + lumenCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(lumenCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(lumenCodexMcpEndMarker)
	sb.WriteString("\n")
...
func isLumenMcpSelectorKey(k string) bool {
	switch k {
...
//
// An explicit configured override (LUMEN_CODEX_FIRST_TURN_TIMEOUT) is honored
