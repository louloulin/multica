...
// LUMEN_CODEX_FIRST_TURN_TIMEOUT path added for GH #3262 / #5959: a positive
...
func TestCodexRawItemMCPToolCall(t *testing.T) {
	...
	c.handleLine(`{"jsonrpc":"2.0","method":"item/started","params":{"item":{"type":"mcpToolCall","id":"mcp-1","server":"plugin-exa-search","tool":"web_search_exa","arguments":{"query":"latest Lumen news","credentials":{"api_key":"sk-12345678901234567890"}},"status":"inProgress"}}}`)
	c.handleLine(`{"jsonrpc":"2.0","method":"item/completed","params":{"item":{"type":"mcpToolCall","id":"mcp-1","server":"plugin-exa-search","tool":"web_search_exa","arguments":{"query":"latest Lumen news"},"status":"completed","durationMs":1429,"result":{"content":[{"type":"text","text":"private provider payload"}]}}}}`)
	...
	if !ok || arguments["query"] != "latest Lumen news" {
...
func TestCodexDeliverableOutputExcludesNarration(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// still stream to the timeline the Lumen UI renders.
...
// codexRuntimeBriefCanary stands in for the Lumen runtime brief the daemon
// would inline if developerInstructions were ever wired back up.
const codexRuntimeBriefCanary = "LUMEN-RUNTIME-BRIEF-CANARY"
...
// timer contract for LUMEN_CODEX_FIRST_TURN_TIMEOUT (GH #3262 / #5959): the
...
func TestEnsureCodexMcpConfigEmptyClearsBlock(t *testing.T) {
	...
	initial := "sandbox_mode = \"workspace-write\"\n\n" +
		lumenCodexMcpBeginMarker + "\n" +
		"[mcp_servers.fetch]\ncommand = \"uvx\"\n" +
		lumenCodexMcpEndMarker + "\n\n" +
	...
	got := string(data)
	if strings.Contains(got, lumenCodexMcpBeginMarker) {
...
func TestEnsureCodexMcpConfigWritesManagedBlock(t *testing.T) {
	...
	if !strings.Contains(got, lumenCodexMcpBeginMarker) || !strings.Contains(got, lumenCodexMcpEndMarker) {
