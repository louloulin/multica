...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Lumen Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Lumen user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, lumen_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Lumen user is removed from the workspace the stale Lark
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Lumen `chat_session`. The Lark side
...
-- =====================
-- Maps a Lumen task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Lumen user. The
