...
--
-- No foreign keys / cascades by design: Lumen enforces cross-table
...
--
-- composio_user_id always equals the Lumen user_id.String() — the
-- application keeps that mapping as an invariant so a Composio session can be
-- created from the Lumen user id alone. It is stored explicitly so a future
