...
func TestRedisClientName(t *testing.T) {
	tests := []struct {
		...
		{"empty_suffix_returns_existing", "lumen-api:store", "", "lumen-api:store"},
		{"empty_existing_uses_default_prefix", "", "store", "lumen-api:store"},
...
func TestNewNamedRedisClient_SetsClientName(t *testing.T) {
	...
	if opts.ClientName != "lumen-api:store" {
		t.Errorf("ClientName = %q, want %q", opts.ClientName, "lumen-api:store")
	}
}
...
func TestNewNamedRedisClient_DisableClientName_InvalidValue(t *testing.T) {
	...
	if opts.ClientName != "lumen-api:store" {
		t.Errorf("ClientName = %q, want %q (invalid env should fall back to naming enabled)", opts.ClientName, "lumen-api:store")
	}
}
...
func TestJWTSecretBootError(t *testing.T) {
	...
	tests := []struct {
		...
		{"production_with_code_default_is_rejected", "lumen-dev-secret-change-in-production", "production", true},
