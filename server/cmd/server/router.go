...
import (
	...
	composiointeg "github.com/lumen-ai/lumen/server/internal/integrations/composio"
	...
	composiosdk "github.com/lumen-ai/lumen/server/pkg/composio"
...
// LUMEN_APP_URL and falls back to FRONTEND_ORIGIN, matching how the backend
// resolves the app URL elsewhere (handler.daemonSetupURLsFromEnv) and the CLI
// login flow (cmd/lumen tryResolveAppURL). Empty when neither is set.
func appURLFromEnv() string {
	if v := strings.TrimRight(strings.TrimSpace(os.Getenv("LUMEN_APP_URL")), "/"); v != "" {
		return v
	}
...
// LUMEN_PLUGIN_API_URL; self-hosted and local deployments can leave it empty
// and serve the same /v1 contract on LUMEN_PUBLIC_URL.
func pluginActionBaseURL(publicURL string) string {
	if value := strings.TrimRight(strings.TrimSpace(os.Getenv("LUMEN_PLUGIN_API_URL")), "/"); value != "" {
		return value
	}
...
// LUMEN_TRUSTED_PROXIES env var. Invalid entries are dropped with a single
...
func parseTrustedProxies(raw string) []netip.Prefix {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil {
			slog.Warn("LUMEN_TRUSTED_PROXIES: ignoring invalid CIDR",
				"value", s, "error", err)
			continue
		}
		out = append(out, p)
...
type RouterOptions struct {
	...
	// LLMMaxRetries carries the parsed LUMEN_LLM_MAX_RETRIES budget. Unlike
	// its three LUMEN_LLM_* siblings it is injected rather than read here,
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb redis.UniversalClient, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		...
		VCSIntegrationEnabled:    os.Getenv("LUMEN_VCS_INTEGRATION_ENABLED") == "true",
		PublicURL:                strings.TrimRight(strings.TrimSpace(os.Getenv("LUMEN_PUBLIC_URL")), "/"),
		AppURL:                   appURLFromEnv(),
		TrustedProxies:           parseTrustedProxies(os.Getenv("LUMEN_TRUSTED_PROXIES")),
		CloudURL:                 strings.TrimSpace(os.Getenv("LUMEN_CLOUD_URL")),
		CloudTimeout:             35 * time.Second,
		AttachmentDownloadMode:   os.Getenv("ATTACHMENT_DOWNLOAD_MODE"),
		AttachmentDownloadURLTTL: envDuration("ATTACHMENT_DOWNLOAD_URL_TTL", 30*time.Minute),
		AttachmentFrameAncestors: origins,
		PluginSurfaceOrigin:      strings.TrimRight(strings.TrimSpace(os.Getenv("LUMEN_PLUGIN_SURFACE_ORIGIN")), "/"),
		LLMAPIKey:                strings.TrimSpace(os.Getenv("LUMEN_LLM_API_KEY")),
		LLMBaseURL:               strings.TrimSpace(os.Getenv("LUMEN_LLM_BASE_URL")),
		LLMDefaultModel:          strings.TrimSpace(os.Getenv("LUMEN_LLM_DEFAULT_MODEL")),
	...
	if entitlementErr != nil {
		slog.Error("entitlement policy client disabled by malformed Lumen Cloud URL", "error", entitlementErr)
	...
	if opts.BusinessMetrics != nil {
		...
		// lumen_cloudruntime_request_* histograms.
	...
	// Lark integration. Only wired when LUMEN_LARK_SECRET_KEY is set:
	...
	if larkKey, err := secretbox.LoadKey("LUMEN_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// LUMEN_LARK_SECRET_KEY is the operator's opt-in for
				...
				// real Lark traffic can point LUMEN_LARK_HTTP_BASE_URL
				// at a mock server.
				//
				// LUMEN_LARK_HTTP_BASE_URL is an OPTIONAL deployment-wide
				...
				larkClient := lark.NewHTTPAPIClient(lark.HTTPClientConfig{
					BaseURL: strings.TrimSpace(os.Getenv("LUMEN_LARK_HTTP_BASE_URL")),
					Logger:  slog.Default(),
				})
				...
				// to initialize (bad LUMEN_LARK_CALLBACK_BASE_URL or
				...
				go lark.BackfillRegionFromLegacyOverride(context.Background(), cs,
					strings.TrimSpace(os.Getenv("LUMEN_LARK_HTTP_BASE_URL")),
					strings.TrimSpace(os.Getenv("LUMEN_LARK_CALLBACK_BASE_URL")),
					slog.Default())
				...
				// LUMEN_LARK_REGISTRATION_DOMAIN / _LARK_DOMAIN env
				// vars override the protocol hosts for staging / dev.
				regCfg := lark.RegistrationConfig{
					Domain:     strings.TrimSpace(os.Getenv("LUMEN_LARK_REGISTRATION_DOMAIN")),
					LarkDomain: strings.TrimSpace(os.Getenv("LUMEN_LARK_REGISTRATION_LARK_DOMAIN")),
				}
		...
		slog.Info("lark integration disabled (LUMEN_LARK_SECRET_KEY not set)")
	}

	// Slack integration. Multi-tenant B2 model (MUL-3666): Lumen hosts ONE
	...
	//   - LUMEN_SLACK_SECRET_KEY decrypts the per-installation bot token
	...
	//     Slack at all.
	//   - LUMEN_SLACK_APP_TOKEN is the app-level token (xapp-) authorizing the
	...
	if slackKey, err := secretbox.LoadKey("LUMEN_SLACK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			// the Slack user to their Lumen account.
			...
			slackReplier := slack.NewOutboundReplier(slack.OutboundReplierConfig{
				...
				// app URL (LUMEN_APP_URL ?? FRONTEND_ORIGIN), NOT LUMEN_PUBLIC_URL
			...
			// On-demand history reader behind the unified `lumen chat history`
		...
		slog.Info("slack integration disabled (LUMEN_SLACK_SECRET_KEY not set)")
	}
	...
	if dingtalkKey, err := secretbox.LoadKey("LUMEN_DINGTALK_SECRET_KEY"); err == nil {
		...
		slog.Info("dingtalk integration disabled (LUMEN_DINGTALK_SECRET_KEY not set)")
	}
	...
	//
	// Gated by LUMEN_WECOM_SECRET_KEY. Without it, the whole block is
	...
	if wecomKey, err := secretbox.LoadKey("LUMEN_WECOM_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// Binding tokens back the per-user "link your Lumen account"
				...
				// not linked to a Lumen account yet. A linked person always
				...
				slog.Info("wecom deployment locale",
					"locale", wecom.SetDeploymentLocale(os.Getenv("LUMEN_WECOM_DEFAULT_LOCALE")))
				...
				// Without it the agent's reply lands only in Lumen's web UI
				...
				if raw := strings.TrimSpace(os.Getenv("LUMEN_WECOM_MEDIA_ALLOW_CIDRS")); raw != "" {
				...
				if wecom.SetTrace(os.Getenv("LUMEN_WECOM_TRACE") == "1") {
					slog.Warn("wecom: frame tracing ON — records message text; unset LUMEN_WECOM_TRACE when done")
				}
		...
		slog.Info("wecom integration disabled (LUMEN_WECOM_SECRET_KEY not set)")
	}
	...
	// the event bus. Gated by LUMEN_TELEGRAM_SECRET_KEY (the at-rest token
	// encryption key); when unset the handlers return 503 and no Factory is
	// registered.
	if telegramKey, err := secretbox.LoadKey("LUMEN_TELEGRAM_SECRET_KEY"); err == nil {
		...
		slog.Info("telegram integration disabled (LUMEN_TELEGRAM_SECRET_KEY not set)")
	}
	...
	// (COMPOSIO_CALLBACK_BASE_URL, falling back to LUMEN_PUBLIC_URL). The
	...
	if composioAPIKey := strings.TrimSpace(os.Getenv("COMPOSIO_API_KEY")); composioAPIKey != "" {
		if !featureflags.ComposioMCPAppsEnabled(context.Background(), opts.FeatureFlags) {
			...
			if err != nil {
				...
				case callbackBase == "":
					slog.Error("composio: no callback base url (set COMPOSIO_CALLBACK_BASE_URL or LUMEN_PUBLIC_URL); composio integration disabled")
				default:
	...
	if vcsKey, err := secretbox.LoadKey("LUMEN_VCS_SECRET_KEY"); err == nil {
		...
		slog.Info("vcs integration disabled (LUMEN_VCS_SECRET_KEY not set)")
	}
	...
	if pluginKey, err := secretbox.LoadKey("LUMEN_PLUGIN_SECRET_KEY"); err == nil {
		...
		slog.Info("Plugin secrets disabled (LUMEN_PLUGIN_SECRET_KEY not set)")
	}
	...
	if h.PluginService != nil {
		...
		if baseURL := pluginActionBaseURL(signupConfig.PublicURL); baseURL != "" {
			...
			slog.Warn("plugins: LUMEN_PLUGIN_API_URL and LUMEN_PUBLIC_URL are not set; hook callbacks will carry no callback_url")
		}
	...
	// Share the same trusted-proxy CIDRs (LUMEN_TRUSTED_PROXIES) so the
	...
	// GitHub App webhook (no Lumen auth — requests are authenticated via
	// HMAC-SHA256 signature in the handler) and post-install setup callback.
	r.Post("/api/webhooks/github", h.HandleGitHubWebhook)
	r.Get("/api/github/setup", h.GitHubSetupCallback)
	// Slack OAuth callback (no Lumen auth in the path — it is hit by Slack's
	...
	// VCS webhook for token-based providers (Forgejo / Gitea / GitLab). No Lumen
	...
	// Stripe webhook (no Lumen auth — Stripe signs the raw body
	// with a shared secret, the lumen-cloud upstream verifies. We
	...
	r.Group(func(r chi.Router) {
		...
		// Lumen identity; the token only carries the WeCom userid to bind.
		...
		// cloud-runtime — lumen-cloud's Fleet and Billing share
...
// LUMEN_LARK_CALLBACK_BASE_URL), we log and fall back to the
...
func buildLarkConnector(installSvc *lark.InstallationService, apiClient lark.APIClient) (lark.EventConnector, string) {
	endpointFetcher, err := lark.NewHTTPConnectionTokenFetcher(lark.HTTPConnectionTokenConfig{
		BaseURL: strings.TrimSpace(os.Getenv("LUMEN_LARK_CALLBACK_BASE_URL")),
		Logger:  slog.Default(),
	})
	if err != nil {
	...
	if proxyURL := strings.TrimSpace(os.Getenv("LUMEN_LARK_WS_PROXY_URL")); proxyURL != "" {
...
// already-resolved LUMEN_PUBLIC_URL, then the app URL.
