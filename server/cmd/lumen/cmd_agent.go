...
// custom_env values — `lumen agent list / get / update` no longer
...
func init() {
	...
	// `lumen agent env set <id>` — that path admits the agent owner or a
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, LUMEN_SERVER_URL env, or 'lumen config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("LUMEN_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("LUMEN_TASK_ID"); taskID != "" {
...
const (
	defaultCloudServerURL = "https://api.lumen.ai"
	defaultCloudAppURL    = "https://lumen.ai"
)
...
// LUMEN_DAEMON_PORT in a host/container environment must not hide the human
...
func tryResolveExplicitServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "LUMEN_SERVER_URL", "")
	if val == "" {
		return ""
	}
...
func missingServerConfigMessage() string {
	return fmt.Sprintf("No server configured. Run 'lumen setup' first%s.", daemonPortOnlyContextHint())
}
...
func resolveHumanServerURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'lumen setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func inAgentExecutionContext() bool {
	return os.Getenv("LUMEN_AGENT_ID") != "" || os.Getenv("LUMEN_TASK_ID") != ""
}

// inDaemonManagedExecutionContext reports whether the CLI is being invoked
// from inside a daemon-managed agent task. LUMEN_DAEMON_PORT is included as
// a defense-in-depth marker for subprocesses that lose LUMEN_AGENT_ID or
// LUMEN_TASK_ID but still run under the daemon environment. In this context
// workspace and token must come from daemon-provided env; falling back to
// user-global ~/.lumen/config.json can make agent writes land as a member.
func inDaemonManagedExecutionContext() bool {
	return inAgentExecutionContext() || os.Getenv("LUMEN_DAEMON_PORT") != "" || hasDaemonTaskContextMarker()
}

// inDaemonTaskIdentityContext reports strong evidence that the current process
// belongs to a daemon-managed task. LUMEN_DAEMON_PORT is deliberately not
...
func daemonPortOnlyContextHint() string {
	if strings.TrimSpace(os.Getenv("LUMEN_DAEMON_PORT")) == "" || inDaemonTaskIdentityContext() {
		return ""
	}
	return "; LUMEN_DAEMON_PORT is set without task identity — if this is a host or container startup shell, remove that variable and retry"
}

// requireTaskLocalConfigRoot prevents daemon-managed subprocesses that lost
// part of their injected environment from silently resolving Lumen state
...
func requireTaskLocalConfigRoot() error {
	...
	if strings.TrimSpace(os.Getenv(cli.TaskConfigRootEnv)) == "" {
		...
		if markerPath := leftoverDaemonTaskMarkerPath(); markerPath != "" {
			return fmt.Errorf("daemon-managed task requires a task-local Lumen config root in %s%s", cli.TaskConfigRootEnv, leftoverMarkerSuffix(markerPath))
		}
		return fmt.Errorf("daemon-managed task requires a task-local Lumen config root in %s%s", cli.TaskConfigRootEnv, daemonPortOnlyContextHint())
	}
	return nil
}
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "LUMEN_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inDaemonManagedExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: LUMEN_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set LUMEN_WORKSPACE_ID env, or run 'lumen config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --conversation-starters, --runtime-id, --runtime-config, --model, --thinking-level, --service-tier, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `lumen agent env set <id>`)")
	}
