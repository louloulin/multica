...
func TestRunRuntimeDeleteProfileInstanceConflictShowsServerGuidance(t *testing.T) {
	...
	const guidance = `cannot delete "MSI-S3TEST" on its own: it is registered from the custom runtime profile "Devin CLI (WSL)". It is offline, and Lumen removes offline runtimes automatically after 7 days.`
