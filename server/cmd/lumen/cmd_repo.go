...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("LUMEN_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("LUMEN_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("LUMEN_WORKSPACE_ID")
	agentName := os.Getenv("LUMEN_AGENT_NAME")
	taskID := os.Getenv("LUMEN_TASK_ID")
	taskToken := os.Getenv("LUMEN_TOKEN")
	if taskToken == "" {
		return fmt.Errorf("LUMEN_TOKEN not set (repo checkout requires the active task credential)")
	}
	...
	reqBody := map[string]any{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("LUMEN_REPO_CHECKOUT_MODE")),
		"retry_busy":    true,
	...
	for {
		...
		if resp.StatusCode == http.StatusServiceUnavailable && resp.Header.Get("X-Lumen-Retryable") == "repo-busy" {
