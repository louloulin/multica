...
func TestRunConfigSetPersistsSupportedKeysInProfile(t *testing.T) {
	...
	workspacesRoot := filepath.Join(t.TempDir(), "lumen-dev")
...
func TestRunConfigCommandsFailClosedWithoutTaskRoot(t *testing.T) {
	...
	t.Setenv("LUMEN_AGENT_ID", "agent-test")
	t.Setenv("LUMEN_TASK_ID", "task-test")
	t.Setenv("LUMEN_TASK_CONFIG_ROOT", "")
...
func TestApplyConfigSetSupportsDaemonKeys(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	workspacesRoot := filepath.Join(t.TempDir(), "lumen")
...
func TestApplyConfigSetNormalizesWorkspacesRoot(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	if err := applyConfigSet(&cfg, "workspaces_root", filepath.Join("data", "lumen")); err != nil {
		t.Fatalf("applyConfigSet: %v", err)
	}
	want := filepath.Join(cwd, "data", "lumen")
...
func TestApplyConfigSetPositiveDurationRoundTripsToDaemonResolver(t *testing.T) {
	const envName = "TEST_LUMEN_PERSISTED_DURATION"
	t.Setenv(envName, "")
