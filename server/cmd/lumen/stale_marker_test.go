...
func TestLeftoverMarkerNotReportedUnderRealTaskIdentity(t *testing.T) {
	for _, tc := range []struct{ name, env, value string }{
		{name: "agent id", env: "LUMEN_AGENT_ID", value: "agent-1"},
		{name: "task id", env: "LUMEN_TASK_ID", value: "task-1"},
		{name: "task config root", env: cli.TaskConfigRootEnv, value: "/tmp/task-lumen"},
		{name: "daemon port", env: "LUMEN_DAEMON_PORT", value: "20032"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			// LUMEN_DAEMON_PORT alone is not task identity, so it does not by
