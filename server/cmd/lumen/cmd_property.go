...
import (
	...
	"github.com/lumen-ai/lumen/server/internal/issueproperty"
	db "github.com/lumen-ai/lumen/server/pkg/db/generated"
)

// lumen property {list|get|create|update|archive|unarchive} — workspace
// custom property definitions, and lumen issue property {list|set|unset} —
...
flags:
  lumen property create --name Severity --type select \
...
The actor types hold workspace members; they take no options:
  lumen property create --name Reviewer --type actor`,
	Args: exactArgs(0),
...
func (d *memberDirectory) load(ctx context.Context, client *cli.APIClient) ([]map[string]any, error) {
	...
	if client.WorkspaceID == "" {
		return nil, fmt.Errorf("workspace ID is required to resolve members; use --workspace-id or set LUMEN_WORKSPACE_ID")
	}
...
func buildPropertiesFilterQueryParam(ctx context.Context, client *cli.APIClient, directory *memberDirectory, properties []propertyDTO, pairs []string) (string, error) {
	...
	for _, pair := range pairs {
		...
		if property.Archived {
			return "", fmt.Errorf("property %q is archived; archived properties are hidden from filtering (matching the web UI) — restore it with `lumen property unarchive` if you need it", property.Name)
		}
...
func resolvePropertyFilterValue(ctx context.Context, client *cli.APIClient, directory *memberDirectory, property propertyDTO, raw string) (string, error) {
	...
	default: // a property type only a newer backend knows about
		return "", fmt.Errorf("--property %s: this CLI does not know how to filter %s properties; update with `lumen update`, or use the %s unset filter", property.Name, property.Type, propertyNoValueSentinel)
	}
}
...
func resolveSortableProperty(properties []propertyDTO, ref string) (propertyDTO, error) {
	...
	if property.Archived {
		return propertyDTO{}, fmt.Errorf("property %q is archived and the server would fall back to position order; restore it first with `lumen property unarchive`", property.Name)
	}
