...
// ~/.lumen/profiles/<name>/daemon.log while ~/.lumen/daemon.log may still
...
func TestDaemonLogSourcePathResolvesPerProfile(t *testing.T) {
	t.Run("default profile", func(t *testing.T) {
		...
		want := filepath.Join(home, ".lumen", "daemon.log")
		if got != want {
	...
	t.Run("named profile", func(t *testing.T) {
		home := mkProfiles(t, "desktop-api.lumen.ai")

		got, err := daemonLogSourcePath("desktop-api.lumen.ai")
		if err != nil {
			t.Fatalf("daemonLogSourcePath = %v, want nil", err)
		}
		want := filepath.Join(home, ".lumen", "profiles", "desktop-api.lumen.ai", "daemon.log")
		if got != want {
			t.Fatalf("daemonLogSourcePath = %q, want %q", got, want)
		}
	})
...
func TestRunDaemonLogsAnnouncesResolvedPath(t *testing.T) {
	cases := []struct {
		...
		{"named profile", "desktop-api.lumen.ai", "desktop-api.lumen.ai", 50, false},
		{"named profile following", "desktop-api.lumen.ai", "desktop-api.lumen.ai", 50, true},
	}
...
// The announcement goes to stderr, so `lumen daemon logs | grep ...` and a
...
func TestRunDaemonLogsMissingFileNamesProfilePath(t *testing.T) {
	...
	seedDaemonLog(t, filepath.Join(home, ".lumen", "daemon.log"))

	cmd, errOut := daemonLogsCmdFor(t, "desktop-api.lumen.ai", 50, false)
	rec := stubTailLog(t, errOut)
