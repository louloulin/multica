...
var rootCmd = &cobra.Command{
	Use:           "lumen",
	Short:         "Lumen CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Lumen from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("lumen {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Lumen server URL (env: LUMEN_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: LUMEN_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: LUMEN_DEBUG)")
