...
func TestFindLocalPathLinksAllowsLegitimateContent(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			body: "the PR is at [#42](https://github.com/lumen-ai/lumen/pull/42)",
		},
		{
			name: "mention link",
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No LUMEN_AGENT_ID / LUMEN_TASK_ID: a person running the CLI. Their
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	err := guardLocalPathLinks(
		"[screenshot]("+shot+")",
		"issue description",
		"`lumen issue update` cannot carry files — deliver the file with `lumen issue comment add <issue-id> --attachment <path>` instead, and drop the link.",
	)
	if err == nil {
		t.Fatal("expected a hard failure")
	}
	msg := err.Error()
	if !strings.Contains(msg, "`lumen issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "lumen issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "lumen issue update --attachment") {
