...
import (
	...
	logger_pkg "github.com/lumen-ai/lumen/server/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-run or per-workspace disk usage.\n" +
		...
		"root — the default root plus each ~/.lumen/profiles/* root, including the Desktop app's dedicated\n" +
		...
		"overridable via LUMEN_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: LUMEN_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: LUMEN_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: LUMEN_AGENT_RUNTIME_NAME)")
	f.String("workspaces-root", "", "Base directory for run workspaces (env: LUMEN_WORKSPACES_ROOT)")
	f.Duration("poll-interval", 0, "Run poll interval (env: LUMEN_DAEMON_POLL_INTERVAL)")
	f.Duration("ws-claim-poll-interval", 0, "Healthy WebSocket claim safety-poll upper bound (env: LUMEN_DAEMON_WS_CLAIM_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: LUMEN_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-run wall-clock cap; 0 = no cap, rely on the watchdogs (env: LUMEN_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: LUMEN_CODEX_HANDSHAKE_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Maximum concurrent runs (env: LUMEN_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: LUMEN_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: LUMEN_DAEMON_AUTO_UPDATE_INTERVAL)")
	f.Bool("no-auto-reload", false, "Disable restarting when the lumen binary on disk changes version (env: LUMEN_DAEMON_AUTO_RELOAD=false)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: LUMEN_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: LUMEN_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: LUMEN_AGENT_RUNTIME_NAME)")
	rf.String("workspaces-root", "", "Base directory for run workspaces (env: LUMEN_WORKSPACES_ROOT)")
	rf.Duration("poll-interval", 0, "Run poll interval (env: LUMEN_DAEMON_POLL_INTERVAL)")
	rf.Duration("ws-claim-poll-interval", 0, "Healthy WebSocket claim safety-poll upper bound (env: LUMEN_DAEMON_WS_CLAIM_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: LUMEN_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-run wall-clock cap; 0 = no cap, rely on the watchdogs (env: LUMEN_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: LUMEN_CODEX_HANDSHAKE_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Maximum concurrent runs (env: LUMEN_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: LUMEN_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: LUMEN_DAEMON_AUTO_UPDATE_INTERVAL)")
	rf.Bool("no-auto-reload", false, "Disable restarting when the lumen binary on disk changes version (env: LUMEN_DAEMON_AUTO_RELOAD=false)")
	...
	df.Bool("all-profiles", false, "Scan every workspace root (default root + all ~/.lumen/profiles/* roots, incl. the Desktop app's) and report a combined total")
...
// Empty profile → ~/.lumen/, named profile → ~/.lumen/profiles/<name>/.
...
func newDaemonLogRotator(logPath string) *lumberjack.Logger {
	return &lumberjack.Logger{
		Filename:   logPath,
		MaxSize:    envPositiveIntOrDefault("LUMEN_DAEMON_LOG_MAX_SIZE_MB", defaultDaemonLogMaxSizeMB),
		MaxBackups: envPositiveIntOrDefault("LUMEN_DAEMON_LOG_MAX_BACKUPS", defaultDaemonLogMaxBackups),
		MaxAge:     envPositiveIntOrDefault("LUMEN_DAEMON_LOG_MAX_AGE_DAYS", defaultDaemonLogMaxAgeDays),
		Compress:   true,
	}
}
...
// directory under ~/.lumen/profiles. It carries the known profile names so
...
func (e *unknownProfileError) Error() string {
	if len(e.Known) == 0 {
		...
		return fmt.Sprintf("unknown profile %q: no named profiles exist yet.\nCreate one with: lumen login --profile %s", e.Profile, shellQuoteArg(e.Profile))
	}
...
// may contain separators: `lumen --profile team/dev config set ...` creates
// ~/.lumen/profiles/team/dev. Matching against top-level entries would
...
// one-glance fix.
//
// The default profile is never validated: it owns ~/.lumen directly, has no
...
// requireDaemonAuth fails fast when the user never ran `lumen login`. The
...
// authenticates via the stored config token, never LUMEN_TOKEN.
func requireDaemonAuth(profile string) error {
	...
	if cfg.Token == "" {
		loginHint := "lumen login"
		if profile != "" {
			loginHint = fmt.Sprintf("lumen login --profile %s", profile)
		}
...
// LUMEN_SERVER_URL env, then the stored CLI config for the profile.
func resolveDaemonServerURL(cmd *cobra.Command, profile string) string {
	serverURL := cli.FlagOrEnv(cmd, "server-url", "LUMEN_SERVER_URL", "")
	if serverURL == "" {
...
func daemonStartupFailureError(logs daemonStartupLogs, waitErr error, profile, serverURL string) error {
	...
	case strings.Contains(joined, "connection refused") ||
		...
		target := "the Lumen server"
		if serverURL != "" {
			target += " at " + serverURL
		}
		return fmt.Errorf("daemon failed to start: cannot reach %s.\nMake sure the server is running and reachable, then rerun 'lumen daemon start'.\nFull log: %s", target, logs.logPath)
	}
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment LUMEN_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	//
	//   --flag  >  LUMEN_… env  >  config.json  >  built-in default
	//
	...
	pollOverride, err := resolveDaemonDurationOverride(pollFlag, "LUMEN_DAEMON_POLL_INTERVAL", fileCfg.PollInterval)
	if err != nil {
		return err
	}
	if pollOverride > 0 {
	...
	wsClaimPollOverride, err := resolveDaemonDurationOverride(wsClaimPollFlag, "LUMEN_DAEMON_WS_CLAIM_POLL_INTERVAL", fileCfg.WSClaimPollInterval)
	if err != nil {
		return err
	}
	...
	heartbeatOverride, err := resolveDaemonDurationOverride(heartbeatFlag, "LUMEN_DAEMON_HEARTBEAT_INTERVAL", fileCfg.HeartbeatInterval)
	if err != nil {
		return err
	}
	...
	agentTimeoutOverride, err := resolveDaemonAgentTimeoutOverride(cmd, "LUMEN_AGENT_TIMEOUT", fileCfg.AgentTimeout)
	if err != nil {
		return err
	}
	...
	semanticOverride, err := resolveDaemonDurationOverride(semanticFlag, "LUMEN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", fileCfg.CodexSemanticInactivityTimeout)
	if err != nil {
		return err
	}
	...
	handshakeOverride, err := resolveDaemonDurationOverride(handshakeFlag, "LUMEN_CODEX_HANDSHAKE_TIMEOUT", fileCfg.CodexHandshakeTimeout)
	if err != nil {
		return err
	}
	...
	if n := resolveDaemonIntOverride(maxFlag, "LUMEN_DAEMON_MAX_CONCURRENT_TASKS", fileCfg.MaxConcurrentTasks); n > 0 {
		overrides.MaxConcurrentTasks = n
	}
	// --no-auto-update / LUMEN_DAEMON_AUTO_UPDATE=false / config.json
	...
	if resolveDaemonDisableSignal(noAutoUpdateFlag, "LUMEN_DAEMON_AUTO_UPDATE", fileCfg.DisableAutoUpdate) {
	...
	if resolveDaemonDisableSignal(noAutoReloadFlag, "LUMEN_DAEMON_AUTO_RELOAD", fileCfg.DisableAutoReload) {
		overrides.DisableAutoReload = true
	}
	autoUpdateFlag, _ := cmd.Flags().GetDuration("auto-update-interval")
	autoUpdateOverride, err := resolveDaemonDurationOverride(autoUpdateFlag, "LUMEN_DAEMON_AUTO_UPDATE_INTERVAL", fileCfg.AutoUpdateCheckInterval)
	if err != nil {
		return err
	}
	...
	cfg.LaunchedBy = os.Getenv("LUMEN_LAUNCHED_BY")
...
// `lumen auth status`) and refuse to touch the running daemon on failure.
func requireDaemonRestartPreflight(cmd *cobra.Command, profile string) error {
	...
	if err := cli.NewAPIClient(baseURL, "", cfg.Token).GetJSON(ctx, "/api/me", nil); err != nil {
		...
		return fmt.Errorf("refusing to restart: cannot reach the Lumen server at %s (%w); the running daemon was left untouched.\nMake sure the server is running and reachable, then rerun 'lumen daemon restart'", baseURL, err)
	}
	return nil
}
...
// LUMEN_DAEMON_PORT remains in the host environment. Inside a managed task it
...
func daemonStatusHealthPort(cmd *cobra.Command) (int, error) {
	...
	raw := strings.TrimSpace(os.Getenv("LUMEN_DAEMON_PORT"))
	if raw == "" {
		return 0, fmt.Errorf("daemon status inside a daemon-managed task requires the daemon-injected LUMEN_DAEMON_PORT")
	}
	port, err := strconv.Atoi(raw)
	if err != nil || port <= 0 {
		return 0, fmt.Errorf("invalid LUMEN_DAEMON_PORT %q inside a daemon-managed task", raw)
	}
	return port, nil
}
...
func describeDaemonManager(launchedBy string) string {
	if launchedBy == "desktop" {
		return "Lumen Desktop app (start and stop it from the app)"
	}
	return launchedBy
}
...
// ~/.lumen/daemon.log for the default profile,
// ~/.lumen/profiles/<name>/daemon.log for a named one — and guarantees it is
...
//  2. LUMEN_AGENT_TIMEOUT is set -> return nil so LoadConfig reads the
...
//  2. LUMEN_DAEMON_AUTO_UPDATE explicitly set to a falsy value ->
...
// boundary: --all-profiles enumerates ~/.lumen/profiles and discloses the
...
// each ~/.lumen/profiles/* root) and renders a per-root breakdown with a
...
// orientation even when empty), then each ~/.lumen/profiles/* root that
// exists on disk, sorted by profile name. Roots that resolve to the same path
// (e.g. when LUMEN_WORKSPACES_ROOT pins every profile to one directory) are
...
func printRepoCacheLine(w io.Writer, report daemon.DiskUsageReport) {
	...
	fmt.Fprintf(w, "Repo cache (.repos): %s across %d repo(s), not included above. Evicted once a repo is unused for LUMEN_GC_REPO_TTL and no longer attached to a workspace.\n",
...
func printDiskUsageOtherRootsHint(w io.Writer, report daemon.DiskUsageReport, profile, rootOverride string, taskContext bool) {
	...
	// The suggestions are built by listing ~/.lumen/profiles, so printing
	...
	fmt.Fprintln(w, "Run 'lumen daemon disk-usage --all-profiles' for a combined total across all roots.")
}
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := resolveWorkspacesRootForProfile("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "lumen daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "lumen --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".lumen", "profiles"), nil
}
