...
const blockingChildEnv = "LUMEN_TEST_BLOCKING_CHILD"
...
// carries no platform build tag, so `go test ./cmd/lumen` has to pass there
...
func TestDaemonStatusInTaskContextReportsNamedProfileHost(t *testing.T) {
	setup := func(t *testing.T) {
		...
		t.Setenv("LUMEN_DAEMON_PORT", strconv.Itoa(stub.port))
	}
