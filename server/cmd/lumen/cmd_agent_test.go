...
func TestHumanLocalCommandDistinguishesPortHintFromTaskIdentity(t *testing.T) {
	...
	t.Setenv(cli.TaskConfigRootEnv, filepath.Join(t.TempDir(), "task-lumen"))
...
func TestHumanLocalCommandRejectsExplicitTaskIdentity(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "agent ID blocks setup", envName: "LUMEN_AGENT_ID", command: "setup"},
		{name: "task ID blocks daemon stop", envName: "LUMEN_TASK_ID", command: "daemon stop"},
	} {
...
func TestMissingServerConfigMessageExplainsPortOnlyContext(t *testing.T) {
	...
	if !strings.Contains(message, "LUMEN_DAEMON_PORT") || !strings.Contains(message, "remove") {
...
// impersonation escape: a sandbox fault strips every LUMEN_* env var from
// an agent subprocess, which then runs `lumen` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.lumen marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("LUMEN_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (LUMEN_AGENT_ID / LUMEN_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.lumen/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		...
		t.Setenv("LUMEN_DAEMON_PORT", "")
		t.Setenv("LUMEN_WORKSPACE_ID", "env-ws")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without LUMEN_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without LUMEN_TOKEN", got)
		}
	})
	...
	// LUMEN_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// LUMEN_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("LUMEN_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("LUMEN_AGENT_ID", "")
		t.Setenv("LUMEN_TASK_ID", "")
		t.Setenv("LUMEN_TOKEN", "")
		t.Setenv("LUMEN_DAEMON_PORT", "")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("LUMEN_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("LUMEN_WORKSPACE_ID", "workspace-123")
	t.Setenv("LUMEN_AGENT_ID", "agent-123")
	t.Setenv("LUMEN_TASK_ID", "task-456")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	...
	t.Setenv("LUMEN_WORKSPACE_ID", "test-ws")
	...
	msg := err.Error()
	if !strings.Contains(msg, "lumen agent env set") {
		t.Fatalf("no-fields error must direct users to `lumen agent env set`; got: %q", msg)
	}
}
