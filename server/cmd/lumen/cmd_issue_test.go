...
func TestRunIssueCreateSendsCanonicalIDKeyedProperties(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/issues":
			...
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "LUM-1", "title": "With properties",
...
func TestRunIssueCreateSendsAllowDuplicate(t *testing.T) {
	...
	t.Setenv("LUMEN_SERVER_URL", srv.URL)
	t.Setenv("LUMEN_WORKSPACE_ID", "ws-1")
	t.Setenv("LUMEN_TOKEN", "test-token")
...
func TestRunIssueCreateSendsExistingAttachmentIDs(t *testing.T) {
	...
	t.Setenv("LUMEN_WORKSPACE_ID", "ws-1")
	t.Setenv("LUMEN_TOKEN", "test-token")
	t.Setenv("LUMEN_QUICK_CREATE_ATTACHMENT_IDS", `["att-env","att-shared"]`)
...
func TestRunIssuePullRequestsListsLinkedPRsAsJSON(t *testing.T) {
		...
		case "/api/issues/issue-uuid/pull-requests":
			json.NewEncoder(w).Encode(map[string]any{
				"pull_requests": []map[string]any{
					{
						"url":    "https://github.com/lumen-ai/lumen/pull/42",
						"number": float64(42),
						"state":  "open",
	...
	if pr["url"] != "https://github.com/lumen-ai/lumen/pull/42" || pr["number"] != float64(42) || pr["state"] != "open" || pr["title"] != "MUL-2818 add issue PR CLI" {
...
func TestRunIssuePullRequestsTableIncludesCoreFields(t *testing.T) {
	...
	out := <-drainCh
	text := string(out)
	for _, want := range []string{"NUMBER", "STATE", "TITLE", "URL", "42", "open", "MUL-2818 add issue PR CLI", "https://github.com/lumen-ai/lumen/pull/42"} {
...
func TestResolveIssueRef(t *testing.T) {
	...
	t.Run("full UUID resolves locally with zero HTTP requests", func(t *testing.T) {
		...
		// agents pay that per `lumen issue …` call during run
...
// lumen-ai/lumen#1620: when one name is a substring of another (e.g.
...
// lumen-ai/lumen#1620: passing a full UUID or its 8-char prefix must
...
// `lumen issue subscriber add --user "<SquadName>"` or
// `lumen project create --lead "<SquadName>"` would resolve to
...
func TestRunIssueCommentListFlagGuards(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.setup(cmd)
			err := runIssueCommentList(cmd, []string{"LUM-1"})
			if err == nil {
...
func TestRunIssueCommentList_RootsOnlyPassesThroughWithSince(t *testing.T) {
	...
	if err := runIssueCommentList(cmd, []string{"LUM-1"}); err != nil {
...
func TestRunIssueCommentList_ThreadTailPassesThroughAndPrintsReplyCursor(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/comments") {
			...
			w.Header().Set("X-Lumen-Next-Before", "2026-01-01T00:00:00.000000001Z")
			w.Header().Set("X-Lumen-Next-Before-Id", "00000000-0000-0000-0000-000000000999")
			w.Write([]byte("[]"))
			return
		}
		t.Errorf("unexpected request: %s %s", r.Method, r.URL.String())
...
func TestRunIssueCommentList_RecentStillLabelsCursorAsThread(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Lumen-Next-Before", "2026-01-01T00:00:00.000000001Z")
		w.Header().Set("X-Lumen-Next-Before-Id", "00000000-0000-0000-0000-000000000777")
		w.Write([]byte("[]"))
	}))
	defer srv.Close()
...
func TestRunIssueUpdateRejectsInvalidStatusBeforeRequest(t *testing.T) {
	...
	cmd.Flags().String("priority", "", "")
	_ = cmd.Flags().Set("status", "not a status")
	err := runIssueUpdate(cmd, []string{"LUM-1"})
	if err == nil {
...
func TestRunIssueUpdateSendsPosition(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/LUM-1":
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "LUM-1", "status": "todo",
			})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			...
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "LUM-1", "status": "todo", "priority": "none",
			})
		default:
			http.NotFound(w, r)
		}
	...
	if err := runIssueUpdate(cmd, []string{"LUM-1"}); err != nil {
...
func TestRunIssueUpdateNoStartSendsSuppressRun(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/LUM-1":
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "LUM-1", "status": "todo"})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Errorf("decode body: %v", err)
			}
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "LUM-1", "status": "in_progress"})
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestRunIssueStatusNoStartSendsSuppressRun(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/LUM-1":
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "LUM-1", "status": "backlog"})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Errorf("decode body: %v", err)
			}
	...
	_ = cmd.Flags().Set("no-start", "true")
	if err := runIssueStatus(cmd, []string{"LUM-1", "in_progress"}); err != nil {
...
func TestRunIssueAssignNoStartSendsSuppressRun(t *testing.T) {
	...
	_ = cmd.Flags().Set("no-start", "true")
	if err := runIssueAssign(cmd, []string{"LUM-1"}); err != nil {
...
func TestRunIssueAssignRejectsNoStartWithUnassign(t *testing.T) {
	...
	_ = cmd.Flags().Set("no-start", "true")
	err := runIssueAssign(cmd, []string{"LUM-1"})
...
func TestIssueReadCommandsUseInjectedTaskToken(t *testing.T) {
	...
	t.Setenv("LUMEN_TOKEN", fakeTaskToken)
	t.Setenv("LUMEN_WORKSPACE_ID", "ws-task")
	t.Setenv("LUMEN_TASK_CONFIG_ROOT", t.TempDir())

	ownerPath := filepath.Join(ownerHome, ".lumen", "config.json")
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/issues/LUM-46":
			_ = json.NewEncoder(w).Encode(map[string]any{"id": "issue-uuid", "identifier": "LUM-46", "title": "Task isolation"})
	...
	if _, err := captureStdout(t, func() error { return runIssueGet(getCmd, []string{"LUM-46"}) }); err != nil {
	...
	if _, err := captureStdout(t, func() error { return runIssueRuns(runsCmd, []string{"LUM-46"}) }); err != nil {
...
func TestIssueReadCommandsFailClosedWithoutTaskToken(t *testing.T) {
	...
	t.Setenv("LUMEN_SERVER_URL", "http://127.0.0.1:1")
	t.Setenv("LUMEN_WORKSPACE_ID", "ws-task")
	t.Setenv("LUMEN_TASK_CONFIG_ROOT", t.TempDir())
...
func TestRunIssueListRejectsInvalidSortAndDirection(t *testing.T) {
	t.Setenv("LUMEN_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("LUMEN_WORKSPACE_ID", "ws-1")
	t.Setenv("LUMEN_TOKEN", "test-token")
...
func TestIssueReorderTargetFlagGroup(t *testing.T) {
	...
	t.Run("two bool targets", func(t *testing.T) {
		cmd := newCmd()
		cmd.SetArgs([]string{"LUM-1", "--top", "--bottom"})
		err := cmd.Execute()
		if err == nil {
	...
	t.Run("before and after together", func(t *testing.T) {
		cmd := newCmd()
		cmd.SetArgs([]string{"LUM-1", "--before", "LUM-2", "--after", "LUM-3"})
		err := cmd.Execute()
		if err == nil {
...
func TestRunIssueReorderRejectsNoOpTargetValues(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.flag, func(t *testing.T) {
			...
			err := runIssueReorder(cmd, []string{"LUM-1"})
			if err == nil {
...
func reorderTestServer(t *testing.T, gotPosition *float64) *httptest.Server {
	...
	a := issue("a-id", "LUM-1", "todo", 1)
	b := issue("b-id", "LUM-2", "todo", 9)
	target := issue("t-id", "LUM-9", "todo", 20)
	c := issue("c-id", "LUM-3", "in_progress", 4)
	byRef := map[string]map[string]any{
		"LUM-1": a, "a-id": a,
		"LUM-2": b, "b-id": b,
		"LUM-9": target, "t-id": target,
		"LUM-3": c, "c-id": c,
	}
...
func TestRunIssueReorderComputesPosition(t *testing.T) {
	tests := []struct {
		...
		{"before another issue", "before", "LUM-2", 5}, // midpoint(a=1, b=9)
		{"after another issue", "after", "LUM-2", 10},  // moves below b (the bottom)
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := runIssueReorder(cmd, []string{"LUM-9"}); err != nil {
...
func TestRunIssueReorderRejectsCrossColumnTarget(t *testing.T) {
	...
	cmd := newIssueReorderTestCmd()
	_ = cmd.Flags().Set("before", "LUM-3") // LUM-3 lives in the in_progress column
	err := runIssueReorder(cmd, []string{"LUM-9"})
	if err == nil {
...
func TestReorderTargetNotInColumnErrorUsesFetchedIssueKey(t *testing.T) {
	...
	if msg := err.Error(); !strings.Contains(msg, "issue LUM-3") || !strings.Contains(msg, "LUM-9") {
...
func TestFetchIssueColumnPaginates(t *testing.T) {
	all := []map[string]any{
		mkIssue("i1", "LUM-1", "todo", 1),
		mkIssue("i2", "LUM-2", "todo", 2),
		mkIssue("i3", "LUM-3", "todo", 3),
	}
...
func TestRunIssueReorderNoOpSkipsPut(t *testing.T) {
	...
	a := mkIssue("a-id", "LUM-1", "todo", 0)
	b := mkIssue("b-id", "LUM-2", "todo", 10)
	target := mkIssue("t-id", "LUM-9", "todo", 5)
	byRef := map[string]map[string]any{
		"LUM-1": a, "a-id": a,
		"LUM-2": b, "b-id": b,
		"LUM-9": target, "t-id": target,
...
func TestRunIssueReorderSingleItemColumnValidatesTarget(t *testing.T) {
	// The "todo" column holds only the issue being moved; LUM-3 lives in a
	...
	target := mkIssue("t-id", "LUM-9", "todo", 5)
	other := mkIssue("c-id", "LUM-3", "in_progress", 2)
	byRef := map[string]map[string]any{
		"LUM-9": target, "t-id": target,
		"LUM-3": other, "c-id": other,
	}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/issues" && r.Method == http.MethodGet {
...
func TestRunIssueUpdateOmitsPositionWhenUnset(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			...
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "LUM-1", "title": "Renamed"})
		default:
			http.NotFound(w, r)
		}
	}))
...
// `lumen issue children --output json`. Since MUL-6243 a workspace can define
...
func fakeIssueRows(n int) []map[string]any {
	...
	for i := 0; i < n; i++ {
		rows = append(rows, map[string]any{
			...
			"identifier": fmt.Sprintf("LUM-%d", i+1),
