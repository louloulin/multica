...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("LUMEN_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.lumen.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("LUMEN_SERVER_URL", srv.URL)
	t.Setenv("LUMEN_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".lumen", "profiles", "staging", "config.json")
...
func TestFetchWorkspacesExplainsPortOnlyFailClosedContext(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "LUMEN_DAEMON_PORT") || !strings.Contains(err.Error(), "remove") {
