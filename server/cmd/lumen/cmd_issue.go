...
var issueReorderCmd = &cobra.Command{
	...
	Long: "Reposition an issue inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `lumen issue status`,\n" +
...
var issueCommentUpdateCmd = &cobra.Command{
	...
	Long: "Update a comment you authored. Workspace owners and admins can update any comment.\n\n" +
		"Pass the revision returned by `lumen issue comment list <issue-id> --output json`; " +
...
func init() {
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Lumen-Next-Before response header, printed on stderr as \"Next thread cursor\" / \"Next reply cursor\"; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Lumen-Next-Before-Id response header; must be paired with --before.")
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("LUMEN_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse LUMEN_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if hasDesc {
		if err := guardLocalPathLinks(desc, "issue description",
			"Deliver the file itself with `lumen issue create --attachment <path>` (repeatable) and drop the link."); err != nil {
			return err
		}
	...
	// Quick-create stamp: when the daemon sets LUMEN_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `lumen issue create` call
	...
	if taskID := os.Getenv("LUMEN_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func runIssueUpdate(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("description") || cmd.Flags().Changed("description-stdin") || cmd.Flags().Changed("description-file") {
		...
		if err := guardLocalPathLinks(desc, "issue description",
			"`lumen issue update` cannot carry files — deliver the file with `lumen issue comment add <issue-id> --attachment <path>` instead, and drop the link."); err != nil {
			return err
		}
		body["description"] = desc
	}
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef resolvedID, issueDisplay, status string) error {
	...
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		...
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `lumen issue status` first, or pick a target in the same column", otherDisplay, otherStatus, issueDisplay, status)
		}
	}
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-Lumen-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Lumen-Next-Before-Id"); nbid != "" {
...
func runIssueCommentAdd(cmd *cobra.Command, args []string) error {
	...
	if err := guardLocalPathLinks(content, "comment body",
		"Deliver the file itself with `lumen issue comment add <issue-id> --attachment <path>` (repeatable) and drop the link."); err != nil {
		return err
	}
...
func runIssueCommentUpdate(cmd *cobra.Command, args []string) error {
	...
	if !cmd.Flags().Changed("expected-revision") || expectedRevision < 1 {
		return fmt.Errorf("--expected-revision is required and must be a positive integer; read the current revision with `lumen issue comment list <issue-id> --output json`")
	}
	...
	if err := guardLocalPathLinks(content, "comment body",
		"Comment updates cannot attach files; deliver the file in a new comment with `lumen issue comment add <issue-id> --attachment <path>` and drop the link."); err != nil {
		return err
	}
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set LUMEN_WORKSPACE_ID")
	}
...
// from `lumen workspace member list --output json`) should use this instead of
