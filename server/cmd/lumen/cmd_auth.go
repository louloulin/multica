...
// loginTokenPrefixes are the token prefixes `lumen login --token` accepts.
// The CLI used to hardcode `lum_` only, which made it impossible to log in
// with a Lumen Cloud Node PAT (`mcn_`) even though the server happily
// authenticates both kinds. Keep this list in sync with the prefix branches
// in server/internal/middleware/auth.go.
var loginTokenPrefixes = []string{"lum_", auth.CloudPATPrefix}
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate lumen with Lumen",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("LUMEN_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the LUMEN_DAEMON_PORT
	// signal for subprocesses that lost LUMEN_AGENT_ID / LUMEN_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"LUMEN_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'lumen setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLogin(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("token") {
		...
		// `--token lum_xxx` (space form) is what users actually type — that's
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `lumen login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `lumen login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `lumen login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if taskContext && !strings.HasPrefix(token, "lat_") {
		return fmt.Errorf("agent execution context requires LUMEN_TOKEN to be a task-scoped lat_ token")
	}
	serverURL := resolveServerURL(cmd)

	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'lumen login' to authenticate.")
		return nil
	}
...
<title>Lumen — Authenticated</title>
<style>
