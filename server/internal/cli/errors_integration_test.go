...
func TestHelperStatusErrorsAreClassified(t *testing.T) {
	...
	statusCases := []struct {
		...
		{http.StatusUnauthorized, ExitAuth, "lumen login"},
...
// context that truncated a longer LUMEN_HTTP_TIMEOUT.
func TestCommandContextNotTruncatedBelowHTTPTimeout(t *testing.T) {
	...
	t.Setenv("LUMEN_HTTP_TIMEOUT", "400ms")
...
// LUMEN_HTTP_TIMEOUT and the AtLeast floor.
func TestAPITimeoutRespectsEnv(t *testing.T) {
	...
	t.Run("AtLeast floor wins when larger", func(t *testing.T) {
		t.Setenv("LUMEN_HTTP_TIMEOUT", "10s")
	...
	t.Run("AtLeast budget wins when larger than floor", func(t *testing.T) {
		t.Setenv("LUMEN_HTTP_TIMEOUT", "120s")
	...
	// responsive, but it must still honor LUMEN_HTTP_TIMEOUT (it is not a
