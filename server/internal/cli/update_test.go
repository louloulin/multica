...
func TestFindReleaseAsset(t *testing.T) {
	t.Run("prefers versioned asset when both names exist", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "lumen_darwin_amd64.tar.gz", BrowserDownloadURL: "old"},
			{Name: "lumen-cli-1.2.3-darwin-amd64.tar.gz", BrowserDownloadURL: "new"},
		}
		...
		if got.Name != "lumen-cli-1.2.3-darwin-amd64.tar.gz" {
	...
	t.Run("falls back to legacy asset when versioned is absent", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "lumen_linux_amd64.tar.gz", BrowserDownloadURL: "old"},
		}
		...
		if got.Name != "lumen_linux_amd64.tar.gz" {
			t.Fatalf("asset mismatch: got %q", got.Name)
		}
	})
...
func TestFindChecksumManifestAsset(t *testing.T) {
	t.Run("finds checksums.txt among assets", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "lumen-cli-1.2.3-darwin-arm64.tar.gz"},
			{Name: "checksums.txt", BrowserDownloadURL: "https://example/checksums.txt"},
			{Name: "lumen-cli-1.2.3-linux-amd64.tar.gz"},
		}
...
func TestParseChecksumManifest(t *testing.T) {
	manifest := []byte(strings.Join([]string{
		...
		"",
		"aaaa1111  lumen-cli-1.2.3-darwin-arm64.tar.gz",
		"bbbb2222  lumen-cli-1.2.3-darwin-amd64.tar.gz",
		"cccc3333\tmulti-tab-separator.tar.gz",
		"DDDD4444  lumen_linux_amd64.tar.gz",
	}, "\n"))

	t.Run("returns lowercase sha for matched entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "lumen-cli-1.2.3-darwin-arm64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "aaaa1111" {
	...
	t.Run("downcases an uppercase entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "lumen_linux_amd64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "dddd4444" {
	...
	t.Run("skips blank lines and comments", func(t *testing.T) {
		...
		if _, err := parseChecksumManifest(manifest, "lumen-cli-1.2.3-darwin-arm64.tar.gz"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
