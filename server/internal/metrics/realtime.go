...
func NewRealtimeCollector(m *realtime.Metrics) *RealtimeCollector {
	return &RealtimeCollector{
		...
		redisMirrorErrors:      prometheus.NewDesc("lumen_realtime_redis_mirror_errors_total", "Total Redis mirror write errors by the realtime relay.", []string{"target"}, nil),
		...
		redisStreamEntries:     prometheus.NewDesc("lumen_realtime_redis_stream_entries", "Current entry count of a sampled relay stream.", []string{"stream"}, nil),
		redisStreamPTTL:        prometheus.NewDesc("lumen_realtime_redis_stream_pttl_milliseconds", "Remaining relay stream TTL in milliseconds; -1 means no TTL and -2 means missing.", []string{"stream"}, nil),
	}
}

func newRealtimeDesc(name, help string) *prometheus.Desc {
	return prometheus.NewDesc("lumen_realtime_"+name, help, nil, nil)
}
