...
func TestResolveWithFallsBackToArgv0(t *testing.T) {
	...
	t.Run("absolute path", func(t *testing.T) {
		want := writeTestExecutable(t, t.TempDir(), "lumen")
	...
	t.Run("relative path", func(t *testing.T) {
		dir := t.TempDir()
		want := writeTestExecutable(t, dir, "lumen")
		t.Chdir(dir)
	...
	t.Run("PATH command", func(t *testing.T) {
		...
		got, err := resolveWith(failExecutable, []string{"lumen"})
		if err != nil {
			t.Fatalf("resolveWith() error = %v", err)
		}
		if got != want {
...
func TestResolveWithRejectsInvalidFallback(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing executable", args: []string{"lumen-does-not-exist"}, want: "lumen-does-not-exist"},
	}
	...
	if runtime.GOOS != "windows" {
		t.Run("non-executable file", func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "lumen")
