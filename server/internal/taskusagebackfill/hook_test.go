...
func TestHook_DirectV034Upgrade(t *testing.T) {
	...
	if adminURL == "" {
		adminURL = "postgres://lumen:lumen@localhost:5432/lumen?sslmode=disable"
	}
	...
	tmpDB := fmt.Sprintf("lumen_v034_upgrade_%d", time.Now().UnixNano())
...
func TestHook_FreshDatabaseStampsWatermarkOnly(t *testing.T) {
	...
	tmpDB := fmt.Sprintf("lumen_v034_fresh_%d", time.Now().UnixNano())
	if err := createDatabase(ctx, adminURL, tmpDB); err != nil {
