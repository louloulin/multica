...
description: "Use when a product-authored kickoff starts or resumes Mika's interactive onboarding for a Lumen workspace. The opening greeting has already been sent; carry the member from their first message to one real, confirmed, issue-based execution and a clear handoff."
...
- Do not greet the member again, introduce yourself again, or restate what
  Lumen is. They just read all of it.
...
- **Digest** — "Set up a daily automation that posts a morning summary of
  ...
  - The profile block carries `Member IANA timezone`. When it holds a zone,
    ...
    `lumen autopilot trigger-add --timezone <IANA>`.
