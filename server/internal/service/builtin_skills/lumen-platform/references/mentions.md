...
- a person → `lumen workspace member list --output json` → use `user_id`
- an agent → `lumen agent list --output json` → use `id`
- a squad  → `lumen squad list --output json` → use `id`
...
- **A hex-ish but wrong UUID.** A well-formed-looking UUID that no entity owns
  ...
  **So when you see `invocation_not_allowed`, check the UUID against the live
  ...
  `lumen squad member list <squad-id> --output json` returns the `member_id`
...
Correct:
  1. `lumen workspace member list --output json`  → Alice's `user_id` = 7f3a…
