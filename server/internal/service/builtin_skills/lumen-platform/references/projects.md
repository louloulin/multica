...
lumen project get <project-id> --output json
...
`.lumen/project/resources.json` as `project_description`. Use it for
...
lumen project list --output json
lumen project get <project-id> --output json
lumen project create --title "<title>" --repo <github-url> --output json
lumen project create --title "<title>" --start-date 2026-03-01 --due-date 2026-03-31 --output json
lumen project update <project-id> --title "<title>" --output json
lumen project update <project-id> --due-date 2026-04-15 --output json
lumen project update <project-id> --start-date "" --output json   # clear the start date
lumen project status <project-id> in_progress --output json
lumen project resource list <project-id> --output json
...
lumen project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --output json
lumen project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --execution-mode worktree --output json
lumen project resource update <project-id> <resource-id> --execution-mode in_place --output json
lumen project resource update <project-id> <resource-id> --url <new-github-url> --output json
lumen project resource update <project-id> <resource-id> --ref <branch-or-sha> --output json
lumen project resource remove <project-id> <resource-id> --output json
```
...
(`refs/lumen/local-state/<branch>`, which holds the owning conversation, the
...
advertise the capability — the fix is updating the Lumen app there, then
...
Project resources are durable and affect future tasks. `lumen repo checkout`
is task-local checkout state.

## Debugging wrong context

1. `lumen project get <project-id> --output json`.
2. `lumen project resource list <project-id> --output json`.
