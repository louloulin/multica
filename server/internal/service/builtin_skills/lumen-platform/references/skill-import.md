...
import a specific skill into the current Lumen workspace. It does not help you
...
A skill is installed for Lumen only when it exists in the current workspace's
...
```bash
lumen skill import --url <url> --output json              # hosted source
lumen skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Lumen workspace DB, so Lumen cannot manage or bind it.

## Supported URL source families

The import endpoint accepts these hosts (and `www.` variants). Pass any of these
forms to `lumen skill import --url <url> --output json`:

```bash
lumen skill import --url clawhub.ai/owner/skill --output json
lumen skill import --url skills.sh/owner/repo/skill --output json
lumen skill import --url github.com/owner/repo --output json
lumen skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
lumen skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`lumen skill import --file <path> --output json` imports a skill from a local
...
After the final `lumen agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`lumen agent skills add` is additive: the server inserts the assignments
without clearing existing ones.

`lumen agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `lumen skill import --url <url>` is equivalent to
...
lumen skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
lumen skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
lumen skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
lumen skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
lumen skill get <skill-id> --output json
```
...
```bash
lumen skill refresh <skill-id> --output json
```
...
`lumen skill get --with-content`), not the import result envelope. Failure
...
`lumen skill get` and `lumen skill files list` return **metadata only** —
...
```bash
lumen skill get <skill-id> --output json                  # metadata
lumen skill get <skill-id> --with-content --output json   # bodies inlined
lumen skill files list <skill-id>                         # paths and sizes
```
...
Incorrect (bypasses Lumen):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Lumen cannot manage it as a workspace skill.
...
```bash
lumen skill import --url https://skills.sh/owner/repo/skill --output json
```
