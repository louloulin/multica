...
The contract for Lumen's agent-creation path: what the create entry points
...
```bash
lumen agent get <agent-id> --output json      # full persisted agent record
lumen agent skills list <agent-id> --output json   # current skill bindings
lumen agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
(`lumen agent create`). At task claim time the daemon re-reads the agent and
...
`lumen agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
lumen agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
lumen agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
lumen agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
lumen agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`lumen agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`lumen agent env set`), which carries the same
...
```bash
lumen agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
lumen agent update <agent-id> --mcp-config-stdin --output json
lumen agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
`lumen workspace mcp list|add|update|remove`). Adding one there gives it to
NO agent — same shape as a workspace skill. It reaches an agent only when
someone assigns it:

```bash
lumen workspace mcp list --output table        # find the server id
lumen agent mcp add <agent-id> <server-id>     # give it to one agent
lumen agent mcp disable <agent-id> <server-id> # stop sending it, keep the assignment
lumen agent mcp remove <agent-id> <server-id>  # take it away
```
...
- `lumen agent create` — creates a new agent.
- `lumen agent copy` — creates a new agent (a fork of an existing one); the
  source is left untouched.
- `lumen agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `lumen agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
