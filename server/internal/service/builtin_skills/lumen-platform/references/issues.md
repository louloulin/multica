...
is to open or update a PR before posting the final Lumen issue comment, unless
...
When a step depends on PR state, query Lumen's link table — do not infer it
...
- Read what exists before writing: `lumen property list` shows the catalog;
  `lumen issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
lumen issue property set <issue-id> --name Environment --value staging
lumen issue property set <issue-id> --name Platforms --value "iOS,Android"
lumen issue property set <issue-id> --name Reviewer --value Bohan
lumen issue property unset <issue-id> --name Environment
```
...
```bash
lumen issue list --property "Impact=High" --property "Impact=Medium" --output json
lumen issue list --property "QA Status=__none__" --status in_review --output json
lumen issue list --sort property:Impact --direction desc --output json
```
...
```bash
lumen issue list --status in_progress --output json --resolve-properties
lumen issue get <issue-id> --resolve-properties
```
...
```bash
lumen issue runs <issue-id> --active --output json     # in-flight runs on this issue
lumen issue runs <issue-id> --siblings --output json   # ...and across the sub-issue family
```
...
id with `lumen issue run-messages`.
...
```bash
lumen issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
lumen issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
lumen issue status <child-id> todo   # promote when the previous step is truly done
```
...
lumen issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
lumen issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
lumen issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
lumen issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
lumen issue children <parent-id>             # sub-issues grouped by stage
lumen issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
lumen issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
lumen issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
lumen issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
lumen issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
lumen issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```

## Issue wakeups

Use `lumen issue wakeup` to arrange a future ordinary run, then finish the
