...
- `create_issue` creates a Lumen issue, making the run visible as issue state.
...
```bash
lumen autopilot list --output json
lumen autopilot get <autopilot-id> --output json
lumen autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
lumen autopilot update <autopilot-id> --status active|paused --output json
lumen autopilot runs <autopilot-id> --output json
lumen autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
lumen autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
lumen autopilot trigger <autopilot-id> --output json
lumen autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
The default `lumen autopilot list` table includes `LAST_STATUS`. Treat a
healthy-looking `STATUS=active` row with `LAST_STATUS=failed` as a failing
autopilot; inspect `lumen autopilot runs <id> --output json` for the failure
reason.
...
1. `lumen autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `lumen autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `lumen squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `lumen agent get <agent-id> --output json` and `lumen runtime list --output json`.
