...
```bash
lumen issue get <issue-id> --output json
lumen squad get <squad-id> --output json
lumen squad member list <squad-id> --output json
lumen issue comment list <issue-id> --roots-only --summary --compact --output json
lumen issue comment list <issue-id> --thread <thread-id> --tail 30 --compact --output json
```
...
```bash
lumen squad list --output json
lumen squad get <squad-id> --output json
lumen squad create --name <name> --leader <agent-name-or-id> --output json
lumen squad update <squad-id> --instructions "<leader coordination policy>" --output json
lumen squad delete <squad-id>
```
...
```bash
lumen squad member list <squad-id> --output json
lumen squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
lumen squad member remove <squad-id> --member-id <id> --type agent|member
lumen squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
lumen squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
lumen issue get <issue-id> --output json
lumen issue update <issue-id> --help
lumen issue comment list <issue-id> --roots-only --summary --compact --output json
lumen issue comment add <issue-id> --help
```
...
For squad leader tasks, Lumen appends a squad leader briefing to the leader
...
members carry no skills segment. Built-in `lumen-*` skills are not listed —
...
- recording squad activity with `lumen squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `lumen issue status` no.
