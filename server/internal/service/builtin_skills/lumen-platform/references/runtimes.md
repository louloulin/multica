...
7. `lumen repo checkout` talks to the local daemon, not directly to GitHub.

## CLI

```bash
lumen runtime list --output json
lumen runtime usage <runtime-id> --output json
lumen runtime activity <runtime-id> --output json
lumen runtime update <runtime-id> --target-version <version> --output json
lumen runtime delete <runtime-id>
lumen repo checkout <url>
lumen repo checkout <url> --ref <branch-or-sha>
lumen repo checkout <url> --fresh
```
...
final usage, Lumen can recover only main-loop input and cache tokens: split
...
bound again (`lumen agent update <id> --runtime-id <runtime-id>`), and every
...
`repo checkout` requires both `LUMEN_DAEMON_PORT` and the injected task-scoped
`LUMEN_TOKEN`; it is intended to run inside the active daemon task and from
...
- API commands such as `issue list`, `issue get`, and `issue runs` use the
  injected task identity and never fall back to the daemon Owner's saved Lumen
  profile.
- `config show` and `config set` operate only on task-local Lumen state. A
...
`LUMEN_DAEMON_PORT` alone is a weak, defense-in-depth signal for task-safe API
and profile resolution, not task identity for human/local command rejection.
Genuine tasks also carry `LUMEN_AGENT_ID` / `LUMEN_TASK_ID`,
`LUMEN_TASK_CONFIG_ROOT`, or a daemon-managed workdir marker. When the port is
...
operators should not export `LUMEN_DAEMON_PORT`; the daemon derives its host
...
4. Is the runtime online? `lumen runtime list --output json`.
