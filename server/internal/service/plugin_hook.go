...
// The hook engine: the one place Lumen calls OUT to a plugin's own server.
//
...
func (s *PluginService) callHookEndpoint(ctx context.Context, invocation HookInvocation) (json.RawMessage, error) {
	...
	if s.isDevOrigin(invocation.Hook.Transport.URL) {
		// The operator named this exact origin in LUMEN_PLUGIN_DEV_ORIGINS —
	...
	request.Header.Set("X-Lumen-Timestamp", timestamp)
	request.Header.Set("X-Lumen-Signature", hookSignatureVersion+"="+signature)
	request.Header.Set("X-Lumen-Plugin-Installation", uuidString(installation.ID))
	request.Header.Set("User-Agent", "Lumen-Hooks/1")
...
func (s *PluginService) hookSigningKey(installationID pgtype.UUID) ([]byte, error) {
	if len(s.DeploymentKey) == 0 {
		return nil, pluginErrf(PluginErrorUnavailable, "hooks are disabled: LUMEN_PLUGIN_SECRET_KEY is not configured")
	}
	if len(s.DeploymentKey) != 32 {
		return nil, pluginErrf(PluginErrorUnavailable, "hooks are disabled: LUMEN_PLUGIN_SECRET_KEY must decode to 32 bytes")
	}
	mac := hmac.New(sha256.New, s.DeploymentKey)
	mac.Write([]byte("lumen-plugin-hook-signature:v1:"))
