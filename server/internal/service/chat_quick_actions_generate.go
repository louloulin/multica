...
// LUMEN_LLM_API_KEY / LUMEN_LLM_BASE_URL, and matches how chat auto-titling
// already degrades.
...
// so it inherited the Lumen runtime brief's identity and drifted toward
...
func (s *TaskService) GenerateChatQuickActionsForTask(ctx context.Context, task db.AgentTaskQueue, origin ChatQuickActionsOrigin) error {
	...
	raw, err := s.QuickActions.GenerateJSON(ctx,
		"", // deployment default: LUMEN_LLM_DEFAULT_MODEL, else llm.FallbackModel
