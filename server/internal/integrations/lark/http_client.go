...
const (
	...
	// Operators do NOT set LUMEN_LARK_HTTP_BASE_URL to pick a cloud
	...
	// latency from a self-hosted Lumen deployment to feishu.cn.
...
func (c HTTPClientConfig) withDefaults() HTTPClientConfig {
	...
	// non-empty BaseURL (LUMEN_LARK_HTTP_BASE_URL, or an httptest URL
...
// keyed by app_id so a single Lumen server reuses Lark's
...
// cfg.BaseURL (LUMEN_LARK_HTTP_BASE_URL, or an httptest URL in tests)
...
func bindingPromptTemplate(bindURL string) (string, error) {
	doc := map[string]any{
		...
		"header": map[string]any{
			"template": "blue",
			"title":    map[string]any{"tag": "plain_text", "content": "Lumen"},
		},
		"elements": []any{
			map[string]any{
				"tag": "div",
				"text": map[string]any{
					"tag":     "lark_md",
					"content": "你还没有绑定 Lumen 账户。点击下方按钮完成绑定后即可使用此 Agent。",
				},
			},
			map[string]any{
				"tag": "action",
