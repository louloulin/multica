...
func TestHTTPClient_SendBindingPromptCard_HappyPath(t *testing.T) {
	...
	if err := c.SendBindingPromptCard(context.Background(), BindingPromptParams{
		...
		BindURL:        "https://lumen.test/lark/bind?token=abc",
	}); err != nil {
	...
	if !strings.Contains(capturedBody["content"], "lumen.test/lark/bind") {
...
func TestBindingPromptTemplate_Shape(t *testing.T) {
	raw, err := bindingPromptTemplate("https://lumen.test/bind?token=abc")
	if err != nil {
	...
	if btn["url"] != "https://lumen.test/bind?token=abc" {
