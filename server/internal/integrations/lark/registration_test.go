...
func TestRegistrationClient_Begin_HappyPath(t *testing.T) {
	...
	res, err := c.Begin(context.Background(), "Ada - Lumen", "")
	if err != nil {
	...
	if !strings.HasPrefix(q.Get("source"), "go-sdk/lumen") {
		t.Errorf("qr source=%q want go-sdk/lumen", q.Get("source"))
	}
	// The name preset pre-fills the Lark PersonalAgent creation form so
	// the bot defaults to "<agent> - Lumen" rather than the
	// auto-generated "{用户姓名}的智能助手".
	if q.Get("name") != "Ada - Lumen" {
		t.Errorf("qr name=%q want %q", q.Get("name"), "Ada - Lumen")
	}
}
