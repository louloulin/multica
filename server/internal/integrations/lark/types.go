...
// can't accidentally pass a Lumen user UUID where a Lark open_id is
// expected.
type OpenID string

// ChatID identifies a Lark conversation (p2p or group). One ChatID maps
// to one Lumen chat_session via lark_chat_session_binding.
type ChatID string
...
// separate clouds with distinct hosts; a single Lumen deployment serves
...
const (
	...
	// DropReasonNonWorkspaceMember — the sender resolved to a Lumen
