...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	LumenUserID  pgtype.UUID // the installer's Lumen account
...
// identity is the supplied lumenUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Lumen user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, lumenUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, lumenUserID)
	if err != nil {
	...
	if err != nil {
		...
		// lumen_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when lumen_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.LumenUserID)
	if err != nil {
...
// open_id) pair and points at a different Lumen user. Account
