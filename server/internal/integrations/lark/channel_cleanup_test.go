...
`, ws, agent, app, ccInstaller).Scan(&id); err != nil {
	...
	exec(`INSERT INTO channel_user_binding (workspace_id, lumen_user_id, installation_id, channel_type, channel_user_id)
...
func TestDeleteChannelInstallationsBySystemRuntimeAgents(t *testing.T) {
	...
	clean := func() {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM channel_user_binding WHERE lumen_user_id = $1`, ccUser)
...
VALUES ($1, $2, 'cc runtime', 'local', 'lumen_daemon')`, ccRuntime, ccWS)
...
func TestDeleteWorkspace_SweepsChannelInstallations(t *testing.T) {
	...
	exec(`INSERT INTO agent_runtime (id, workspace_id, name, runtime_mode, provider)
VALUES ($1, $2, 'cc runtime del', 'local', 'lumen_daemon')`, ccRuntimeDel, ccWSDel)
	exec(`INSERT INTO agent (id, workspace_id, name, runtime_mode, runtime_id)
