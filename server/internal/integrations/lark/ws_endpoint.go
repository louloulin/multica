...
// LUMEN_LARK_CALLBACK_BASE_URL still overrides every installation when
// set (staging / mock).
...
// `lumen` -> register a PersonalAgent -> enable WS -> watch logs.
...
func (c HTTPConnectionTokenConfig) withDefaults() HTTPConnectionTokenConfig {
	...
	// (LUMEN_LARK_CALLBACK_BASE_URL, or an httptest URL in tests)
...
func (f *HTTPConnectionTokenFetcher) Endpoint(ctx context.Context, creds InstallationCredentials) (WSEndpoint, error) {
	...
	// the audience Lumen server logs are read by today; if i18n
