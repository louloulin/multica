...
const (
	...
	chatStartedText         = "✅ Started a new Lumen chat. Your next message will enter it."
	...
	issueNotMemberText = "You're not a member of this Lumen workspace, so I can't file an issue for you. Ask a workspace admin to invite you, then send the command again."
	issueDisabledText  = "This DingTalk robot isn't connected to Lumen (or was disconnected). Ask the agent owner or a workspace owner/admin to reconnect it."
)
...
type OutboundReplierConfig struct {
	...
	Decrypt Decrypter
	Client  *Client
	// AppURL is the Lumen web app host the user clicks into to redeem the
	// binding token (e.g. https://lumen.example). The bind page (/dingtalk/bind)
...
func (r *OutboundReplier) sendBindingPrompt(ctx context.Context, inst engine.ResolvedInstallation, msg channel.InboundMessage, res engine.Result) error {
	...
	text := "👋 To start chatting with me, link your DingTalk account to Lumen: [link your account](" +
