...
func TestIssueCreatedText(t *testing.T) {
	...
	if got := issueCreatedText(engine.Result{IssueID: issueID, IssueIdentifier: "LUM-42", IssueTitle: "Fix login"}, ""); got != "✅ Created LUM-42 — Fix login" {
		t.Fatalf("got %q", got)
	}
...
func TestIssueDuplicateText(t *testing.T) {
	...
	if got != "⚠️ Not created — active issue LUM-42 already exists: Fix login" {
...
func TestReplierCommandConfirmationsKeepSuccessMarker(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "issue", result: engine.Result{Outcome: engine.OutcomeIngested, IssueID: pgtype.UUID{Valid: true}, IssueWorkspaceSlug: "team-b", IssueIdentifier: "LUM-42", IssueTitle: "✅ Keep this title"}, text: "Created [LUM\\-42](https://lumen.example/team-b/issues/00000000-0000-0000-0000-000000000000) — ✅ Keep this title", success: true},
