...
const verifiedTestBotName = "Lumen Bot - Local"
...
func TestInboundFromCallbackWithBotName_PlainTextRemovesOnlyVerifiedBot(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			content:     "@Lumen Bot - Local /new inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "bot after chat",
			content:     "/new @Lumen Bot - Local inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "bot after body",
			content:     "/new inspect this @Lumen Bot - Local",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "repeated bot mentions",
			content:     "@Lumen Bot - Local @Lumen Bot - Local /new inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "other mention before bot",
			content:     "@Alice @Lumen Bot - Local /new inspect this",
			want:        "@Alice /new inspect this",
			wantCommand: false,
		},
		{
			name:        "other mention after command",
			content:     "@Lumen Bot - Local /new inspect this with @Alice",
			want:        "/new inspect this with @Alice",
			wantCommand: true,
		},
		{
			name:        "different bot preserved",
			content:     "@Lumen Bot - DEV @Lumen Bot - Local /new inspect this",
			want:        "@Lumen Bot - DEV /new inspect this",
			wantCommand: false,
		},
		{
			name:        "longer name preserved",
			content:     "@Lumen Bot - Locality /new inspect this",
			want:        "@Lumen Bot - Locality /new inspect this",
			wantCommand: false,
		},
		{
			name:        "punctuation-extended name preserved",
			content:     "@Lumen Bot - Local-DEV /new inspect this",
			want:        "@Lumen Bot - Local-DEV /new inspect this",
			wantCommand: false,
		},
		{
			name:        "embedded literal preserved",
			content:     "quote@Lumen Bot - Local /new inspect this",
			want:        "quote@Lumen Bot - Local /new inspect this",
			wantCommand: false,
		},
		{
			name:        "mid sentence command stays prose",
			content:     "@Lumen Bot - Local please /new later",
			want:        "please /new later",
			wantCommand: false,
		},
		{
			name:        "blank line before command",
			content:     "@Lumen Bot - Local\n\n/new inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			content: `{"richText":[
				...
				{"text":"@Lumen Bot - Local /new inspect this"}
			]}`,
			wantText:        "[Image]\ninspect this",
		...
		},
		{
			...
			content: `{"richText":[
				{"text":"@Lumen Bot - Local /new ask @Alice"},
				{"type":"picture","downloadCode":"dl-1"}
			]}`,
			wantText:        "ask @Alice\n[Image]",
		...
		},
		{
			...
			content: `{"richText":[
				{"text":"@Alice @Lumen Bot - Local /new inspect this"},
				{"type":"picture","downloadCode":"dl-1"}
			]}`,
...
func TestInboundFromCallbackWithBotName_FailClosedWithoutVerifiedName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			text: "@Lumen Bot - Local /new inspect this",
			want: "@Lumen Bot - Local /new inspect this",
		},
		{
		...
		},
		{
			...
			text:    "/new inspect this @Lumen Bot - Local",
			want:    "/new inspect this @Lumen Bot - Local",
			command: true,
		},
		{
			name:    "rich text preserves unknown bot",
			msgtype: "richText",
			content: `{"richText":[{"text":"@Lumen Bot - Local /new inspect this"}]}`,
			want:    "@Lumen Bot - Local /new inspect this",
		},
	}
...
func TestInboundFromCallbackWithBotName_DoesNotStripPrivateOrUnaddressedText(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cb.Text.Content = "@Lumen Bot - Local /new inspect this"
			msg, ok := inboundFromCallbackWithBotName(cb, "appkey-A", verifiedTestBotName)
...
func TestInboundFromCallbackWithBotName_AllSharedCommandsKeepFirstTextLineContract(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "new",
			content: "@Lumen Bot - Local /clear inspect this",
			parse: func(body string) bool {
		...
		{
			name:    "issue",
			content: "@Lumen Bot - Local /issue login failed",
			parse: func(body string) bool {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cb.Text.Content = "@Lumen Bot - Local please " + strings.TrimPrefix(tc.content, "@Lumen Bot - Local ")
			msg, ok = inboundFromCallbackWithBotName(cb, "appkey-A", verifiedTestBotName)
			if !ok || tc.parse(msg.CommandText) {
