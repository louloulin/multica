// Package composio is the Stage 2 business-integration glue between Lumen and
// the standalone Composio SDK (server/pkg/composio). It owns Lumen semantics:
...
import (
	...
	sdk "github.com/lumen-ai/lumen/server/pkg/composio"
...
type Config struct {
	...
	// trailing slash (e.g. "https://lumen.ai"). The Composio callback
	...
	// browser redirect (e.g. "https://lumen.ai"). May be empty, in which
...
//
// The composio_user_id sent to Composio is the Lumen user id verbatim — the
...
func (s *Service) CompleteCallback(ctx context.Context, state, status, connectedAccountID string) (string, error) {
	...
	// composio_user_id == lumen user id invariant) and was created under the
	...
	if _, err := s.store.UpsertUserComposioConnection(ctx, db.UpsertUserComposioConnectionParams{
		...
		// Invariant: composio_user_id == Lumen user id.
