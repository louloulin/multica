...
func newTestService(t *testing.T, client SDK, store Store) *Service {
	...
	svc, err := NewService(client, store, Config{
		...
		CallbackBaseURL: "https://lumen.ai",
		FrontendBaseURL: "https://lumen.ai",
...
func TestBeginConnect_MappingAndState(t *testing.T) {
	...
	// composio_user_id == lumen user id
	...
	if !strings.HasPrefix(cb, "https://lumen.ai"+callbackPath+"?state=") {
...
func TestCallbackRedirect(t *testing.T) {
	t.Parallel()
	svc := newTestService(t, &fakeSDK{}, newFakeStore())
	if got := svc.CallbackRedirect("notion", true); got != "https://lumen.ai/settings?tab=integrations&connected=notion" {
		t.Errorf("success redirect = %q", got)
	}
	if got := svc.CallbackRedirect("notion", false); got != "https://lumen.ai/settings?tab=integrations&error=composio_connect_failed" {
