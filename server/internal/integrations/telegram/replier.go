...
const (
	...
	msgChatStarted    = "✅ Started a new Lumen chat. Your next message will enter it."
	msgIssueUsage     = "Please include an issue title. Use:\n\n/issue <title>\n[description] (optional)"
	msgIssueNotMember = "You're not a member of this Lumen workspace, so I can't file an issue for you. Ask a workspace admin to invite you, then send the command again."
	msgIssueDisabled  = "This Telegram bot isn't connected to Lumen (or was disconnected). Ask a workspace admin to reconnect it."
)
...
type OutboundReplierConfig struct {
	Binding bindingMinter
	Decrypt Decrypter
	// AppURL is the Lumen web app host for the redeem link, same sourcing as
	// the Slack replier (LUMEN_APP_URL ?? FRONTEND_ORIGIN).
	AppURL      string
...
func (r *OutboundReplier) sendBindingPrompt(ctx context.Context, inst engine.ResolvedInstallation, msg channel.InboundMessage, res engine.Result) error {
	...
	// would bind the original sender's Telegram identity to the wrong Lumen
	...
	text := "👋 To start chatting with me, link your Telegram account to Lumen:\n" + bindURL + "\n(This link expires in 15 minutes.)"
