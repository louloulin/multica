...
// Slack user id is bound to their Lumen account. It mirrors
...
var (
	...
	// different Lumen user (account transfer must go through explicit unbind).
...
// lumenUserID (taken from the session, never from the token). Returns
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, lumenUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	if _, err := qtx.CreateChannelUserBinding(ctx, db.CreateChannelUserBindingParams{
		...
		// the ON CONFLICT DO UPDATE WHERE lumen_user_id=… gating rejected it.
		if errors.Is(err, pgx.ErrNoRows) {
