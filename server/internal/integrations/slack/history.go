...
// failed) read so the unified `lumen chat history` / `lumen chat thread`
...
// `lumen chat history` (channel overview) and `lumen chat thread [id]`
...
// boundary for `lumen chat thread <id>` (the agent supplies only a
...
// drills into with `lumen chat thread <id>`. Backs `lumen chat history`.
...
// threads) reads its linear conversation. Backs `lumen chat thread [id]`.
...
// agent can drill in with `lumen chat thread <id>`.
