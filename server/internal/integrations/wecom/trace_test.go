...
// trace_test.go — guards for the LUMEN_WECOM_TRACE operator switch. Six
...
// with LUMEN_WECOM_TRACE unset, a real inbound message and the frames around
...
// and puts it in a URL inside the message body; with a normal LUMEN_APP_URL
...
func TestBindingPromptFitsInsideThePreviewCap(t *testing.T) {
	...
	bindURL := "https://lumen.example" + "/wecom/bind" + "?token=" + url.QueryEscape(rawToken)
	upToTokenEnd := "👋 请先绑定你的 Lumen 账号，才能与我对话：\n" + bindURL
...
// cut lands inside an escape — so an operator who turned LUMEN_WECOM_TRACE
