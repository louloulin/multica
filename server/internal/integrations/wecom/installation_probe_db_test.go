...
VALUES ($1, $2, 'wecom probe runtime', 'local', 'lumen_daemon') ON CONFLICT (id) DO NOTHING`, wcPrbRuntime, wcPrbWS)
	exec(`INSERT INTO agent_runtime (id, workspace_id, name, runtime_mode, provider)
VALUES ($1, $2, 'wecom probe other runtime', 'local', 'lumen_daemon') ON CONFLICT (id) DO NOTHING`, wcPrbOtherRT, wcPrbOtherWS)
