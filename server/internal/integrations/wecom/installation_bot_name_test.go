...
VALUES ($1, $2, 'wecom bot name runtime', 'local', 'lumen_daemon') ON CONFLICT (id) DO NOTHING`, wcNameRuntime, wcNameWS)
...
func TestRotatingTheSecretKeepsTheBotName(t *testing.T) {
	...
	if _, err := svc.Upsert(ctx, botNameParams(wcNameBotA, "Lumen Bot")); err != nil {
	...
	if again.BotDisplayName != "Lumen Bot" {
...
func TestTheBotNameSurvivesTheConfigRoundTrip(t *testing.T) {
	t.Parallel()
	cfg, err := encodeInstallConfig(Installation{BotID: "bot-xyz", BotDisplayName: "Lumen Bot"})
	if err != nil {
	...
	if out.BotDisplayName != "Lumen Bot" {
		t.Fatalf("BotDisplayName round trip = %q, want Lumen Bot", out.BotDisplayName)
	}
}
