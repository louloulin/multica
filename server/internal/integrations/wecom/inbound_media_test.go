...
// mention arrives glued to the front of the text run: "@Lumen Bot /issue
...
// parser sees "@Lumen Bot", drop the placeholder removal and it sees
...
func TestDispatchFrame_GroupMentionedMixedCommand(t *testing.T) {
	...
	env := mixedFrame(t, group,
		...
		textRun("@Lumen Bot /issue 登录坏了"),
	)
	got, called, _ := dispatchOneAs(t, env, "Lumen Bot")
	if !called {
	...
	if want := "[Image]\n@Lumen Bot /issue 登录坏了"; got.Text != want {
...
// "@Lumen Bot /issue 登录坏了". WeCom delivers one 图文混排 with an image run
...
func TestDispatchFrame_GroupMentionedSpokenIssueWithAnImage(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			items:    []map[string]any{imageRun("https://cos.example.com/a"), voiceRun("@Lumen Bot /issue 登录坏了")},
			wantBody: "[Image]\n@Lumen Bot /issue 登录坏了",
		},
		{
			...
			items:    []map[string]any{textRun("@Lumen Bot"), imageRun("https://cos.example.com/a"), voiceRun("/issue 登录坏了")},
			wantBody: "@Lumen Bot\n[Image]\n/issue 登录坏了",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got, called, _ := dispatchOneAs(t, mixedFrame(t, group, tc.items...), "Lumen Bot")
			if !called {
				t.Fatal("the group message never reached the handler")
			}
			if got.Source.ChatType != channel.ChatTypeGroup {
