...
func newHarness(t *testing.T) *harness {
	...
	h := &harness{
		...
		issues:    &fakeIssues{},
		tasks:     &fakeTasks{},
		reader:    &fakeReader{ws: db.Workspace{IssuePrefix: "LUM", Slug: "demo-web"}},
		lifecycle: &fakeChannelChatLifecycle{},
	}
...
func TestRouter_IssueCommand_Creates(t *testing.T) {
	...
	if !waitFor(time.Second, func() bool {
		for _, r := range h.replier.calls() {
			if r.IssueIdentifier == "LUM-42" && r.IssueWorkspaceSlug == "demo-web" && r.IssueTitle == "Fix login" {
				return true
			}
		}
		return false
...
func TestRouter_IssueCommand_ActiveDuplicateIsTerminalProductOutcome(t *testing.T) {
	...
	if !waitFor(time.Second, func() bool {
		for _, result := range h.replier.calls() {
			if result.IssueDuplicate && result.IssueID == duplicate.ID && result.IssueIdentifier == "LUM-44" && result.IssueWorkspaceSlug == "demo-web" && result.IssueTitle == duplicate.Title {
				return true
			}
		}
		return false
