...
// `lumen chat history` (the channel OVERVIEW: top-level messages + thread
// metadata, not thread contents) and `lumen chat thread [id]` (one thread's
...
// platform so the agent reads a uniform list, like `lumen issue comment list
// --output json`.
type HistoryMessage struct {
	...
	// so the agent can `lumen chat thread <thread_id>` to read its contents.
	// They are absent on a plain message and on thread-read rows.

	// ThreadID is the identifier to pass to `lumen chat thread <id>` to read
