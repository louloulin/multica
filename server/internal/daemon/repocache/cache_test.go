...
func TestCreateWorktreeContextCancelsRunningGitProcessTree(t *testing.T) {
	...
	body := "#!/bin/sh\n(sleep 0.5; echo leaked > \"$LUMEN_TEST_GIT_MARKER\") &\nwait\n"
	if err := os.WriteFile(script, []byte(body), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", binDir+string(os.PathListSeparator)+os.Getenv("PATH"))
	t.Setenv("LUMEN_TEST_GIT_MARKER", marker)
...
// invocation Windows Codex tasks now use (lumen-ai/lumen#6449) and proves
...
// for the Lumen Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: lumen-agent <github@lumen.ai>"
...
func TestCoAuthoredByHookIdempotent(t *testing.T) {
	...
	trailer := "Co-authored-by: lumen-agent <github@lumen.ai>"
	if err := os.WriteFile(filepath.Join(result.Path, "test.txt"), []byte("hello\n"), 0o644); err != nil {
		t.Fatalf("write test file: %v", err)
	}
	runGitAuthored(t, result.Path, "add", ".")
...
// path: a bare cache that already carries the Lumen prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: lumen-agent") {
...
// prepare-commit-msg hook that does NOT include the lumenHookMarker
// sentinel — only the older `# Installed by the Lumen daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no lumenHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: lumen-agent") {
...
// without the Lumen marker (e.g. one a user added manually) must be left
...
func TestCoAuthoredByStateStopsTrailerInExistingCheckout(t *testing.T) {
	...
	if msg := commit("a.txt", "enabled commit"); !strings.Contains(msg, "Co-authored-by: lumen-agent") {
	...
	if msg := commit("b.txt", "disabled commit"); strings.Contains(msg, "Co-authored-by: lumen-agent") {
		t.Errorf("commit in the existing checkout still carries the trailer after the toggle was turned off.\ngot:\n%s", msg)
	}
	if msg := commit("c.txt", "disabled commit, no-verify", "--no-verify"); strings.Contains(msg, "Co-authored-by: lumen-agent") {
	...
	if msg := commit("d.txt", "re-enabled commit"); !strings.Contains(msg, "Co-authored-by: lumen-agent") {
...
func TestReconcileCoAuthoredByHooksMigratesReleasedHook(t *testing.T) {
	...
	if msg := commitInWorktree(t, worktreePath, "a.txt", "before the toggle"); !strings.Contains(msg, "Co-authored-by: lumen-agent") {
	...
	if msg := commitInWorktree(t, worktreePath, "b.txt", "after the toggle"); strings.Contains(msg, "Co-authored-by: lumen-agent") {
...
func TestReconcileCoAuthoredByHooksUpgradesReleasedHookInPlace(t *testing.T) {
	...
	if msg := commitInWorktree(t, worktreePath, "a.txt", "still enabled"); !strings.Contains(msg, "Co-authored-by: lumen-agent") {
	...
	if msg := commitInWorktree(t, worktreePath, "b.txt", "toggled off"); strings.Contains(msg, "Co-authored-by: lumen-agent") {
...
func TestCreateWorktreeDoesNotResurrectDisabledState(t *testing.T) {
	...
	if msg := commitInWorktree(t, result.Path, "a.txt", "checked out with a stale snapshot"); strings.Contains(msg, "Co-authored-by: lumen-agent") {
...
func TestReconcileCoAuthoredByHookInCheckoutMigratesReleasedHook(t *testing.T) {
	...
	if msg := commitInWorktree(t, result.Path, "a.txt", "before the toggle"); !strings.Contains(msg, "Co-authored-by: lumen-agent") {
		t.Fatalf("precondition: the previous release's hook should still add the trailer.\ngot:\n%s", msg)
	}
	...
	if msg := commitInWorktree(t, result.Path, "b.txt", "after the toggle"); strings.Contains(msg, "Co-authored-by: lumen-agent") {
