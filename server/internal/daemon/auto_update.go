...
// ParseSelfVersion pulls the version out of `lumen --version` output, whose
// first line is rendered by cmd/lumen's version template:
//
//	lumen 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)
...
//
// Exported solely so cmd/lumen can pin that contract from the producing side:
...
func ParseSelfVersion(raw string) string {
	...
	if fields := strings.Fields(line); len(fields) >= 2 && fields[0] == "lumen" {
		return fields[1]
	}
	return line
}
...
// different lumen binary". It runs two independent checks on one goroutine,
...
// (--no-auto-update / LUMEN_DAEMON_AUTO_UPDATE=false), when the server is
...
func (d *Daemon) autoUpdateLoop(ctx context.Context) {
	...
	if reloadEnabled {
		d.logger.Info("auto-reload: watching the lumen binary on disk",
...
// trySelfReload restarts the daemon when the lumen binary on disk no longer
// reports the version compiled into this process.
//
// This is the out-of-band half of self-update. `brew upgrade lumen`, a
...
// has its own switch (--no-auto-reload / LUMEN_DAEMON_AUTO_RELOAD=false /
// disable_auto_reload) rather than riding on LUMEN_DAEMON_AUTO_UPDATE.
//
...
func (d *Daemon) trySelfReload(ctx context.Context) {
	...
	reason := fmt.Sprintf("lumen binary on disk reports %s, running %s", onDisk, d.cfg.CLIVersion)
...
// setReloadPending records that a lumen version change is confirmed but the
