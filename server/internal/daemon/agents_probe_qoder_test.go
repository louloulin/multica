...
// rescuing an operator-pinned LUMEN_QODER_PATH. A pinned absolute path that
...
func TestProbeAgentCLIs_QoderPinnedPathStaysHardMiss(t *testing.T) {
	...
	t.Setenv("PATH", "")
	t.Setenv("LUMEN_QODER_PATH", "/nonexistent/pinned/qodercli")

	if entry, ok := probeAgentCLIs()["qoder"]; ok {
		t.Errorf("pinned-but-missing LUMEN_QODER_PATH resolved to %q, want a hard miss", entry.Path)
	}
}
