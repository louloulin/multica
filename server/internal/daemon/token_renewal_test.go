...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "lumen login") {
		t.Fatalf("401 warning must tell the user to run 'lumen login', got: %s", out)
	}
}
...
// and the operator would never see a "run lumen login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "lumen login") {
		t.Fatalf("expected the actionable 'run lumen login' hint in the WARN, got: %s", out)
	}
}
