...
// same machine shares one UUID stored at `~/.lumen/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.lumen/profiles/*/daemon.id` and returns every
...
// explicitly pins LUMEN_DAEMON_ID to the hostname itself, there's nothing
