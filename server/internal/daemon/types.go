...
type AgentEntry struct {
	...
	// Command is the bare command name or LUMEN_*_PATH value that Path was
...
type Task struct {
	...
	ChatMessageAttachments        []ChatAttachmentMeta   `json:"chat_message_attachments,omitempty"`         // attachments linked to the chat message; agent uses these to `lumen attachment download <id>`
	...
	// The daemon injects it into the spawned agent as LUMEN_TOKEN so the
...
// run `lumen attachment download <id>` instead of guessing from a
