...
// stubSelfVersion makes the `lumen --version` probe report version (or fail).
...
// LUMEN_DAEMON_AUTO_UPDATE. Self-hosted daemons default auto-update off
...
func TestParseSelfVersion(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			raw:  "lumen 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)\ngo: go1.26.1, os/arch: darwin/arm64\n",
			want: "0.3.7",
		},
		{
			name: "dev build from git describe",
			raw:  "lumen v0.3.7-42-gabcdef0-dirty (commit: abcdef0, built: unknown)\n",
