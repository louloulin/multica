...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Lumen writes the
	...
	// so cross-task and cross-workspace context leaks (lumen#3130) cannot
...
// enumerated by a plain `codex` run, keeping Lumen task history out of the
// user's own thread list.
const codexSessionStoreRoot = "lumen-sessions"
...
// shared home's lumen-sessions root that have not been touched within
