...
const sidecarManifestFile = ".lumen_sidecar_manifest.json"
...
//     so the agent still discovers the Lumen skill, just under a
//     different directory name.
//   - For the Lumen-only namespace (.lumen/project/resources.json)
...
// down a Lumen skill without colliding with a user-installed skill
...
// collision we append `-lumen`, then `-lumen-2`, `-lumen-3`,
...
// baseSlug: the bare slug first, then `-lumen`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-lumen"
	default:
		return fmt.Sprintf("%s-lumen-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .lumen/, .claude/skills/, .github/skills/,
...
// repository, so "left on disk" means a marker that disables every lumen
...
// issue-review-lumen. A managed skill directory is platform-owned — the
