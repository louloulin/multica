...
// `no such host` errors when calling out (for example, `lumen issue get`
// hitting the Lumen API). See upstream issue openai/codex#10390.
//
...
//     rejects non-safe mutation commands "by policy" — e.g. `lumen issue
...
//     danger-full-access so the Lumen CLI can reach the API.
...
// lumenManagedBeginMarker / lumenManagedEndMarker delimit the block the
...
const (
	lumenManagedBeginMarker = "# BEGIN lumen-managed (do not edit; regenerated by daemon)"
	lumenManagedEndMarker   = "# END lumen-managed"
)

// renderLumenManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.lumen]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertLumenManagedBlock), avoids both traps.
func renderLumenManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(lumenManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(lumenManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(lumenManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(lumenManagedEndMarker) + `\n*`)

// upsertLumenManagedBlock returns the config content with the lumen-managed
...
// `[permissions.lumen]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderLumenManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertLumenManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderLumenManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the lumen-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertLumenManagedBlock(existing, policy)
