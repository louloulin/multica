...
Hidden body.`,
	...
	// The brief is now the only Lumen-rendered skill listing, for every kind.
...
// writeSkillFiles puts the second in `a-b-lumen`. The second skill then has
...
func TestBatchSlugAllocationCountsHiddenSkills(t *testing.T) {
	...
	// written — as `a-b-lumen`.
	if visible[0].Name != "a-b-lumen" {
		t.Errorf("visible skill listed as %q, want %q", visible[0].Name, "a-b-lumen")
	}
	if _, err := os.Stat(filepath.Join(dir, "a-b-lumen", "SKILL.md")); err != nil {
