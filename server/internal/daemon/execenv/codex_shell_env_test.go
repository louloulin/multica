...
`,
			...
			removed: []string{"legal quoted table", `FOO = "bar"`, `LUMEN_TOKEN = "stale"`},
		},
		{
...
`,
	...
	includeOnly := []string{"CUSTOM_FLAG", "LUMEN_SERVER_URL", "LUMEN_TOKEN", "PATH", "SystemRoot"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(s, `"LUMEN_*"`) {
				t.Fatalf("managed policy must not use a LUMEN_* wildcard:\n%s", s)
			}
...
func TestEnsureCodexShellEnvPolicyConfigRejectsInvalidInputWithoutWriting(t *testing.T) {
	...
	if err := EnsureCodexShellEnvPolicyConfig(configPath, []string{"LUMEN_TOKEN", "PATH"}, testLogger()); err == nil {
...
func TestEnsureCodexShellEnvPolicyConfigIsIdempotent(t *testing.T) {
	t.Parallel()
	configPath := filepath.Join(t.TempDir(), "config.toml")
	includeOnly := []string{"LUMEN_SERVER_URL", "LUMEN_TOKEN", "PATH"}
	...
	s := string(data)
	assertValidToml(t, s)
	if n := strings.Count(s, lumenShellEnvBeginMarker); n != 1 {
