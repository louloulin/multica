...
// subagents. The Lumen daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Lumen runtime
...
//
// Users who explicitly want Codex native subagents inside a Lumen task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `LUMEN_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// LumenCodexMultiAgentEnv is the env var users can set to keep Codex
...
const LumenCodexMultiAgentEnv = "LUMEN_CODEX_MULTI_AGENT"

// lumenMultiAgentBeginMarker / lumenMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	lumenMultiAgentBeginMarker = "# BEGIN lumen-managed multi-agent (do not edit; regenerated by daemon)"
	lumenMultiAgentEndMarker   = "# END lumen-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(lumenMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(lumenMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(LumenCodexMultiAgentEnv))
...
// renderLumenMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderLumenMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(lumenMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(lumenMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderLumenMultiAgentBlock(true)
...
//
// When LUMEN_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per LUMEN_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderLumenMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
