...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Lumen-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN LUMEN-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END LUMEN-RUNTIME -->"
...
func runtimeConfigPath(workDir, provider string) string {
	...
	case "codex", "copilot", "opencode", "codearts", "deveco", "openclaw", "hermes", "pi", "cursor", "kimi", "reasonix", "dsh", "kiro", "antigravity", "qoder", "qoderclicn", "traecli", "grok", "qwenpaw", "mcode", "dim", "zeroclaw", "lumos-acp":
...
// writeRuntimeConfigFile writes the Lumen runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Lumen marker
...
//
//   - User content carries a stray `<!-- END LUMEN-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Lumen marker block from the runtime
...
// of Lumen runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Lumen task would pick up
...
// agent about the Lumen runtime environment and available CLI tools.
//
