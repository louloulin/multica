...
func TestBriefSurfaceDeliveryPolicy(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"comment": {
			mustHave: []string{"`--attachment <path>` to `lumen issue comment add`"},
			mustNot:  []string{"lumen attachment upload"},
		},
		"assignment": {
			mustHave: []string{"`--attachment <path>` to `lumen issue comment add`"},
			mustNot:  []string{"lumen attachment upload"},
		...
		"chat_direct": {
			mustHave: []string{"`lumen attachment upload <local-path>`"},
		...
		"quickcreate": {
			mustHave: []string{"your stdout is text-only", "`lumen issue create` call itself via `--attachment <path>`"},
			mustNot:  []string{"lumen attachment upload"},
		},
	}
