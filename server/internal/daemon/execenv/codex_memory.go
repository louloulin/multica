...
// — users cannot audit or edit the contents from any Lumen UI.
//
// This conflicts with the Lumen daemon's context model. Lumen
...
//     unrelated host-project context into Lumen tasks. The reproduction
//     in github.com/lumen-ai/lumen#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Lumen issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Lumen task
// (and accept the leak risk) can keep the feature enabled by setting
// `LUMEN_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Lumen-owned, user-visible,
...
// LumenCodexMemoryEnv is the env var users can set to keep Codex
...
const LumenCodexMemoryEnv = "LUMEN_CODEX_MEMORY"

const (
	lumenMemoryFeatureBeginMarker = "# BEGIN lumen-managed memory-feature (do not edit; regenerated by daemon)"
	lumenMemoryFeatureEndMarker   = "# END lumen-managed memory-feature"
	lumenMemoryConfigBeginMarker  = "# BEGIN lumen-managed memory-config (do not edit; regenerated by daemon)"
	lumenMemoryConfigEndMarker    = "# END lumen-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(lumenMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(lumenMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(lumenMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(lumenMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(LumenCodexMemoryEnv))
...
// renderLumenMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderLumenMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(lumenMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(lumenMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderLumenMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderLumenMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(lumenMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(lumenMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderLumenMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderLumenMemoryConfigBlock(true))
}
...
//
// When LUMEN_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per LUMEN_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderLumenMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderLumenMemoryConfigBlock(false))
	}
