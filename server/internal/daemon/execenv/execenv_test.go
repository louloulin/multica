...
func TestPrepareDirectoryMode(t *testing.T) {
	...
	for _, sub := range []string{"workdir", "output", "logs", "lumen-config"} {
		path := filepath.Join(env.RootDir, sub)
		if _, err := os.Stat(path); os.IsNotExist(err) {
			t.Fatalf("expected %s to exist", path)
		}
	}
	if env.LumenConfigRoot != filepath.Join(env.RootDir, "lumen-config") {
		t.Fatalf("LumenConfigRoot = %q, want task-local config directory", env.LumenConfigRoot)
	}
	info, err := os.Stat(env.LumenConfigRoot)
	if err != nil {
		t.Fatalf("stat LumenConfigRoot: %v", err)
	}
	if got := info.Mode().Perm(); got != 0o700 {
		t.Fatalf("LumenConfigRoot mode = %o, want 700", got)
	}
	...
	// daemon task when a sandbox strips LUMEN_* from the environment.
...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/lumen-ai/lumen","ref":"release/v2","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".lumen", "project", "resources.json")
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".lumen", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestPrepareWithRepoContext(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".lumen" && name != "CLAUDE.md" && name != ".claude" {
...
// collision-free duplicates (issue-review, issue-review-lumen,
// issue-review-lumen-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -lumen duplicates", names)
	}
...
// next refresh dodges to issue-review-lumen. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -lumen duplicate", names)
	}
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		"### Squad maintenance",
		"lumen squad member set-role <squad-id>",
	} {
	...
	for _, banned := range []string{
		"--no-start",
		"lumen issue list [--status",
		"lumen issue label list",
		"lumen issue subscriber list",
		"lumen label list",
		"lumen workspace member list",
		"lumen agent list",
		"lumen squad list",
		"lumen issue runs",
		"lumen issue run-messages",
		"lumen attachment download",
		"lumen autopilot list",
		"lumen autopilot create",
		"lumen autopilot update",
		"lumen autopilot trigger",
		"lumen autopilot delete",
		"lumen project get",
		"lumen project resource list",
		"lumen issue label add",
		"lumen issue label remove",
		"lumen issue subscriber add",
		"lumen issue subscriber remove",
		"lumen issue comment delete",
		"lumen label create",
	} {
		if strings.Contains(s, banned) {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Lumen Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "lumen issue get") {
		t.Error("should reference lumen CLI even without skills")
	}
...
// user-installed skill already sitting at the natural slug pushes Lumen's
// copy to `<slug>-lumen`; the frontmatter has to follow it there, or the
...
func TestWriteContextFilesFrontmatterNameFollowsCollisionSlug(t *testing.T) {
	...
	// Lumen's copy landed at the fallback slug and names itself after it.
	moved, err := os.ReadFile(filepath.Join(dir, ".opencode", "skills", "review-lumen", "SKILL.md"))
	if err != nil {
...
func TestInjectRuntimeConfigQoderCN(t *testing.T) {
	...
	if !strings.Contains(string(content), "Lumen Agent Runtime") {
		t.Error("AGENTS.md missing meta skill header")
	}
}
...
func TestPrepareWithRepoContextOpencode(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".lumen" && name != "AGENTS.md" {
			t.Errorf("unexpected entry in workdir: %s", name)
		}
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `lumen issue comment add`
			...
			mustContain := []string{
				...
				"post it with `lumen issue comment add` using",
				"mandatory",
			}
...
// heredoc on `lumen issue create`, the flags were silently swallowed into
...
// non-ASCII as `?` before reaching `lumen.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		lumenManagedBeginMarker,
		lumenMultiAgentBeginMarker,
		lumenMemoryFeatureBeginMarker,
		lumenMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, lumenManagedBeginMarker) || !strings.Contains(s, lumenManagedEndMarker) {
...
// workspace-write and rejects mutation commands (e.g. `lumen issue create`)
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), lumenManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.lumen.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, lumenManagedBeginMarker)
	endIdx := strings.Index(s, lumenManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.lumen]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.lumen]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.lumen content")
	}
	...
	if n := strings.Count(string(data2), lumenManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.lumen], got:\n%s", s)
	}
	if strings.Count(s, lumenManagedBeginMarker) != 1 {
...
func TestPrepareCodexHomeWritesManagedSandboxBlock(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, lumenManagedBeginMarker) {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if reused.LumenConfigRoot != filepath.Join(reused.RootDir, "lumen-config") {
		t.Fatalf("LumenConfigRoot = %q, want restored task-local config directory", reused.LumenConfigRoot)
	}
	if info, err := os.Stat(reused.LumenConfigRoot); err != nil {
		t.Fatalf("stat restored LumenConfigRoot: %v", err)
	} else if got := info.Mode().Perm(); got != 0o700 {
		t.Fatalf("restored LumenConfigRoot mode = %o, want 700", got)
	}
	...
	if !strings.Contains(string(data), lumenManagedBeginMarker) {
		t.Error("reused config.toml missing lumen-managed block")
	}
}
...
// inside a Lumen task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigAssignmentTriggerScansRootsFirst(t *testing.T) {
	...
	// TestIssueCommentListHelpCarriesReadContract in cmd/lumen); the brief
	...
	for _, banned := range []string{
		"lumen issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigCatchUpScansRootsFirst(t *testing.T) {
	...
	for _, want := range []string{
		...
		// (TestIssueCommentListHelpCarriesReadContract in cmd/lumen).
	...
	if strings.Contains(s, "lumen issue comment list issue-1 --recent") {
...
func TestBriefCarriesNoMetadataGuidance(t *testing.T) {
	...
	banned := []string{
		...
		"lumen issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"lumen issue metadata delete <issue-id> --key <k>",
		...
		"Read the metadata bag (`lumen issue metadata list`)",
	}
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				"1. Read the issue (`lumen issue get`) to understand the context.",
