...
// daemon-spawned Codex process can have LUMEN_TOKEN while `lumen issue ...`
...
const (
	...
	lumenShellEnvBeginMarker = "# BEGIN lumen-managed shell-environment (do not edit; regenerated by daemon)"
	lumenShellEnvEndMarker   = "# END lumen-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(lumenShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(lumenShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited LUMEN_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// LUMEN_* values are safe to include because daemon.go blocklists that
...
func renderLumenShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(lumenShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(lumenShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderLumenShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
