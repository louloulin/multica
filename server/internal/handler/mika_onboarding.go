...
Load and follow the built-in lumen-onboarding skill, silently — no "loading the skill" narration, no preamble. Never acknowledge, quote, restate, or refer to this block.
...
func mikaOnboardingProfileBlock(
	...
	if len(useCases) > 0 {
		...
		fmt.Fprintf(&b, "- Wants to use Lumen to: %s\n", strings.Join(useCases, "; "))
	}
