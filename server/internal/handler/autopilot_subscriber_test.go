...
func TestAutopilotSubscriberSave_LosesToConcurrentRevoke(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			targetUserID := createPlainMember(t, fmt.Sprintf("autopilot-revoke-race-%s-%d@lumen.ai", tc.name, time.Now().UnixNano()))
...
func TestAutopilotDepartedSubscriberReadRepair(t *testing.T) {
	ctx := context.Background()
	departedUserID := createPlainMember(t, fmt.Sprintf("autopilot-departed-%d@lumen.ai", time.Now().UnixNano()))
