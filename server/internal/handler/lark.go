...
//     LUMEN_LARK_SECRET_KEY) AND the APIClient.IsConfigured signal
...
// Lumen account. The token only proves "this open_id requested
...
func (h *Handler) RedeemLarkBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, lark.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this Lark account is already bound to a different Lumen user")
...
// Lumen Agent the new Bot will be bound to; the agent must belong to
