...
func TestGitHubConnectRepositoryReturnTarget(t *testing.T) {
	t.Setenv("GITHUB_APP_SLUG", "lumen-test")
	t.Setenv("GITHUB_WEBHOOK_SECRET", "test-secret-123")
	wsID := "11111111-2222-3333-4444-555555555555"
...
func TestGitHubSetupCallbackRepositoryReturnTarget(t *testing.T) {
	t.Setenv("GITHUB_WEBHOOK_SECRET", "test-secret-123")
	t.Setenv("FRONTEND_ORIGIN", "https://app.lumen.test/")
	wsID := "11111111-2222-3333-4444-555555555555"
	...
	if got := rec.Header().Get("Location"); got != "https://app.lumen.test/settings?tab=repositories&github_error=bad_installation_id" {
...
// from GitHub issue lumen-ai/lumen#3264: a PR that mentions three issues
...
func TestGitHubRoutes_RoleGating(t *testing.T) {
	...
	mkUser := func(t *testing.T, label string) string {
		t.Helper()
		var id string
		email := fmt.Sprintf("github-routes-%s-%s@lumen.ai", slug, label)
		dbfx.QueryRow(t, `
