...
type AppConfig struct {
	...
	// self-hosted instances can show `lumen setup self-host` commands
	// with the operator's own domains instead of Lumen Cloud defaults.
	DaemonServerURL string `json:"daemon_server_url,omitempty"`
	DaemonAppURL    string `json:"daemon_app_url,omitempty"`

	// VCSIntegrationAvailable mirrors the LUMEN_VCS_INTEGRATION_ENABLED
	...
	// LUMEN_VCS_SECRET_KEY" hint a cloud user cannot resolve. Omitted when
...
func daemonSetupURLsFromEnv() (string, string) {
	serverURL := normalizePublicURL(os.Getenv("LUMEN_DAEMON_SERVER_URL"))
	if serverURL == "" {
		serverURL = normalizePublicURL(os.Getenv("LUMEN_PUBLIC_URL"))
	}
...
// (LUMEN_APP_URL, falling back to FRONTEND_ORIGIN), normalized. Shared by
// the daemon-setup URLs and the managed-cloud detection so both read the same
// signal.
func resolveFrontendAppURL() string {
	appURL := normalizePublicURL(os.Getenv("LUMEN_APP_URL"))
	if appURL == "" {
...
// Lumen Cloud, identified by its frontend host alone (lumen.ai). The
// daemon setup for the managed cloud is always
// `lumen setup` (which hardcodes api.lumen.ai), so the per-deployment URLs
// must be omitted from /api/config even when LUMEN_PUBLIC_URL is unset or
// misconfigured. Previously this also required serverURL==api.lumen.ai, so a
// cloud deployment that forgot LUMEN_PUBLIC_URL fell through and emitted a
// `setup self-host --server-url https://lumen.ai` command — pointing the
// daemon's backend at the frontend (no /health, no WebSocket proxy).
func isOfficialCloudDaemonConfig(appURL string) bool {
	return urlHostEquals(appURL, "lumen.ai")
}

// isOfficialCloudDeployment reports whether this server is the official Lumen
// Cloud, reusing the same frontend-host signal as the daemon setup (lumen.ai).
