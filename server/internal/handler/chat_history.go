...
// channel table-of-contents (`lumen chat history`), Thread reads one thread's
// messages (`lumen chat thread [id]`). Both are scoped server-side to the
...
// GetChatChannelHistory serves `lumen chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `lumen chat thread`.
//
...
// paging contract. It backs `lumen chat history` for sessions with no IM
// channel (web chat, Feishu, WeCom, DingTalk), whose history lives only in
// Lumen — there is no platform to read back. It pages through the same
...
// GetChatThread serves `lumen chat thread [id]` — one thread's messages. With
