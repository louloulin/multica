...
const (
	onboardingAssistantName       = "Lumen Helper"
	onboardingIssueTitle          = "Start here: learn Lumen with Lumen Helper"
	onboardingAgentCreationSource = "lumen_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Lumen questions and runs CLI operations."
...
// Lumen Helper agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are Lumen Helper, the built-in AI assistant for this Lumen workspace. Your role is to help any member use Lumen better — answer questions, give advice, and execute workspace operations on their behalf.

## What Lumen is

Lumen is an open-source, AI-native team workspace (source: https://github.com/lumen-ai/lumen). The core idea: AI agents are treated as real teammates — they get assigned issues on a kanban-style board, comment in threads, change status, and run code, exactly like human members. You can also chat directly with agents (chat), group them into squads, and run scheduled or triggered automation (autopilot).

For concept details (workspace / issue / project / agent / runtime / skill / squad / autopilot / inbox / chat session): fetch https://lumen.ai/docs via WebFetch — that's authoritative. For the "why" or implementation, fetch the GitHub repo above. Never paraphrase concepts from memory.

For ANY product-usage problem the user runs into (bug, unclear behavior, missing feature, improvement idea), suggest they file an issue at https://github.com/lumen-ai/lumen/issues — that's the official feedback channel.

## What you can do

Your toolbox is the ` + "`lumen`" + ` CLI. It's already on your PATH and authenticated as the workspace owner.

Your full capability surface = whatever ` + "`lumen --help`" + ` shows. Run ` + "`lumen --help`" + ` first, then ` + "`lumen <command> --help`" + ` for any subcommand; use ` + "`--output json`" + ` for structured data. The CLI is your manifest — never invent commands or flags.
...
const onboardingIssueDescription = `Welcome to Lumen.

This is your guided first run. Lumen Helper is assigned to this issue and will help you try the core workflow:

1. Read Lumen Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Lumen Helper when you want it to continue.
...
//
// Creates or reuses one "Lumen Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to Lumen.",
		"",
		"Agents need a runtime before they can execute work. You can still use Lumen as a lightweight project-management workspace while you install one.",
		"",
		"## Try Lumen first",
		"",
		...
		"",
		"Full guide: https://lumen.ai/docs/install-agent-runtime",
		"",
		...
		"   which codex",
		"   codex --version",
		"5. Restart the Lumen daemon:",
		"   lumen daemon restart",
		...
		"",
		"When the runtime is connected, you can create Lumen Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 Lumen。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 Lumen 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://lumen.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Lumen Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
