...
func TestJoinCloudWaitlistRecordsEmailAndReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-ok@lumen.ai")
...
func TestJoinCloudWaitlistAllowsEmptyReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-noreason@lumen.ai")
...
func TestJoinCloudWaitlistMissingEmailReturns400(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-missing@lumen.ai")
...
func TestJoinCloudWaitlistRejectsOverlongReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-long@lumen.ai")
...
func TestJoinCloudWaitlistSecondCallOverwrites(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-overwrite@lumen.ai")
...
func TestBootstrapOnboardingRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	if avatarURL == nil || *avatarURL != onboardingAssistantAvatarURL {
		t.Fatalf("agent avatar_url = %v, want seeded Lumen Helper avatar", avatarURL)
	}
...
func TestBootstrapOnboardingRuntime_WithStarterPrompt(t *testing.T) {
	...
	const wantPrompt = "Introduce Lumen to me, please."
	body := map[string]string{
		"workspace_id":   testWorkspaceID,
		"runtime_id":     testRuntimeID,
...
func TestBootstrapOnboardingNoRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	for _, want := range []string{
		"Try Lumen first",
		"https://lumen.ai/docs/install-agent-runtime",
...
func TestPatchOnboardingSplitsQuestionnaireAndSourceEvents(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-event-split@lumen.ai")

	m := obsmetrics.NewBusinessMetrics()
	h := *testHandler
	h.Metrics = m

	const questionnaireCounter = "lumen_onboarding_questionnaire_submitted_total"
	const sourceCounter = "lumen_onboarding_source_submitted_total"
...
func TestPatchOnboardingAllSkippedResolvesBothCounters(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-source-skip@lumen.ai")
	...
	if got := counterValue(t, m, "lumen_onboarding_questionnaire_submitted_total"); got != 1 {
		t.Fatalf("expected questionnaire counter 1 for all-skip, got %v", got)
	}
	if got := counterValue(t, m, "lumen_onboarding_source_submitted_total"); got != 1 {
