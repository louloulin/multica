...
import (
	...
	dingtalkintegration "github.com/lumen-ai/lumen/server/internal/integrations/dingtalk"
...
func dingTalkAgentPermissionCases(t *testing.T, prefix, agentOwnerID, memberID string) []dingTalkAgentPermissionCase {
	t.Helper()
	adminID := createPermissionTestAdmin(t, prefix+"-admin@lumen.test")
	nonMemberID := createDingTalkNonMember(t, prefix+"-non-member@lumen.test")
...
func TestListDingTalkGroupsForAgent_FollowsAgentViewPermissionMatrix(t *testing.T) {
	...
	adminID := createPermissionTestAdmin(t, "dingtalk-view-admin@lumen.test")
	targetMemberID := createPermissionTestMember(t, "dingtalk-view-target@lumen.test")
	nonMemberID := createDingTalkNonMember(t, "dingtalk-view-non-member@lumen.test")
	t.Cleanup(func() {
...
func TestListDingTalkSettingsData_FollowsAgentViewPermissionMatrix(t *testing.T) {
	...
	agentID, ownerID, unrelatedMemberID := privateAgentTestFixture(t)
	adminID := createPermissionTestAdmin(t, "dingtalk-settings-view-admin@lumen.test")
	targetMemberID := createPermissionTestMember(t, "dingtalk-settings-view-target@lumen.test")
	nonMemberID := createDingTalkNonMember(t, "dingtalk-settings-view-non-member@lumen.test")
...
func TestRevokeDingTalkInstallation_OrphanCleanableByAdminNotMember(t *testing.T) {
	...
	adminID := createPermissionTestAdmin(t, "dingtalk-orphan-admin@lumen.test")
	nonMemberID := createDingTalkNonMember(t, "dingtalk-orphan-non-member@lumen.test")
...
func TestRedeemDingTalkBindingTokenPublishesAccountBindingUpdateAfterCommit(t *testing.T) {
	...
	if bindingAtEvent.LumenUserID != parseUUID(testUserID) {
		t.Errorf("binding user = %v, want %s", bindingAtEvent.LumenUserID, testUserID)
	}
