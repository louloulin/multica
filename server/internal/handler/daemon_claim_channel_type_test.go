...
// mis-flag made the daemon inject `lumen attachment upload` guidance into a
...
// `lumen chat history` and `lumen chat thread`, and both endpoints are
...
// inherited the web chat's `lumen attachment upload` guidance, the exact
...
func TestClaim_ChannelOriginChatWithoutTaskDeliveryIsPrivateLumenTurn(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
