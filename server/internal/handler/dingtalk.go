...
// DingTalkGroupBotResponse identifies one connected Lumen bot observed in a
...
// Settings inventory of observed groups and connected Lumen bots. Workspace
...
func (h *Handler) RegisterDingTalkBYO(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, dingtalk.ErrRobotOwnedByAnotherWorkspace):
			writeError(w, http.StatusConflict, "this DingTalk robot is already connected to a different Lumen workspace — disconnect it there before connecting it here")
...
// DingTalk user id carried by the bearer token to the logged-in Lumen user.
...
func (h *Handler) RedeemDingTalkBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, dingtalk.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this DingTalk account is already bound to a different Lumen user")
