...
// Key layout:
//
//   lumen:{runtime_pending}:model_list:req:<request_id>           → JSON-encoded ModelListRequest, TTL = retention
//   lumen:{runtime_pending}:model_list:pending:<runtime_id>       → ZSET { member = request_id, score = created_at UnixNano }
...
const (
	// Namespaced under lumen:*:model_list:* so the key set doesn't collide with
	// the realtime relay (ws:*) or the local-skill stores.
	modelListKeyPrefix          = "lumen:" + runtimePendingRedisHashTag + ":model_list:req:"
	modelListPendingPrefix      = "lumen:" + runtimePendingRedisHashTag + ":model_list:pending:"
