...
type WecomBindingRedeemer interface {
	RedeemAndBind(ctx context.Context, raw string, lumenUserID pgtype.UUID) (wecom.RedeemedBindingToken, error)
}
...
func writeWecomInstallError(w http.ResponseWriter, err error, wsUUID, agentUUID pgtype.UUID) {
	...
	case errors.Is(err, wecom.ErrBotOwnedByAnotherWorkspace):
		writeErrorCode(w, http.StatusConflict, "wecom_bot_owned_by_another_workspace",
			"this bot is already connected to a different Lumen workspace — disconnect it there before connecting it here")
...
// through from the bot's "link your Lumen account" prompt.
...
// aibot userid carried by the token to the logged-in Lumen user. The
...
func (h *Handler) RedeemWecomBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, wecom.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this WeCom user is already bound to a different Lumen user")
