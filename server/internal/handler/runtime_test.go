...
func TestResolveViewingTZ(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Resolve', 'tz-resolve@lumen.ai', 'Asia/Tokyo') RETURNING id`,
	).Scan(&userID); err != nil {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email)
		 VALUES ('TZ Bare', 'tz-bare@lumen.ai') RETURNING id`,
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Bad', 'tz-bad@lumen.ai', 'Bad/Zone') RETURNING id`,
