...
func runtimeVisibilityFixture(t *testing.T) (runtimeID, runtimeOwnerID, plainMemberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Runtime Owner', 'runtime-owner@lumen.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'runtime-owner@lumen.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Runtime Member', 'plain-runtime-member@lumen.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-runtime-member@lumen.test'`)
	})
